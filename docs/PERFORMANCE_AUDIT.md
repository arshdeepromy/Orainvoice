...
| 3 | Add the index pack in [Appendix A](#appendix-a) via `CREATE INDEX CONCURRENTLY` | 1 d | Customer search and invoice list go from seq-scan to index-scan. | None — `CONCURRENTLY` takes only SHARE UPDATE EXCLUSIVE lock. Run after-hours. | ✅ DONE 2026-05-30 — Alembic migration 0202 adds 20 indexes via `CREATE INDEX CONCURRENTLY` (D-H4 hot list ordering, D-H5 missing FKs, D-H7 pg_trgm GIN for customer search, D-H9 partial overdue index, D-M1 active-row partials, D-M5 covering subquery index) plus the `pg_trgm` extension. Verified on dev: planner now uses Index Scan on `idx_invoices_org_created_desc` for invoice list (was seq-scan + sort) and Bitmap Index Scan on `idx_customers_first_trgm` for `first_name ILIKE '%foo%'` (was seq-scan). Migration uses `op.get_context().autocommit_block()` so each `CONCURRENTLY` runs outside a transaction; reversible (downgrade verified). |
| 4 | Reduce DB pool: `pool_size=15`, `max_overflow=10` × 4 workers = 100 conns. Drop Postgres `max_connections` to 80, `work_mem` to 8 MB | 0.5 d | Eliminates connection-storm risk; ~30 GB of theoretical work-mem reclaimed. | Requires PG restart + app restart. Roll during low-traffic window. Reversible by reverting compose values. | 🚫 NOT REQUIRED 2026-05-30 — skipped at user discretion. Current scale (7 orgs, 72 invoices, 9 users) shows no measurable gain from the retune; connection-storm + work_mem blow-up are forward-looking risks that do not justify a PG restart maintenance window today. Revisit if/when load grows toward the 200-org range (see §2 capacity trajectory) or if Postgres OOM / connection saturation is actually observed. |
...
- **Status:** ⚠️ PARTIAL 2026-05-30 — migration 0202 is now the canonical CONCURRENTLY pattern (uses `op.get_context().autocommit_block()` to escape Alembic's transaction wrapper, plus `IF NOT EXISTS` / `IF EXISTS` guards for re-runnability). All future index DDL should follow this template. CI lint to enforce / reject `op.create_index(` is still pending.
...
- [x] **Index pack from [Appendix A](#appendix-a)** via `CREATE INDEX CONCURRENTLY`. Budget ~5 min per index on 5 M-row tables. Live-safe. ✅ DONE 2026-05-30 — Alembic 0202 applied on dev, planner verified picking up `idx_invoices_org_created_desc` and `idx_customers_first_trgm`. Pending PROD apply on the next deploy.
- [ ] ~~**Pool size 15 + overflow 10 × 4 workers; PG `max_connections=80`, `work_mem=8MB`.**~~ 🚫 **NOT REQUIRED** 2026-05-30 — skipped at user discretion. No measurable gain at current scale (7 orgs, 9 users). Revisit if Postgres OOM or connection saturation appears, or load grows toward 200 orgs.
- [ ] ~~**`lock_timeout=5000` in Postgres command** (D-M7)~~ 🚫 **NOT REQUIRED** 2026-05-30 — bundled with the pool retune above; same rationale.
