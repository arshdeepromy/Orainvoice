...
Surfaced again during Phase 4 task 4.1 scoped run (2026-05-27) — one new pre-existing failure, no C3 regressions:

- `tests/test_auth_password_reset.py::TestCompletePasswordReset::test_resets_password_and_invalidates_sessions` — `pydantic_core._pydantic_core.ValidationError: 4 validation errors for OrgSecuritySettings`. The test mocks `db.execute()` with a plain `AsyncMock`, so `get_security_settings` reads `MagicMock` proxy objects out of the JSONB `settings` column and Pydantic rejects them. Predates Phase 4 — verified by stashing the C3 changes and re-running the test against a clean checkout (same failure). Same class of bug as failure #2 above (test expectations drifted from the implementation). The 4 `TestRequestPasswordReset` tests that exercise the migrated `_send_password_reset_email` call path all pass.
