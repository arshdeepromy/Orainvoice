 ...
 *
 * Validates: Requirement 9.1, 9.7, 10.1
 */
export default function ProductDetail({ productId }: { productId?: string }) {
  ...
  if (loading) {
    return <div className="py-16"><Spinner label="Loading product" /></div>
  }
  ...
  const detailsTab = (
    <form onSubmit={handleSave} className="max-w-2xl space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Input label="Product name *" value={form.name} onChange={(e) => updateField('name', e.target.value)} />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Input label="Low stock threshold" type="number" step="1" value={form.low_stock_threshold} onChange={(e) => updateField('low_stock_threshold', e.target.value)} />
        <Input label="Reorder quantity" type="number" step="1" value={form.reorder_quantity} onChange={(e) => updateField('reorder_quantity', e.target.value)} />
      </div>
      ...
      <div>
        ...
        {existingImages.length + imageFiles.length < 5 && (
          <input type="file" accept="image/*" multiple onChange={handleImageUpload} className="mt-2 text-sm" aria-label="Upload product images" />
        )}
      </div>
  ...
  const pricingRulesTab = (
    <div>
      {pricingRules.length === 0 ? (
        <p className="text-sm text-gray-500 py-8 text-center">No pricing rules configured for this product.</p>
      ) : (
        <div className="overflow-x-auto rounded-lg border border-gray-200">
          <table className="min-w-full divide-y divide-gray-200" role="grid">
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-6">
      <h1 className="text-2xl font-semibold text-gray-900 mb-4">
        {productId ? (product?.name || 'Product') : 'New Product'}
      </h1>
