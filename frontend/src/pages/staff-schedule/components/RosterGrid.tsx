    ...
    >
      ...
      >
        ...
        {Array.from({ length: cols }).map((_, c) => (
          <div
            key={`skh-${c}`}
            className="border-b border-r border-gray-200 bg-gray-50 p-2"
          >
...
function EmptyStaffState() {
  return (
    <div
      className="mx-auto mt-12 max-w-md rounded-lg border border-gray-200 bg-white p-8 text-center"
    ...
    >
      ...
      <Link
        to="/staff"
        className="mt-4 inline-block rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500"
      >
        Go to Staff
      </Link>
    </div>
    ...
    >
      ...
      >
        ...
        <div
          role="columnheader"
          aria-label="Staff"
          className="sticky top-0 z-10 border-b border-r border-gray-200 bg-gray-50 px-2 py-2 font-medium text-gray-700"
        >
          Staff
        </div>
        {dates.map((d, idx) => {
          ...
          return (
            <button
              ...
              className={`sticky top-0 z-10 cursor-pointer border-b border-r border-gray-200 px-2 py-2 text-center font-medium text-gray-700 ${
                isWeek2Start ? 'border-l-2 border-l-gray-400' : ''
              } ${
                isSelectedDay ? 'bg-blue-100' : 'bg-gray-50 hover:bg-gray-100'
              }`}
            >
              <div>{dow}</div>
          ...
          return (
            ...
            >
              <button
                ...
                className={`cursor-pointer border-b border-r border-gray-200 px-2 py-2 text-left ${
                  isSelectedStaff ? 'bg-blue-100' : 'bg-white hover:bg-gray-50'
                }`}
              >
                ...
                {s.position && (
                  <div className="text-[11px] text-gray-500">{s.position}</div>
                )}
              </button>
              {dates.map((d, colIndex) => {
                ...
                const baseCellClass = `relative min-h-[56px] border-b border-r border-gray-200 p-1 ${
                  isWeek2Start ? 'border-l-2 border-l-gray-400' : ''
                }`
                ...
                if (isLeave) {
                  return (
                    <div
                      ...
                      className={`${baseCellClass} bg-gray-100 text-gray-500 ${
                        isInPaint ? 'ring-2 ring-blue-400' : ''
                      } ${
                        isConflict ? 'outline outline-2 outline-red-500' : ''
                      }`}
                      style={{
                        backgroundImage:
                ...
                return (
                  <button
                    ...
                    className={`${baseCellClass} group cursor-pointer text-left hover:bg-blue-50 focus:bg-blue-50 focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                      isInPaint ? 'ring-2 ring-blue-400 bg-blue-50' : ''
                    } ${
                      isInSelectionRect ? 'bg-blue-50' : ''
                    } ${
                      isConflict ? 'outline outline-2 outline-red-500' : ''
                    } ${isFocused ? 'ring-2 ring-blue-500' : ''}`}
                  >
                    {empty ? (
                      <span className="invisible text-[11px] text-blue-600 group-hover:visible">
                        + Add
                      </span>
                    ) : (
                            ...
                            >
                              ...
                              {onResizeCommit && !isSaving && (
                                <span
                                  ...
                                  className="absolute right-0 top-0 h-full w-[6px] cursor-ew-resize bg-transparent hover:bg-blue-300/40"
                                  onPointerDown={(ev) =>
