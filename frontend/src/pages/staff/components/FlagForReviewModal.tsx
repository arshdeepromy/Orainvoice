/**
 * FlagForReviewModal — confirms flagging a clock entry for follow-up
 * review (G10).
 *
 * POSTs `/api/v2/staff/{staff_id}/clock-entries/{entry_id}/flag` with an
 * optional reason; the row's `flags.flagged_for_review` jsonb key flips
 * to `true`. Server enforces RBAC (org_admin / branch_admin /
 * location_manager only).
 *
 * Refs: Phase 3 G10 / R8.3 / R16. Touch targets ≥ 44×44.
 */
...
export interface FlagForReviewModalProps {
  ...
  entryId: string
  /**
   * Optional context shown in the modal header (e.g. "Mon 9 Jun ·
   * 08:42 → 17:08").
   */
      ...
      <div className="w-full max-w-md overflow-hidden rounded-lg bg-white shadow-xl dark:bg-gray-900">
        ...
        <div className="space-y-4 px-6 py-4">
          ...
          <p className="text-sm text-gray-700 dark:text-gray-300">
            This entry will be marked with a 🚩 flag. The week's approval
            modal will require explicit acknowledgement before approving.
          </p>
          ...
          <div className="flex flex-wrap justify-end gap-2 pt-2">
            ...
            <button
              ...
              disabled={submitting}
              className="min-h-[44px] rounded-lg bg-amber-600 px-4 py-2 text-sm font-medium text-white hover:bg-amber-700 disabled:cursor-not-allowed disabled:opacity-50"
