    ...
    <div>
      ...
      {step === 'upload' && (
        <div className="space-y-4">
          ...
          <div
            className="cursor-pointer rounded-card border-2 border-dashed border-border p-8 text-center transition-colors hover:border-accent"
          ...
          >
            <svg className="mx-auto h-12 w-12 text-muted-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
            </svg>
            <p className="mt-2 text-sm text-text">Click to upload or drag and drop</p>
            <p className="text-xs text-muted-2">CSV files only</p>
            <input
              ref={fileInputRef}
              type="file"
              accept=".csv"
      ...
      {step === 'mapping' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-lg font-medium text-text">Map Fields</h3>
          ...
          <div className="divide-y divide-border rounded-card border border-border bg-card">
            {mappings.map((m, idx) => (
              <div key={m.csvHeader} className="flex items-center gap-4 px-4 py-3">
                <span className="w-40 truncate text-sm font-medium text-text" title={m.csvHeader}>
                  {m.csvHeader}
                </span>
      ...
      {step === 'preview' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-lg font-medium text-text">Validation Preview</h3>
              <p className="text-sm text-muted">
          ...
          <div className="overflow-x-auto overflow-hidden rounded-card border border-border">
            <table className="min-w-full divide-y divide-border" role="grid" aria-label="Validation preview">
              <thead className="bg-canvas">
                <tr role="row">
                  <th scope="col" className="mono px-4 py-2 text-left text-[10.5px] font-medium uppercase tracking-[0.08em] text-muted-2">Row</th>
                  <th scope="col" className="mono px-4 py-2 text-left text-[10.5px] font-medium uppercase tracking-[0.08em] text-muted-2">Status</th>
                  {mappings.filter((m) => m.targetField).map((m) => (
                    <th key={m.targetField} scope="col" className="mono px-4 py-2 text-left text-[10.5px] font-medium uppercase tracking-[0.08em] text-muted-2">
                      {targetFields.find((f) => f.value === m.targetField)?.label || m.targetField}
                    </th>
                  ))}
                  <th scope="col" className="mono px-4 py-2 text-left text-[10.5px] font-medium uppercase tracking-[0.08em] text-muted-2">Errors</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border bg-card">
                {validationRows.slice(0, 50).map((vr) => (
                  <tr key={vr.row} role="row" className={vr.valid ? '' : 'bg-danger-soft'}>
                    <td className="mono px-4 py-2 text-sm text-text">{vr.row}</td>
                    <td className="px-4 py-2 text-sm">
                      {vr.valid ? (
                        <span className="font-medium text-ok">Valid</span>
                      ) : (
                        <span className="font-medium text-danger">Error</span>
                      )}
                    </td>
                    {mappings.filter((m) => m.targetField).map((m) => (
                      <td key={m.targetField} className="max-w-[200px] truncate px-4 py-2 text-sm text-text">
                        {vr.data[m.targetField] || '—'}
                      </td>
                    ))}
                    <td className="px-4 py-2 text-sm text-danger">
                      {vr.errors.join('; ')}
                    </td>
                  </tr>
                ))}
          ...
          {validationRows.length > 50 && (
            <p className="text-sm text-muted">Showing first 50 of {validationRows.length} rows.</p>
          )}
        ...
        <div className="space-y-6 py-8 text-center">
          ...
          <div>
            ...
            <div className="mx-auto max-w-md">
              ...
              <p className="mt-1 text-xs text-muted">{importProgress}% complete</p>
            </div>
          </div>
        </div>
      )}
      ...
      {step === 'results' && importResult && (
        <div className="space-y-4">
          <h3 className="text-lg font-medium text-text">Import Complete</h3>

          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            <div className="rounded-card border border-border bg-card p-4 text-center">
              <p className="mono text-2xl font-bold text-text">{(importResult.imported_count ?? 0) + (importResult.skipped_count ?? 0)}</p>
              <p className="text-sm text-muted">Total Rows</p>
            </div>
            <div className="rounded-card border border-ok bg-ok-soft p-4 text-center">
              <p className="mono text-2xl font-bold text-ok">{importResult.imported_count}</p>
              <p className="text-sm text-ok">Imported</p>
            </div>
            <div className="rounded-card border border-danger bg-danger-soft p-4 text-center">
              <p className="mono text-2xl font-bold text-danger">{importResult.skipped_count}</p>
              <p className="text-sm text-danger">Skipped</p>
            </div>
          </div>
