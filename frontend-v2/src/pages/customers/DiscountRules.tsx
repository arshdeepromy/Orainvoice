/**
 * DiscountRules — Task 24 port of frontend/src/pages/customers/DiscountRules.tsx.
 *
 * ALL logic is copied VERBATIM from the original: the discount-rule CRUD
 * (GET/POST/PUT/DELETE /customers/discount-rules), the create/edit modal with
 * rule-type-conditional threshold field, the percentage/fixed discount value
 * formatting, the active/inactive toggle (inline PUT), and the delete
 * confirmation. The only changes are safe-API consumption hardening
 * (`res.data?.discount_rules ?? []`) and remapping presentation onto the design
 * system.
 *
 * This page has no dedicated HTML prototype, so the surface is designed on the
 * fly (FR-2b) in the established v2 language: `page page-wide` head with an
 * eyebrow, a card-wrapped token table (uppercase `.mono` column heads, hover
 * rows), Badge status pills, and the shared Modal/Button/Input/Select
 * primitives. `.mono` is applied to numeric threshold / discount cells (FR-2).
 */
...
/**
 * Map the original page's rule-type badge variants onto the v2 Badge tones.
 * The original used the ui kit's success/warning/error/info/neutral set; v2
 * Badge exposes info / warn / neutral (+ others), so visit_count→info,
 * spend_threshold→warn, customer_tag→neutral is preserved 1:1.
 */
function ruleTypeBadgeVariant(type: RuleType): BadgeVariant {
  switch (type) {
    case 'visit_count': return 'info'
    case 'spend_threshold': return 'warn'
  ...
  return (
    <div className="page page-wide">
      ...
      <div className="page-head">
        <div>
          <div className="eyebrow">People · Loyalty</div>
          <h1>Discount Rules</h1>
          <p className="sub">Manage loyalty discounts based on visit count, spend, or customer tags</p>
        </div>
            ...
            <table className="w-full border-collapse" role="grid">
              ...
              <tbody>
                {rules.length === 0 ? (
                  ...
                  rules.map((rule) => (
                    <tr key={rule.id} className="border-b border-border transition-colors last:border-b-0 hover:bg-canvas">
                      <td className="whitespace-nowrap px-5 py-3 text-[13.5px] font-medium text-text">{rule.name}</td>
                      ...
                      <td className="mono whitespace-nowrap px-5 py-3 text-right text-[13.5px] text-muted">
                        {rule.threshold_value || '—'}
                      </td>
                      <td className="mono whitespace-nowrap px-5 py-3 text-right text-[13.5px] font-medium text-text">
                        {rule.discount_type === 'percentage' ? `${rule.discount_value}%` : `${rule.discount_value}`}
                      </td>
                      <td className="whitespace-nowrap px-5 py-3 text-center text-[13.5px]">
                        <button
