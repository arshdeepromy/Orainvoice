/**
 * BookingListPanel — Task 28 port of frontend/src/pages/bookings/BookingListPanel.tsx.
 *
 * Table of bookings for the selected calendar range, rendered below the
 * BookingCalendar. ALL logic copied VERBATIM: the fetch (GET /bookings ?view&
 * date with local-date string to avoid UTC shift), the Scheduled/Completed tab
 * split, cancel (PUT /bookings/:id status=cancelled), Create Job (callback) vs
 * Confirm & Invoice (POST /bookings/:id/convert target=invoice → redirect),
 * the ConfirmDialog, the imperative markConverted ref handle (in-place row flash),
 * and the exported pure helpers (canActOnBooking / bookingSortKey /
 * sortBookingsByStartTime / filterBookingsByDateRange). Presentation remapped
 * onto the design tokens (FR-2b).
 *
...
function BookingListPanel({
  ...
  if (loading) {
    return (
      <div className="mt-6 py-8 text-center" role="status" aria-label="Loading bookings">
        <Spinner />
        <p className="mt-2 text-[13px] text-muted">Loading bookings…</p>
      </div>
    )
  }
  ...
  if (error) {
    return (
      <div className="mt-6 rounded-ctl border border-danger/30 bg-danger-soft px-4 py-3 text-[13px] text-danger" role="alert">
        {error}
      </div>
    )
  }
  ...
  if (bookings.length === 0) {
    return (
      <div className="mt-6 py-8 text-center text-[13px] text-muted">
  ...
  return (
    <div className="mt-6">
      <h2 className="mb-3 text-[15px] font-medium text-text">Bookings</h2>
      ...
      <div className="overflow-x-auto rounded-b-card border border-t-0 border-border">
        <table className="w-full border-collapse" role="table">
          <thead>
            <tr>
              ...
              {isAutomotive && <th scope="col" className={TH}>Vehicle Rego</th>}
          ...
          <tbody>
            {filteredBookings.map((b) => {
              ...
              return (
                <tr
                  key={b.id}
                  className={`border-b border-border transition-colors duration-700 last:border-b-0 ${
                    flashId === b.id
                      ? 'bg-ok-soft'
                      : muted
                        ? 'opacity-50'
                ...
                >
                  <td className="px-4 py-3 text-[13.5px] text-text">{b.customer_name ?? '—'}</td>
                  <td className="px-4 py-3 text-[13.5px] text-muted">{b.service_type ?? '—'}</td>
                  <td className="mono px-4 py-3 text-[13px] text-muted">{dateStr ? formatDateTime(dateStr) : '—'}</td>
                  {isAutomotive && <td className="mono px-4 py-3 text-[13px] text-muted">{b.vehicle_rego ?? '—'}</td>}
