 ...
 *
 * Logic copied verbatim from frontend/src/pages/scheduling/StaffSchedule.tsx;
 * the page chrome (page-head, add-shift form card, token table) is remapped
 * onto the design-system tokens per the OraInvoice_Handoff prototype.
 */
    ...
    <div className="page page-wide">
      ...
      <div className="page-head">
        <div>
          ...
          <p className="sub">Staff schedule{selectedBranchId ? '' : ' across all branches'}.</p>
        </div>
        <div className="head-actions flex items-center gap-2">
          <Link
            to="/staff-schedule/grid"
            className="hidden lg:inline-flex h-[34px] items-center rounded-ctl border border-border bg-card px-3 text-sm font-medium text-text hover:bg-canvas hover:border-border-strong focus:outline-none focus:ring-2 focus:ring-accent"
          >
            Grid view
          </Link>
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="mb-6 rounded-card border border-border bg-card p-4 space-y-3 shadow-card">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            <div>
              <label htmlFor="sched-user" className={labelCls}>Staff Member</label>
            ...
            {!selectedBranchId && (
              <div>
                <label htmlFor="sched-branch" className={labelCls}>Branch</label>
      ...
      {grouped ? (
        Object.entries(grouped).map(([branchName, branchEntries]) => (
          <div key={branchName} className="mb-6">
            <h3 className="text-sm font-semibold text-text mb-2">{branchName}</h3>
      ...
      {entries.length === 0 && !loading && (
        <p className="text-center text-sm text-muted py-12">No schedule entries found.</p>
      )}
    </div>
  )
}
    ...
    <div className="overflow-x-auto rounded-card border border-border bg-card shadow-card">
      <table className="min-w-full divide-y divide-border" role="grid">
        ...
        <thead className="bg-canvas">
          <tr>
            {showBranch && <th scope="col" className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-muted">Branch</th>}
            <th scope="col" className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-muted">Staff</th>
            <th scope="col" className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-muted">Date</th>
            <th scope="col" className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-muted">Start</th>
            <th scope="col" className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-muted">End</th>
            <th scope="col" className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-muted">Notes</th>
            <th scope="col" className="px-4 py-3 text-center text-xs font-medium uppercase tracking-wider text-muted">Actions</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-border bg-card">
          {entries.map((e) => (
            <tr key={e.id} className="hover:bg-canvas">
              {showBranch && <td className="whitespace-nowrap px-4 py-3 text-sm text-text">{e.branch_name ?? '—'}</td>}
              <td className="whitespace-nowrap px-4 py-3 text-sm text-text">{e.user_name ?? '—'}</td>
              <td className="whitespace-nowrap px-4 py-3 text-sm text-text mono">{e.shift_date}</td>
              <td className="whitespace-nowrap px-4 py-3 text-sm text-text mono">{e.start_time}</td>
              <td className="whitespace-nowrap px-4 py-3 text-sm text-text mono">{e.end_time}</td>
              <td className="px-4 py-3 text-sm text-muted max-w-xs truncate">{e.notes ?? '—'}</td>
