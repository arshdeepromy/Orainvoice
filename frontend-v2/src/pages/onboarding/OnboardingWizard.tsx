  ...
  return (
    <div className="mb-8">
      <div className="flex items-center justify-between mb-2">
        ...
        <span className="text-sm text-muted">{STEP_LABELS[current]}</span>
      </div>
...
function StepOrgContact({
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-xl font-semibold text-text">Organisation Details</h2>
      <p className="text-sm text-muted">Enter your workshop's contact information.</p>
      <Input
...
function StepBranding({
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-xl font-semibold text-text">Branding</h2>
      <p className="text-sm text-muted">Upload your logo and choose brand colours.</p>
      <div className="flex flex-col gap-1">
        ...
        />
        {data.logoFile && (
          <p className="text-xs text-muted">Selected: {data.logoFile.name}</p>
        )}
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="flex flex-col gap-1">
          ...
          <div className="flex items-center gap-3">
            ...
            />
            <span className="text-sm text-muted">{data.primaryColour}</span>
          </div>
        </div>
        <div className="flex flex-col gap-1">
          ...
          <div className="flex items-center gap-3">
            <input
              ...
              className="h-10 w-14 cursor-pointer rounded-ctl border border-border"
            />
            <span className="text-sm text-muted">{data.secondaryColour}</span>
          </div>
        </div>
      </div>
    </div>
...
function StepGst({
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-xl font-semibold text-text">GST Settings</h2>
      <p className="text-sm text-muted">Configure your GST details for NZ tax compliance.</p>
      <Input
      ...
      <div className="flex items-center gap-3">
        <button
          ...
          className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors ${
            data.gstInclusive ? 'bg-accent' : 'bg-border'
          }`}
        >
          <span
...
function StepInvoice({
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-xl font-semibold text-text">Invoice Numbering</h2>
      <p className="text-sm text-muted">Set up your invoice number format.</p>
      <Input
      ...
      {data.invoicePrefix && data.startingNumber && (
        <p className="text-sm text-muted">
          Preview: <span className="font-mono font-medium text-text">{data.invoicePrefix}{data.startingNumber}</span>
        </p>
      )}
    </div>
  )
...
function StepPaymentTerms({
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm text-muted">Configure default payment terms for your invoices.</p>
      <Input
      ...
      />
      <div className="flex flex-col gap-1">
        <label htmlFor="payment-terms-text" className="text-sm font-medium text-text">
          Payment Terms Text
        </label>
        <textarea
          ...
          className="rounded-ctl border border-border bg-card px-3 py-2 text-text shadow-sm
            placeholder:text-muted-2 focus:outline-none focus:ring-2 focus:ring-accent focus:border-accent"
        />
        <p className="text-sm text-muted">This text appears on your invoice PDFs.</p>
      </div>
    </div>
  )
}
...
function StepServiceType({
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-xl font-semibold text-text">First Service Type</h2>
      <p className="text-sm text-muted">Add your first service to the catalogue. You can add more later.</p>
      <Input
        label="Service Name"
    ...
    <div className="bg-canvas flex items-start justify-center px-4 py-12 sm:py-16">
      <div className="w-full max-w-lg">
        <div className="text-center mb-8">
          <h1 className="text-2xl font-bold text-text">Set Up Your Workshop</h1>
        ...
        <div className="bg-card rounded-card shadow-card border border-border p-6 sm:p-8">
          {renderStep()}
