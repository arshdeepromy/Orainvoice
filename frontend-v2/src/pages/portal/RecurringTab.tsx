...
export function RecurringTab({ token }: RecurringTabProps) {
  ...
  if (schedules.length === 0) return <p className="py-8 text-center text-sm text-muted">No recurring schedules found.</p>
  ...
  return (
    <div className="space-y-4">
      {schedules.map((sched) => {
        ...
        return (
          <div key={sched.id} className="rounded-card border border-border bg-card shadow-card p-4">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                <Badge variant={statusCfg.variant}>{statusCfg.label}</Badge>
                <span className="text-sm font-semibold text-text">{freqLabel}</span>
              </div>
            ...
            <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted">
              <span>
                Next billing: <span className="text-text">{formatDate(sched.next_generation_date, locale)}</span>
              </span>
              <span>
                Started: <span className="text-text">{formatDate(sched.start_date, locale)}</span>
              </span>
              {sched.end_date && (
                <span>
                  Ends: <span className="text-text">{formatDate(sched.end_date, locale)}</span>
                </span>
              )}
              {sched.auto_issue && (
                <span className="text-ok">Auto-issue</span>
              )}
            </div>
          </div>
        )
