...
const STATUS_CONFIG: Record<string, { label: string; variant: 'success' | 'warn' | 'danger' | 'info' | 'neutral' }> = {
  open: { label: 'Pending', variant: 'warn' },
...
export function JobsTab({ token }: JobsTabProps) {
  ...
  if (jobs.length === 0) return <p className="py-8 text-center text-sm text-muted">No jobs found.</p>
  ...
  return (
    <div className="space-y-4">
      {jobs.map((job) => {
        ...
        return (
          <div key={job.id} className="rounded-card border border-border bg-card shadow-card p-4">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                <Badge variant={statusCfg.variant}>{statusCfg.label}</Badge>
                {job.vehicle_rego && (
                  <span className="mono text-sm font-medium text-text">{job.vehicle_rego}</span>
                )}
              </div>
              <span className="text-xs text-muted-2">{formatDate(job.created_at, locale)}</span>
            </div>

            {job.description && (
              <p className="text-sm text-text mb-2">{job.description}</p>
            )}

            <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted">
              {job.assigned_staff_name && (
                <span>Assigned to: <span className="text-text">{job.assigned_staff_name}</span></span>
              )}
              {job.linked_invoice_number && (
                <span>Invoice: <span className="text-text">{job.linked_invoice_number}</span></span>
              )}
              {job.estimated_completion && (
                <span>Est. completion: <span className="text-text">{formatDate(job.estimated_completion, locale)}</span></span>
              )}
            </div>
          </div>
        )
