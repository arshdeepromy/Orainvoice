...
const QUOTE_STATUS: Record<string, { label: string; variant: 'success' | 'warn' | 'danger' | 'info' | 'neutral' }> = {
...
export function QuoteAcceptance({ token }: QuoteAcceptanceProps) {
  ...
  if (quotes.length === 0) return <p className="py-8 text-center text-sm text-muted">No quotes found.</p>
  ...
  return (
    <div className="space-y-3">
      {quotes.map((q) => {
        ...
        return (
          <div key={q.id} className="rounded-card border border-border bg-card shadow-card p-4">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  <span className="mono font-medium text-text">{q.quote_number}</span>
