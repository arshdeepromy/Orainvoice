...
export function AssetHistory({ token }: AssetHistoryProps) {
  ...
  if (assets.length === 0) return <p className="py-8 text-center text-sm text-muted">No assets found.</p>

  return (
    <div className="space-y-4">
      {assets.map((asset) => (
        <div key={asset.id} className="rounded-card border border-border bg-card shadow-card p-4">
          ...
          {asset.description && (
            <p className="text-sm text-muted-2 mt-1">{asset.description}</p>
          )}

          {asset.service_history.length > 0 && (
            <div className="mt-3 border-t border-border pt-3">
              <h4 className="text-xs font-semibold text-muted uppercase mb-2">Service History</h4>
                ...
                {asset.service_history.map((entry) => (
                  <div key={entry.reference_id} className="flex items-center justify-between text-sm">
                    <div>
                      ...
                      {entry.description && (
                        <span className="text-muted-2 ml-2 truncate">{entry.description}</span>
                      )}
                    </div>
                    <div className="flex items-center gap-2">
                      {entry.status && <Badge variant="neutral">{entry.status}</Badge>}
                      {entry.date && (
                        <span className="text-xs text-muted-2">{formatDate(entry.date, locale)}</span>
                      )}
                    </div>
                  </div>
                ))}
