...
export function DocumentsTab({ token }: DocumentsTabProps) {
  ...
  if (documents.length === 0) return <p className="py-8 text-center text-sm text-muted">No compliance documents found.</p>
  ...
  return (
    <div className="space-y-4">
      {documents.map((doc) => {
        ...
        return (
          <div key={doc.id} className="rounded-card border border-border bg-card shadow-card p-4">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                <Badge variant="info">{typeLabel}</Badge>
                {doc.linked_invoice_number && (
                  <span className="text-xs text-muted-2">Invoice: {doc.linked_invoice_number}</span>
                )}
              </div>
              <a
                ...
                className="inline-flex items-center gap-1 rounded-ctl bg-accent-soft px-3 py-1.5 text-xs font-medium text-accent hover:bg-accent-soft/70 transition-colors"
              >
                <DownloadIcon />
                Download
              </a>
            ...
            {doc.description && (
              <p className="text-sm text-text">{doc.description}</p>
            )}
          </div>
        )
      })}
