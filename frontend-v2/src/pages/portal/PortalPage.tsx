    ...
    >
      ...
      <div className="mb-6 flex items-start justify-between">
        ...
        <button
          ...
          disabled={signingOut}
          className="inline-flex items-center gap-1.5 rounded-ctl border border-border bg-card px-3 py-2 text-sm font-medium text-text shadow-card hover:bg-canvas focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:opacity-50 min-h-[44px]"
...
function SummaryCard({ label, value, variant }: SummaryCardProps) {
  return (
    <div
      className={`rounded-card border bg-card shadow-card p-4 ${variantBorder[variant]}`}
      style={{ borderLeftWidth: '4px', borderLeftColor: 'var(--portal-accent, #2563eb)' }}
    >
      <p className="text-sm text-muted">{label}</p>
      <p className={`mt-1 mono text-xl font-semibold tabular-nums ${variantText[variant]}`}>
        {value}
      </p>
    </div>
  )
