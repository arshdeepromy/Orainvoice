...
export interface WizardData {
  ...
  website: string
  // Composite single-line address (read-only convenience used by the
  // branding preview + ready-step summary). Never set by the wizard, so it is
  // always undefined at runtime — preserving the original behaviour where the
  // bare `address` property was read but never populated.
  address?: string
