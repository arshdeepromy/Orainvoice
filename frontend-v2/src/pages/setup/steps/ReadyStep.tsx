    ...
    <div className="rounded-card border border-border p-4">
      <div className="flex items-center justify-between mb-2">
        ...
        <button
          ...
          className="text-xs text-accent hover:text-accent-press underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent rounded"
        >
          Edit
        </button>
      </div>
      ...
      <div className="space-y-3">
        ...
        <SummarySection title="Country & Region" stepIndex={0} onEdit={onGoToStep}>
          {country ? (
            ...
            <p className="text-muted-2 italic">Not configured</p>
          )}
        </SummarySection>
        ...
        <SummarySection title="Business Details" stepIndex={2} onEdit={onGoToStep}>
          {businessName ? (
            <div>
              <p className="font-medium text-text">{businessName}</p>
          ...
          <div className="flex items-center gap-3">
            {logoUrl ? (
              <img
                src={logoUrl}
                alt="Logo"
                className="h-8 w-8 object-contain rounded-ctl border border-border"
              />
            ) : (
              <span className="text-muted-2 italic">No logo</span>
            )}
            <div className="flex gap-2">
              <div
                className="h-6 w-6 rounded-ctl border border-border"
        ...
        <SummarySection title="Modules" stepIndex={4} onEdit={onGoToStep}>
          {enabledModules.length > 0 ? (
            ...
            <p className="text-muted-2 italic">Default modules only</p>
          )}
        </SummarySection>
        ...
        <SummarySection title="Catalogue" stepIndex={5} onEdit={onGoToStep}>
          {data.catalogueItems.length > 0 ? (
            ...
            <p className="text-muted-2 italic">No items added</p>
          )}
        </SummarySection>
      </div>
    </div>
