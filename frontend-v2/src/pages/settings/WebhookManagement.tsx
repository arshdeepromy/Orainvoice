    ...
    <Modal open={true} onClose={onClose} title={isEdit ? 'Edit webhook' : 'Create webhook'}>
      <div className="space-y-4">
        <div>
          ...
          />
          {urlError && <p className="text-danger text-sm" role="alert">{urlError}</p>}
        </div>
        <div>
          <EventTypeSelector selected={eventTypes} onChange={setEventTypes} />
          {eventError && <p className="text-danger text-sm" role="alert">{eventError}</p>}
        </div>
        ...
        <div className="flex gap-2 justify-end">
          <Button variant="ghost" onClick={onClose} style={{ minHeight: 44, minWidth: 44 }}>Cancel</Button>
      ...
      <div className="space-y-3" data-testid="test-result-modal">
        ...
        {result.body && (
          <div>
            ...
            <pre className="mt-1 p-2 bg-canvas rounded-ctl text-sm overflow-auto max-h-48">
              {result.body}
            </pre>
          </div>
        )}
        {result.error && (
          <div>
            ...
            <p className="text-danger text-sm mt-1">{result.error}</p>
          </div>
        )}
      ...
      render: (r) => {
        ...
        return (
          <div className="flex gap-2">
            ...
            <Button size="sm" variant="ghost" onClick={() => onEdit(w)} style={{ minHeight: 44, minWidth: 44 }}>
              Edit
            </Button>
