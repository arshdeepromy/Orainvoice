...
import { Button, Input, Select, ToastContainer, useToast } from '@/components/ui'
...
export default function BranchSettings() {
  ...
  if (loading) {
    return <p className="text-muted-2 px-4 py-6">Loading branch settings…</p>
  }
  ...
  return (
    <div className="px-4 py-6 sm:px-6 lg:px-8 max-w-3xl">
      ...
      <div className="space-y-6">
        {/* Contact Details */}
        <section className="rounded-card border border-border p-5 space-y-4">
          <h2 className="text-lg font-medium text-text">Contact Details</h2>
        ...
        {/* Logo */}
        <section className="rounded-card border border-border p-5 space-y-4">
          <h2 className="text-lg font-medium text-text">Branch Logo</h2>
          <p className="text-sm text-muted-2">
            This logo will appear on invoices generated for this branch instead of the organisation logo.
          </p>
          {logoUrl && (
            <div className="flex items-center gap-4">
              <img src={logoUrl} alt="Branch logo" className="h-16 w-auto rounded border border-border" />
              <button
                onClick={() => setLogoUrl('')}
          ...
          <div>
            <label className="inline-flex items-center gap-2 rounded-ctl border border-border bg-card px-4 py-2 text-sm text-muted hover:bg-canvas cursor-pointer">
        ...
        {/* Timezone */}
        <section className="rounded-card border border-border p-5 space-y-4">
          <h2 className="text-lg font-medium text-text">Timezone</h2>
          <Select
        ...
        {/* Operating Hours */}
        <section className="rounded-card border border-border p-5 space-y-4">
          <h2 className="text-lg font-medium text-text">Operating Hours</h2>
          <p className="text-sm text-muted-2">
            ...
            {DAYS_OF_WEEK.map((day) => (
              <div key={day} className="flex items-center gap-4">
                <label className="flex items-center gap-2 w-32 cursor-pointer">
                  ...
                  />
                  <span className="text-sm text-muted">{DAY_LABELS[day]}</span>
                </label>
                {enabledDays[day] && (
        ...
        <div className="flex justify-end gap-2 pt-2">
          <Button variant="ghost" onClick={() => navigate('/settings/branches')}>Cancel</Button>
