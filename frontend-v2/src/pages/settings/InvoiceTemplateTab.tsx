...
function TemplateCard({ template, isSelected, isCurrent, isDefault, onSelect, onPreview, previewLoadingId }: TemplateCardProps) {
  ...
  return (
    <div
      className={`group relative flex flex-col rounded-card border-2 bg-card p-3 text-left transition-colors ${
        isSelected
    ...
    >
      ...
      <button
        ...
        className="relative mb-3 aspect-[1/1.4] w-full overflow-hidden rounded-ctl bg-canvas focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent"
        aria-label={`Select ${template.display_name} template`}
      >
        {imgError ? (
          <div className="flex h-full w-full items-center justify-center p-4 text-center">
            <span className="text-sm font-medium text-muted-2">{template.display_name}</span>
          </div>
        ) : (
          <img
        ...
        <div className="absolute inset-0 flex items-center justify-center bg-black/40 opacity-0 transition-opacity group-hover:opacity-100">
          <span
            ...
            className="rounded-ctl bg-card px-4 py-2 text-sm font-semibold text-text shadow-pop transition-transform hover:scale-105 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent"
          >
      ...
      >
        <h3 className="text-sm font-semibold text-text">{template.display_name}</h3>
        <p className="mt-1 text-xs text-muted-2 line-clamp-2">{template.description}</p>
      </button>

      {/* Labels */}
      <div className="mt-2 flex flex-wrap gap-1">
        <span className="rounded bg-canvas px-1.5 py-0.5 text-[11px] font-medium text-muted capitalize">
...
export function InvoiceTemplateTab() {
  ...
  if (loading) {
    return <p className="text-sm text-muted-2">Loading templates…</p>
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <h2 className="text-lg font-medium text-text">Invoice Template</h2>
      ...
      <div className="flex flex-wrap items-center gap-4">
        <div className="flex items-center gap-2">
          <span className="text-xs font-medium text-muted-2">Layout:</span>
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs font-medium text-muted-2">Logo:</span>
          {LOGO_FILTERS.map((f) => (
            <FilterPill
              key={f.value}
              label={f.label}
      ...
      {filteredTemplates.length === 0 && (
        <p className="text-sm text-muted-2">No templates match the selected filters.</p>
      )}
      ...
      {selectedId && selectedTemplate && (
        <div className="rounded-card border border-border bg-card p-5">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-sm font-semibold text-text">Colour Customisation</h3>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="rounded-ctl bg-accent px-3 py-1.5 text-xs font-medium text-white shadow-sm transition-colors hover:bg-accent-press disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent"
              >
              ...
              <button
                ...
                className="rounded-ctl border border-border bg-card px-3 py-1.5 text-xs font-medium text-muted shadow-sm transition-colors hover:bg-canvas focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent"
              >
                Reset to Defaults
              </button>
            </div>
          ...
          <div className="mt-4 flex flex-wrap gap-6">
            <label className="flex items-center gap-2">
              ...
              />
              <span className="text-sm text-muted">Primary Colour</span>
            </label>
            ...
            <label className="flex items-center gap-2">
              <input
                ...
                className="h-8 w-8 cursor-pointer rounded border border-border"
              />
              <span className="text-sm text-muted">Header Background</span>
            </label>
          </div>
        </div>
      )}
      ...
      {selectedId && (
        <div>
          <button
            ...
            onClick={handleSave}
            disabled={saving}
            className="rounded-ctl bg-accent px-4 py-2 text-sm font-medium text-white shadow-sm transition-colors hover:bg-accent-press disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent"
          >
            {saving ? 'Saving…' : 'Save'}
          </button>
        </div>
      ...
      {previewHtml !== null && (
        ...
        >
          <div className="flex min-h-full items-start justify-center p-4 sm:p-8">
            <div
              className="relative w-full max-w-4xl rounded-card bg-card shadow-pop"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="sticky top-0 z-10 flex items-center justify-between rounded-t-card border-b border-border bg-card px-5 py-3">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className={`rounded-ctl px-3 py-1.5 text-xs font-medium shadow-sm transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent ${
                      savedTemplateId === previewTemplateId
                        ? 'bg-ok-soft text-ok cursor-default'
                        : 'bg-accent text-white hover:bg-accent-press disabled:opacity-50'
                    }`}
                  >
                    {savingFromPreview
                      ? 'Saving…'
                  ...
                  <button
                    type="button"
                    onClick={() => { setPreviewHtml(null); setPreviewTemplateId(null) }}
                    className="rounded-ctl p-1 text-muted-2 transition-colors hover:bg-canvas hover:text-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent"
