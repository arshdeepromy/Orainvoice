...
export function MfaSettings() {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-12">
        <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-accent" role="status" aria-label="Loading MFA settings" />
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-sm font-medium text-text">Multi-Factor Authentication</h3>
      ...
      {successMsg && (
        <div className="rounded-ctl bg-ok-soft border border-ok p-3" role="status">
          <p className="text-sm text-ok">{successMsg}</p>
        </div>
      )}
