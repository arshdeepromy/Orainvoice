...
function EventTypeSelector({
  ...
  return (
    <fieldset>
      <legend className="text-sm font-medium text-text mb-2">Event types</legend>
      <div className="space-y-2">
        {ALL_EVENT_TYPES.map((evt) => (
          <label key={evt.value} className="flex items-center gap-2 text-sm text-text cursor-pointer">
            <input
              type="checkbox"
  ...
  const columns: Column<Webhook>[] = [
    {
      ...
      header: 'URL',
      render: (row) => (
        <span className="mono text-xs break-all">{row.url}</span>
      ),
    },
    {
      key: 'event_types',
...
function DeliveryLog({ deliveries }: { deliveries: WebhookDelivery[] }) {
  ...
  return (
    <div>
      {deliveries.length === 0 ? (
        <p className="text-sm text-muted py-4">No delivery history yet.</p>
      ) : (
...
export function WebhookSettings() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-semibold text-text mb-6">Webhook settings</h1>
