...
import { Button, Input, Modal, DataTable, type Column, Badge, ToastContainer, useToast } from '@/components/ui'
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex gap-2">
          ...
          <Button size="sm" variant="ghost" onClick={() => openAssign(row.id)}>Assign Users</Button>
        </div>
      ),
    },
  ]
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-semibold text-text">Branch Management</h1>
        <Button onClick={openAdd}>Add Branch</Button>
      </div>
      <ToastContainer toasts={toasts} onDismiss={dismissToast} />

      {loading ? (
        <p className="text-muted-2">Loading branches&hellip;</p>
      ) : (
      ...
      >
        ...
        {(modalStep === 'details' || !isAddMode) && (
          <div className="space-y-4">
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              {isAddMode && (
                <>
                  <Button variant="ghost" onClick={saveBranch} disabled={!form.name.trim() || saving} loading={saving}>
                    Create
                  </Button>
        ...
        {modalStep === 'staff' && isAddMode && (
          <div className="space-y-4">
            {staffLoading && (
              <p className="text-muted-2 text-sm">Loading staff&hellip;</p>
            )}

            {staffError && !staffLoading && (
              <div className="rounded-ctl bg-danger-soft border border-danger p-4">
                <p className="text-sm text-danger mb-3">{staffError}</p>
                <div className="flex gap-2">
                  <Button size="sm" variant="ghost" onClick={retryStaff}>Retry</Button>
                  <Button size="sm" variant="ghost" onClick={saveBranch} disabled={saving} loading={saving}>Skip</Button>
                </div>
              </div>
            )}

            {!staffLoading && !staffError && staff.length === 0 && (
              <p className="text-muted-2 text-sm">No staff members found</p>
            )}

            {!staffLoading && !staffError && staff.length > 0 && (
              <ul className="divide-y divide-border" role="list" aria-label="Staff members">
                {staff.map((member) => {
                    ...
                    <li key={member.id} className="flex items-center justify-between py-3 gap-3">
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          <span className="font-medium text-text text-sm truncate">{member.name}</span>
                      ...
                      <div className="flex-shrink-0">
                        {invitable ? (
                          ...
                          <div className="relative group">
                            <label className="flex items-center gap-2 text-sm text-muted-2 cursor-not-allowed">
                              <input type="checkbox" disabled className="rounded border-border opacity-50" />
                              <span>{label}</span>
                            </label>
                            <div className="absolute bottom-full right-0 mb-1 hidden group-hover:block bg-ink text-white text-xs rounded px-2 py-1 whitespace-nowrap z-10">
            ...
            <div className="flex justify-between pt-2 border-t border-border">
              <Button variant="ghost" onClick={() => setModalStep('details')}>Back</Button>
              <div className="flex gap-2">
                <Button variant="ghost" onClick={saveBranch} disabled={saving} loading={saving}>Skip</Button>
      ...
      <Modal open={assignModalOpen} onClose={() => setAssignModalOpen(false)} title={`Assign Users \u2014 ${assignedBranch?.name ?? ''}`}>
        {assignedBranch && (
          <div className="space-y-2">
            {users.length === 0 && <p className="text-muted-2 text-sm">No users found</p>}
            {users.map((user) => {
            ...
            <div className="flex justify-end pt-2">
              <Button variant="ghost" onClick={() => setAssignModalOpen(false)}>Close</Button>
            </div>
          </div>
        )}
      </Modal>
