  ...
  if (loadingMethods) {
    return (
      <div className="flex items-center gap-2 mt-3">
        ...
        <span className="text-sm text-muted-2">Loading payment methods…</span>
      </div>
    )
  }
      ...
      {!editing ? (
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button
            ...
            className="inline-flex items-center text-muted-2 hover:text-accent transition-colors p-0.5 rounded focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent"
        ...
        <div>
          <div className="mb-2">
            <span className="text-sm font-medium text-text">Payment Methods:</span>
            <p className="text-xs text-muted-2 mt-0.5">Choose which payment methods to accept:</p>
          </div>
            ...
            {(methods ?? []).map((method) => {
              ...
              return (
                <label
                  key={method?.type ?? ''}
                  className={`
                    flex items-center gap-2 rounded-ctl border px-3 py-2 text-sm cursor-pointer transition-all
                    ${isChecked ? 'border-accent bg-accent-soft' : 'border-border bg-card hover:border-border-strong'}
                ...
                >
                  ...
                  <span className="text-text truncate">{method?.name ?? ''}</span>
                </label>
              )
            })}
          </div>
...
function SurchargeSettingsSection() {
    ...
    return (
      <div className="mt-4 pt-3 border-t border-border">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-muted-2">Loading surcharge settings…</span>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="mt-4 pt-3 border-t border-border">
      <h4 className="text-sm font-semibold text-text mb-2">Surcharge Settings</h4>
      ...
      <div className="rounded-ctl border border-warn bg-warn-soft p-3 mb-3">
      ...
      <label className="flex items-center gap-2 mb-3 cursor-pointer">
        ...
        />
        <span className="text-sm font-medium text-text">Pass processing fees to customers</span>
      </label>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border">
                  <th className="text-left py-2 pr-3 font-medium text-muted">Payment Method</th>
                  <th className="text-left py-2 px-3 font-medium text-muted">Percentage (%)</th>
                  <th className="text-left py-2 px-3 font-medium text-muted">Fixed ($)</th>
                  <th className="text-center py-2 pl-3 font-medium text-muted">Enabled</th>
                </tr>
              </thead>
              <tbody>
                      ...
                      <td className="py-2 px-3">
                        <div>
                          <input
                            ...
                            className={`w-20 rounded-ctl border bg-card px-2 py-1 text-sm tabular-nums text-text ${
                              pctError ? 'border-danger focus:ring-danger' : 'border-border focus:ring-accent'
                            } focus:outline-none focus:ring-1`}
                            aria-label={`${METHOD_DISPLAY_NAMES[method] ?? method} percentage`}
                          />
                          {pctError && (
                            <p className="text-xs text-danger mt-0.5">{pctError}</p>
                          )}
                          {rateWarning && !pctError && (
                            <p className="text-xs text-warn mt-0.5">{rateWarning}</p>
                          )}
                        </div>
                      </td>
                      <td className="py-2 px-3">
                        <div>
                          <input
                            ...
                            className={`w-20 rounded-ctl border bg-card px-2 py-1 text-sm tabular-nums text-text ${
                              fixedError ? 'border-danger focus:ring-danger' : 'border-border focus:ring-accent'
                            } focus:outline-none focus:ring-1`}
                            aria-label={`${METHOD_DISPLAY_NAMES[method] ?? method} fixed fee`}
                          />
                          {fixedError && (
                            <p className="text-xs text-danger mt-0.5">{fixedError}</p>
                          )}
                        </div>
                      </td>
...
function PayoutSettingsSection() {
    ...
    return (
      <div className="mt-4 pt-3 border-t border-border">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-muted-2">Loading payout settings…</span>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="mt-4 pt-3 border-t border-border">
      <h4 className="text-sm font-semibold text-text mb-2">Payout Settings</h4>
      ...
      {hasBankAccount ? (
        <div className="space-y-1 mb-3">
          ...
          {(payoutInfo?.payout_schedule ?? '') !== '' && (
            <p className="text-sm text-muted">
              Payout Schedule:{' '}
              <span className="font-medium text-text">{payoutInfo?.payout_schedule ?? ''}</span>
            </p>
          )}
          <p className="text-sm text-muted">
            Status:{' '}
            {payoutInfo?.payouts_enabled ? (
              <span className="font-medium text-ok">✓ Payouts enabled</span>
            ) : (
              <span className="font-medium text-warn">Payouts not enabled</span>
            )}
          </p>
        </div>
      ) : (
        <div className="mb-3">
          <p className="text-sm text-muted">No bank account configured for payouts.</p>
          <p className="text-sm text-muted-2">Set up your bank account to receive payments.</p>
        </div>
      )}
...
export default function OnlinePaymentsSettings() {
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div>
        <h2 className="text-lg font-semibold text-text">Online Payments</h2>
      ...
      {!status?.connect_client_id_configured && (
        <div className="rounded-card border border-border p-5">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-lg font-semibold text-text">Stripe</h3>
      ...
      {status?.connect_client_id_configured && status?.is_connected && (
        <div className="rounded-card border border-border p-5">
          ...
          <div className="space-y-1 mb-3">
            <p className="text-sm text-muted">
              Integration Status: <span className="font-medium text-ok">Active</span>
            </p>
            {(status?.account_name ?? '') !== '' && (
              <p className="text-sm text-muted">
                Account Name: <span className="font-semibold text-text">{status?.account_name ?? ''}</span>
              </p>
            )}
            <p className="text-sm text-muted-2">
              Account: <span className="font-medium text-muted">····{status?.account_id_last4 ?? ''}</span>
            </p>
            {(status?.application_fee_percent ?? null) !== null && (
              <p className="text-sm text-muted-2">
                Platform fee: <span className="font-medium text-muted">{status?.application_fee_percent ?? 0}%</span> per transaction
              </p>
            )}
          </div>
      ...
      >
        ...
        <p className="text-sm text-warn bg-warn-soft border border-warn rounded-ctl p-3 mb-4">
