...
function ConnectionCard({
  ...
  const statusBadge = (() => {
    ...
    switch (connection.sync_status) {
      ...
      case 'failed':
        return { variant: 'danger' as const, text: 'Sync failed' }
      default:
        return { variant: 'success' as const, text: 'Connected' }
    }
  })()
        ...
        <>
          {connection.account_name && (
            <p className="text-sm text-muted mb-1">
              Account: <span className="font-medium text-text">{connection.account_name}</span>
            </p>
          )}
          <p className="text-sm text-muted mb-1">
...
function SyncLog({
  ...
  const columns: Column<SyncLogEntry>[] = [
    {
      ...
      header: 'Time',
      sortable: true,
      render: (row) => <span className="mono">{formatDate(row.synced_at)}</span>,
    },
    {
      key: 'provider',
      header: 'Provider',
  ...
  return (
    <div>
      <h3 className="text-lg font-semibold text-text mb-3">Recent sync activity</h3>
      {entries.length === 0 ? (
        <p className="text-sm text-muted-2">No sync activity yet. Connect an accounting provider to get started.</p>
      ) : (
...
export function AccountingIntegrations() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-semibold text-text mb-6">Accounting integrations</h1>
