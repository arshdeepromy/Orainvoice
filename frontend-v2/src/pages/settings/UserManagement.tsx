...
export function UserManagement() {
  ...
  const columns: Column<OrgUser>[] = [
    { key: 'email', header: 'Email', sortable: true },
    {
      ...
      render: (row) => (
        <Badge variant={row.role === 'kiosk' ? 'warn' : 'info'}>
          {roleLabel(row.role)}
        </Badge>
      ),
    },
    ...
    { key: 'last_login_at', header: 'Last Activity', render: (row) => {
      ...
      if (row.role === 'kiosk' && row.last_login_at) {
        return <span className="mono text-sm font-medium">{dateStr}</span>
      }
      return <span className="mono">{dateStr}</span>
    }},
    {
      ...
      render: (row) => {
        if (!row.is_active) return (
          <div className="flex gap-2">
            <span className="text-sm text-muted-2">Deactivated</span>
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap items-center gap-6 mb-6 p-4 bg-canvas rounded-card">
        ...
        <div className="flex items-center gap-3">
          <span className="text-sm font-medium text-muted">MFA Policy:</span>
          <button role="switch" aria-checked={mfaPolicy === 'mandatory'} aria-label="MFA policy toggle"
            onClick={toggleMfaPolicy}
            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${mfaPolicy === 'mandatory' ? 'bg-accent' : 'bg-border-strong'}`}>
            <span className={`inline-block h-4 w-4 transform rounded-full bg-card transition-transform ${mfaPolicy === 'mandatory' ? 'translate-x-6' : 'translate-x-1'}`} />
          </button>
          <span className="text-sm text-muted">{mfaPolicy === 'mandatory' ? 'Mandatory' : 'Optional'}</span>
        </div>
      </div>

      {loading ? (
        <p className="text-muted-2">Loading users…</p>
      ) : (
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            {resetTouched.newPassword && resetNewPassword.length > 0 && resetNewPassword.length < 8 && (
              <p className="text-sm text-danger mt-1">Password must be at least 8 characters</p>
            )}
          </div>
          <div>
            <Input
          ...
          <div className="flex justify-end gap-2 pt-2">
            <Button variant="ghost" onClick={closeResetPasswordModal}>Cancel</Button>
