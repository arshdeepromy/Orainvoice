  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold text-text">Enable Currency</h3>
        <Button
          variant="ghost"
          size="sm"
          onClick={onClose}
      ...
      <div>
        <label htmlFor="currency-search" className="text-sm font-medium text-muted block mb-1">
          Search currencies
        </label>
        <input
          id="currency-search"
          ...
          style={{ minHeight: 44 }}
          className="w-full rounded-ctl border border-border bg-card px-3 py-2 text-text shadow-sm
      ...
      <div className="max-h-64 overflow-y-auto divide-y divide-border">
        {filtered.length === 0 && (
          <p className="text-center text-muted-2 py-4" data-testid="currency-search-empty">
          ...
          return (
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span className="text-muted-2 text-sm">{c.name}</span>
                <span className="text-muted-2 text-sm">{c.symbol}</span>
  ...
  return (
    ...
    >
        ...
        <div className="flex gap-1" role="group" aria-label="Chart date range">
          {rangeOptions.map((opt) => (
            <button
              ...
              className={`px-3 py-1 text-sm rounded-ctl transition-colors
      ...
      {loading ? (
        ...
        <p className="text-center text-muted-2 py-4" data-testid="chart-empty">
          ...
          <div className="flex items-end h-full gap-px">
            {history.map((h, i) => {
              ...
              return (
                <div
                  key={i}
                  className="flex-1 bg-accent rounded-t-sm hover:bg-accent-press transition-colors relative group"
                  style={{ height: `${Math.max(height, 2)}%` }}
                  title={`${h.date}: ${h.rate}`}
                  data-testid={`chart-bar-${i}`}
                >
                  <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 hidden group-hover:block
                    bg-ink text-white text-xs rounded px-2 py-1 whitespace-nowrap z-10">
...
function RateProviderSection({
  ...
  const statusVariant = config?.status === 'active' ? 'success' : config?.status === 'error' ? 'danger' : 'neutral'
  ...
  return (
    ...
    >
      <h3 className="text-lg font-semibold text-text">Rate Provider</h3>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <span className="text-sm text-muted-2">Provider</span>
          <p className="font-medium text-text" data-testid="provider-name">
            {config?.provider ?? 'Not configured'}
          </p>
        </div>
        <div>
          <span className="text-sm text-muted-2">Update Frequency</span>
          <p className="font-medium text-text" data-testid="provider-frequency">
            {config?.update_frequency ?? '—'}
          </p>
        </div>
        <div>
          <span className="text-sm text-muted-2">Last Sync</span>
          <p className="font-medium text-text" data-testid="provider-last-sync">
            {config?.last_sync ?? 'Never'}
          </p>
        </div>
        <div>
          <span className="text-sm text-muted-2">Status</span>
...
function ManualRateForm({
  ...
  return (
    ...
    >
      <h3 className="text-lg font-semibold text-text">Set Manual Rate</h3>
      {error && (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label htmlFor="rate-base" className="text-sm font-medium text-muted block mb-1">
            Base Currency
          </label>
          <input
            id="rate-base"
            ...
            disabled
            style={{ minHeight: 44 }}
            className="w-full rounded-ctl border border-border px-3 py-2 text-muted-2 bg-canvas"
            data-testid="rate-base-input"
          />
        </div>
        <div>
          <label htmlFor="rate-target" className="text-sm font-medium text-muted block mb-1">
            Target Currency
          </label>
          <select
            id="rate-target"
            ...
            required
            style={{ minHeight: 44 }}
            className="w-full rounded-ctl border border-border bg-card px-3 py-2 text-text
              focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent"
        ...
        <div>
          <label htmlFor="rate-value" className="text-sm font-medium text-muted block mb-1">
            Exchange Rate
          </label>
          <input
            id="rate-value"
        ...
        <div>
          <label htmlFor="rate-date" className="text-sm font-medium text-muted block mb-1">
            Effective Date
          </label>
          <input
            id="rate-date"
...
function ExchangeRateRow({
  ...
  return (
    <tr data-testid={`rate-row-${rate.base_currency}-${rate.target_currency}`} className="border-b border-border last:border-b-0 hover:bg-canvas">
      <td className="px-4 py-3 text-sm font-medium text-text mono">{rate.base_currency}</td>
      ...
      <td className="px-4 py-3 text-sm text-text mono">{rate.rate}</td>
      <td className="px-4 py-3 text-sm">
        <Badge variant={rate.source === 'manual' ? 'warn' : 'info'}>
          {rate.source}
        </Badge>
      </td>
      <td className="px-4 py-3 text-sm text-muted-2 mono">{rate.effective_date}</td>
      <td className="px-4 py-3 text-sm">
        <button
          type="button"
          onClick={() => onShowChart(rate.base_currency, rate.target_currency)}
          style={{ minHeight: 44, minWidth: 44 }}
          className="text-accent hover:text-accent-press underline text-sm
...
export default function CurrencySettings() {
  ...
  const currenciesTab = (
    <div className="space-y-4" data-testid="currencies-tab">
      ...
      <div className="bg-accent-soft border border-accent rounded-card p-4" data-testid="base-currency-display">
        <span className="text-sm text-accent">Base {currencyLabel}</span>
        <p className="text-xl font-bold text-accent" data-testid="base-currency-code">
          {baseCurrency}
        </p>
      ...
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold text-text">Enabled Currencies</h3>
        <Button
          variant="primary"
          size="sm"
      ...
      <section className="overflow-hidden rounded-card border border-border bg-card shadow-card">
        <div className="overflow-x-auto">
          <table className="min-w-full" data-testid="enabled-currencies-table">
            <thead>
              <tr>
                <th className="mono border-b border-border px-4 py-3 text-left text-[10.5px] font-medium uppercase tracking-[0.08em] text-muted-2">{currencyLabel}</th>
                <th className="mono border-b border-border px-4 py-3 text-left text-[10.5px] font-medium uppercase tracking-[0.08em] text-muted-2">Symbol</th>
                <th className="mono border-b border-border px-4 py-3 text-left text-[10.5px] font-medium uppercase tracking-[0.08em] text-muted-2">Decimals</th>
                ...
                <th className="mono border-b border-border px-4 py-3 text-left text-[10.5px] font-medium uppercase tracking-[0.08em] text-muted-2">Sample</th>
              </tr>
            </thead>
            <tbody>
              {currencies.filter((c) => c.enabled).map((c) => {
                ...
                return (
                  <tr key={c.id} data-testid={`currency-row-${c.currency_code}`} className="border-b border-border last:border-b-0 hover:bg-canvas">
                    <td className="px-4 py-3 text-sm font-medium text-text mono">{c.currency_code}</td>
                    <td className="px-4 py-3 text-sm text-muted">{fmt.symbol}</td>
                    <td className="px-4 py-3 text-sm text-muted mono">{fmt.decimalPlaces}</td>
                    <td className="px-4 py-3 text-sm">
                      {c.is_base ? (
                    ...
                    <td className="px-4 py-3 text-sm">
                      {missing ? (
                        <span data-testid={`missing-rate-${c.currency_code}`}>
                          <Badge variant="warn">⚠ No Rate</Badge>
                        </span>
                      ) : (
                        <Badge variant="success">Active</Badge>
                      )}
                    </td>
                    <td className="px-4 py-3 text-sm mono text-muted" data-testid={`sample-format-${c.currency_code}`}>
  ...
  const ratesTab = (
    <div className="space-y-4" data-testid="rates-tab">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <h3 className="text-lg font-semibold text-text">Exchange Rates</h3>
        <Button
          variant="primary"
          size="sm"
      ...
      <section className="overflow-hidden rounded-card border border-border bg-card shadow-card">
        <div className="overflow-x-auto">
          <table className="min-w-full" data-testid="exchange-rates-table">
            <thead>
              <tr>
                <th className="mono border-b border-border px-4 py-3 text-left text-[10.5px] font-medium uppercase tracking-[0.08em] text-muted-2">Base</th>
                <th className="mono border-b border-border px-4 py-3 text-left text-[10.5px] font-medium uppercase tracking-[0.08em] text-muted-2">Target</th>
                <th className="mono border-b border-border px-4 py-3 text-left text-[10.5px] font-medium uppercase tracking-[0.08em] text-muted-2">Rate</th>
                ...
                <th className="mono border-b border-border px-4 py-3 text-left text-[10.5px] font-medium uppercase tracking-[0.08em] text-muted-2">Chart</th>
              </tr>
            </thead>
            <tbody>
              {rates.map((r) => (
  ...
  return (
    <div data-testid="currency-settings" className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold text-text">{currencyLabel} Settings</h1>
        <Button
          variant="ghost"
          onClick={() => {
            setLoading(true)
