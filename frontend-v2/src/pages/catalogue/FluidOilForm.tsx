...
function SegmentedToggle<T extends string>({ options, value, onChange, label, required }: {
  ...
  return (
    <div>
      <label className="block text-[12.5px] font-medium text-text mb-1">{label}{required && ' *'}</label>
        ...
        {options.map((opt, i) => (
          <button key={opt.value} type="button" onClick={() => onChange(opt.value)}
            className={`flex-1 py-2.5 text-sm font-medium transition-colors ${i > 0 ? 'border-l border-border' : ''} ${
              value === opt.value ? 'bg-accent text-white' : 'bg-card text-text hover:bg-canvas'
            }`}>
            {opt.label}
          </button>
        ))}
...
export default function FluidOilForm({ onClose, onSubmit }: { onClose?: () => void; onSubmit?: (data: FluidFormData) => void }) {
  ...
  return (
    <div className="space-y-5">
      <h2 className="text-xl font-semibold text-text">Add Fluid / Oil Product</h2>
      ...
      />
      {errors.fluid_type && <p className="text-xs text-danger">{errors.fluid_type}</p>}
      ...
      <Section show={isNonOil}>
        <div className="space-y-4 rounded-card border border-border bg-card p-4">
          ...
          <div className="rounded-card border border-border bg-canvas p-3 space-y-3">
            <p className="text-sm font-medium text-text">How is this product purchased?</p>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <Field label="Qty per pack" required error={errors.qty_per_pack}>
                <input type="number" min="1" step="0.1" value={f.qty_per_pack} onChange={e => set('qty_per_pack', e.target.value)}
                  className="w-full rounded-ctl border border-border bg-card px-3 py-2 text-sm text-text focus:border-accent focus:outline-none focus:shadow-[0_0_0_3px_var(--accent-soft)]" placeholder="e.g. 5" />
              </Field>
              <Field label="Unit">
                <div className="inline-flex rounded-ctl border border-border overflow-hidden w-full">
                  {(['litre', 'gallon'] as UnitType[]).map((u, i) => (
                    <button key={u} type="button" onClick={() => set('unit_type', u)}
                      className={`flex-1 py-2 text-sm font-medium transition-colors ${i > 0 ? 'border-l border-border' : ''} ${
                        f.unit_type === u ? 'bg-accent text-white' : 'bg-card text-text hover:bg-canvas'
                      }`}>
              ...
              <Field label="Total packs" required error={errors.total_quantity}>
                <input type="number" min="1" step="1" value={f.total_quantity} onChange={e => set('total_quantity', e.target.value)}
                  className="w-full rounded-ctl border border-border bg-card px-3 py-2 text-sm text-text focus:border-accent focus:outline-none focus:shadow-[0_0_0_3px_var(--accent-soft)]" placeholder="e.g. 6" />
              </Field>
            </div>
            {totalVolume > 0 && (
              <div className="flex items-center gap-2 rounded-ctl bg-card border border-border px-3 py-2">
                <span className="text-sm text-muted">Total Volume:</span>
                <span className="text-sm font-semibold text-text">{totalVolume.toLocaleString()} {unitLabel}s</span>
              </div>
            )}
          </div>
          ...
          <div className="rounded-card border border-border bg-canvas p-3 space-y-3">
            ...
            {errors.gst_mode && <p className="text-xs text-danger">{errors.gst_mode}</p>}
            {costPerUnit > 0 && (
              <div className="rounded-ctl bg-card border border-border px-3 py-2 flex items-center justify-between">
                <span className="text-sm text-muted">Cost per {unitLabel}:</span>
                <span className="text-sm font-semibold text-text">{formatNZD(costPerUnit)}</span>
              </div>
            )}
            ...
            {sellPerUnit > 0 && costPerUnit > 0 && (
              <div className={`rounded-ctl border px-3 py-2 flex items-center justify-between ${margin >= 0 ? 'bg-ok-soft border-ok/30' : 'bg-danger-soft border-danger/30'}`}>
                <span className="text-sm text-muted">Margin:</span>
                <span className={`text-sm font-semibold ${margin >= 0 ? 'text-ok' : 'text-danger'}`}>
          ...
          <Field label="Description">
            <textarea value={f.description} onChange={e => set('description', e.target.value)} rows={2} placeholder="Optional"
              className="w-full rounded-ctl border border-border bg-card px-3 py-2 text-sm text-text focus:border-accent focus:outline-none focus:shadow-[0_0_0_3px_var(--accent-soft)] resize-none" />
          </Field>
        </div>
      </Section>
      ...
      <Section show={showGrade}>
        <div className="rounded-card border border-accent/20 bg-accent-soft p-4 space-y-3">
          <p className="text-xs font-medium text-accent uppercase tracking-wider">Engine Oil Grade</p>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <Field label="Qty per pack" required error={errors.qty_per_pack}>
              <input type="number" min="1" step="1" value={f.qty_per_pack} onChange={e => set('qty_per_pack', e.target.value)}
                className="w-full rounded-ctl border border-border bg-card px-3 py-2 text-sm text-text focus:border-accent focus:outline-none focus:shadow-[0_0_0_3px_var(--accent-soft)]" placeholder="e.g. 205" />
            </Field>
            <Field label="Unit">
              <div className="inline-flex rounded-ctl border border-border overflow-hidden w-full">
                {(['litre', 'gallon'] as UnitType[]).map((u, i) => (
            ...
            <Field label="Total packs" required error={errors.total_quantity}>
              <input type="number" min="1" step="1" value={f.total_quantity} onChange={e => set('total_quantity', e.target.value)}
                className="w-full rounded-ctl border border-border bg-card px-3 py-2 text-sm text-text focus:border-accent focus:outline-none focus:shadow-[0_0_0_3px_var(--accent-soft)]" placeholder="e.g. 3" />
            </Field>
          </div>
          {totalVolume > 0 && (
            <div className="flex items-center gap-2 rounded-ctl bg-canvas border border-border px-3 py-2">
              <span className="text-sm text-muted">Total Volume:</span>
              <span className="text-sm font-semibold text-text">{totalVolume.toLocaleString()} {unitLabel}s</span>
              {f.container_type && f.total_quantity && (
                <span className="text-xs text-muted-2 ml-auto">
      ...
      <Section show={showPricing}>
        <div className="rounded-card border border-border bg-card p-4 space-y-4">
          ...
          {costPerUnit > 0 && (
            <div className="rounded-ctl bg-canvas border border-border px-3 py-2 flex items-center justify-between">
              <span className="text-sm text-muted">Cost per {unitLabel}:</span>
              <span className="text-sm font-semibold text-text">{formatNZD(costPerUnit)}</span>
            </div>
          )}
        ...
        <div className="space-y-3">
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              <label className="block text-[12.5px] font-medium text-text">Suppliers</label>
              <button type="button" onClick={() => { setNewSupplier({ name: '', contact_name: '', email: '', phone: '', address: '' }); setAddSupplierError(''); setShowAddSupplier(true) }}
                className="text-xs text-accent hover:underline font-medium">+ Add Supplier</button>
            </div>
            ...
            {selectedSupplierIds.length > 0 && (
              <div className="flex flex-wrap gap-1 mb-2">
                {selectedSupplierIds.map(sid => {
                  ...
                  return s ? (
                    <span key={sid} className="inline-flex items-center gap-1 rounded-full bg-accent-soft text-accent px-2.5 py-0.5 text-xs font-medium">
                      {s.name}
                      <button type="button" onClick={() => toggleSupplier(sid)} className="text-accent hover:text-text">✕</button>
                    </span>
                  ) : null
                })}
              </div>
            ...
            <div ref={supplierDropRef} className="relative">
              <input type="text" value={supplierSearch}
                ...
                className="w-full rounded-ctl border border-border bg-card px-3 py-2 text-sm text-text placeholder:text-muted-2 focus:border-accent focus:outline-none focus:shadow-[0_0_0_3px_var(--accent-soft)]" autoComplete="off" />
              {supplierDropOpen && (
                <div className="absolute top-full left-0 right-0 z-50 mt-1 max-h-48 overflow-auto rounded-ctl border border-border bg-card shadow-pop">
                  {suppliers.filter(s => !supplierSearch.trim() || s.name.toLowerCase().includes(supplierSearch.toLowerCase())).slice(0, 15).map(s => (
                    <button key={s.id} type="button" onClick={() => { toggleSupplier(s.id); setSupplierSearch(''); setSupplierDropOpen(false) }}
                      className="w-full px-3 py-2 text-left text-sm hover:bg-canvas flex items-center justify-between">
                      <span className="text-text">{s.name}</span>
                      {selectedSupplierIds.includes(s.id) && <span className="text-accent text-xs">✓ selected</span>}
                    </button>
                  ))}
                  {suppliers.filter(s => !supplierSearch.trim() || s.name.toLowerCase().includes(supplierSearch.toLowerCase())).length === 0 && (
                    <div className="px-3 py-2 text-sm text-muted">No suppliers match</div>
                  )}
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label={`Min stock threshold (${unitLabel}s)`}>
              <input type="number" min="0" step="0.1" value={f.min_stock_volume} onChange={e => set('min_stock_volume', e.target.value)}
                placeholder="0" className="w-full rounded-ctl border border-border bg-card px-3 py-2 text-sm text-text focus:border-accent focus:outline-none focus:shadow-[0_0_0_3px_var(--accent-soft)]" />
            </Field>
      ...
      <Section show={(isOil && !!f.oil_type && totalVolume > 0) || (isNonOil && !!f.product_name && totalVolume > 0)}>
        <div className="rounded-card border border-accent/20 bg-accent-soft p-4">
          <p className="text-xs font-semibold text-accent uppercase tracking-wider mb-2">Summary</p>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
            {isOil && (
              <>
                <div>
                  <p className="text-muted text-xs">Product</p>
                  <p className="font-medium text-text">{oilTypeLabel}{f.grade ? ` ${f.grade}` : ''}</p>
                  {f.synthetic_type && <p className="text-xs text-muted capitalize">{f.synthetic_type.replace('_', ' ')}</p>}
                </div>
                <div>
                  <p className="text-muted text-xs">Container</p>
                  <p className="font-medium text-text">
                    {f.total_quantity || '?'} × {f.qty_per_pack || '?'}{f.unit_type === 'litre' ? 'L' : 'gal'} {containerLabel}
                  </p>
                  <p className="text-xs text-muted">{totalVolume > 0 ? `${totalVolume.toLocaleString()} ${unitLabel}s total` : ''}</p>
                </div>
                <div>
                  <p className="text-muted text-xs">Pricing</p>
                  <p className="font-medium text-text">{f.purchase_price ? formatNZD(Number(f.purchase_price)) : '—'}</p>
                ...
                <div>
                  <p className="text-muted text-xs">Margin</p>
                  {margin !== 0 ? (
                    <p className={`font-medium ${margin >= 0 ? 'text-ok' : 'text-danger'}`}>
                      {formatNZD(margin)} ({marginPct.toFixed(1)}%)
                    </p>
            ...
            {isNonOil && (
              <>
                <div>
                  <p className="text-muted text-xs">Product</p>
                  <p className="font-medium text-text">{f.brand_name} {f.product_name}</p>
                </div>
                <div>
                  <p className="text-muted text-xs">Container</p>
                  <p className="font-medium text-text">
      ...
      {saveSuccess && <p className="text-sm text-ok">✓ Product saved successfully.</p>}
      ...
      {products.length > 0 && (
        <div className="mt-6">
          <h3 className="text-lg font-medium text-text mb-3">Saved Fluid / Oil Products</h3>
          <div className="overflow-x-auto rounded-card border border-border bg-card shadow-card">
            <table className="min-w-full text-sm border-collapse">
              <thead>
                <tr>
                  <th className="mono border-b border-border px-4 py-2 text-left text-[10.5px] font-medium uppercase tracking-[0.08em] text-muted-2">Product</th>
                  <th className="mono border-b border-border px-4 py-2 text-left text-[10.5px] font-medium uppercase tracking-[0.08em] text-muted-2">Type</th>
                  <th className="mono border-b border-border px-4 py-2 text-right text-[10.5px] font-medium uppercase tracking-[0.08em] text-muted-2">Volume</th>
                  <th className="mono border-b border-border px-4 py-2 text-right text-[10.5px] font-medium uppercase tracking-[0.08em] text-muted-2">Cost/Unit</th>
                  <th className="mono border-b border-border px-4 py-2 text-right text-[10.5px] font-medium uppercase tracking-[0.08em] text-muted-2">Sell/Unit</th>
                  <th className="mono border-b border-border px-4 py-2 text-right text-[10.5px] font-medium uppercase tracking-[0.08em] text-muted-2">Margin</th>
                  <th className="mono border-b border-border px-4 py-2 text-center text-[10.5px] font-medium uppercase tracking-[0.08em] text-muted-2">Status</th>
                  <th className="mono border-b border-border px-4 py-2 text-right text-[10.5px] font-medium uppercase tracking-[0.08em] text-muted-2">Actions</th>
                </tr>
              </thead>
              <tbody>
                {products.map((p: any) => (
                  <tr key={p.id} className={`border-b border-border last:border-b-0 hover:bg-canvas ${!p.is_active ? 'opacity-60' : ''}`}>
                    <td className="px-4 py-2 font-medium text-text">
                      {p.brand_name ? `${p.brand_name} ` : ''}{p.product_name || p.oil_type || p.fluid_type}
                      {p.grade && <span className="ml-1 text-muted">{p.grade}</span>}
                    </td>
                    <td className="px-4 py-2 text-muted capitalize">{p.fluid_type}{p.oil_type ? ` / ${p.oil_type.replace('_', ' ')}` : ''}</td>
                    <td className="mono px-4 py-2 text-right text-text">{p.total_volume ? `${Number(p.total_volume).toLocaleString()} ${p.unit_type === 'gallon' ? 'gal' : 'L'}` : '—'}</td>
                    <td className="mono px-4 py-2 text-right text-text">{p.cost_per_unit ? formatNZD(Number(p.cost_per_unit)) : '—'}</td>
                    <td className="mono px-4 py-2 text-right text-text">{p.sell_price_per_unit ? formatNZD(Number(p.sell_price_per_unit)) : '—'}</td>
                    <td className="mono px-4 py-2 text-right text-text">{p.margin_pct != null ? `${Number(p.margin_pct).toFixed(1)}%` : '—'}</td>
                    <td className="px-4 py-2 text-center">
                      <Badge variant={p.is_active ? 'success' : 'neutral'}>{p.is_active ? 'Active' : 'Inactive'}</Badge>
                    </td>
                    <td className="px-4 py-2 text-right">
                      <div className="flex justify-end gap-1">
                        <Button size="sm" variant="ghost" onClick={() => { setEditProduct({...p}); setEditSupplierIds(p.supplier_id ? [String(p.supplier_id)] : []); setEditSupplierSearch('') }}>Edit</Button>
                        <Button size="sm" variant="ghost" onClick={() => handleToggleActive(p.id)}>
      ...
      <Modal open={!!editProduct} onClose={() => setEditProduct(null)} title="Edit Fluid / Oil Product">
        {editProduct && (() => {
          ...
          <div className="space-y-3 max-h-[70vh] overflow-y-auto pr-1">
            ...
            {/* Pack format */}
            <div className="rounded-card border border-border bg-canvas p-3 space-y-2">
              <p className="text-xs font-medium text-muted uppercase">Pack Format</p>
              <div className="grid grid-cols-4 gap-2">
                <Field label="Qty/pack">
                  <input type="number" min="1" step="0.1" value={ep.qty_per_pack || ''} onChange={e => upd('qty_per_pack', e.target.value)}
                    className="w-full rounded-ctl border border-border bg-card px-2 py-1.5 text-sm text-text focus:border-accent focus:outline-none focus:shadow-[0_0_0_3px_var(--accent-soft)]" />
                </Field>
                <Field label="Unit">
              ...
              {eTotalVol > 0 && (
                <p className="text-xs text-muted">Total Volume: <span className="font-semibold text-text">{eTotalVol.toLocaleString()} {eUnitLabel}s</span></p>
              )}
            </div>
            {/* Pricing */}
            <div className="rounded-card border border-border bg-canvas p-3 space-y-2">
              <p className="text-xs font-medium text-muted uppercase">Pricing</p>
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="inline-flex rounded-ctl border border-border overflow-hidden w-full">
                  {(['inclusive', 'exclusive', 'exempt'] as const).map((mode, i) => (
                    <button key={mode} type="button" onClick={() => upd('gst_mode', mode)}
                      className={`flex-1 py-1.5 text-xs font-medium transition-colors ${i > 0 ? 'border-l border-border' : ''} ${
                        ep.gst_mode === mode ? 'bg-accent text-white' : 'bg-card text-text hover:bg-canvas'
                      }`}>
              ...
              {eCpu > 0 && (
                <div className="flex justify-between text-xs text-muted">
                  <span>Cost/{eUnitLabel}: <span className="font-semibold text-text">{formatNZD(eCpu)}</span></span>
                  {eMargin !== 0 && (
                    <span>Margin: <span className={`font-semibold ${eMargin >= 0 ? 'text-ok' : 'text-danger'}`}>{formatNZD(eMargin)} ({eMarginPct.toFixed(1)}%)</span></span>
                  )}
                </div>
              )}
            </div>
                ...
                <div className="flex flex-wrap gap-1 mb-2">
                  {editSupplierIds.map(sid => {
                    ...
                    return s ? (
                      <span key={sid} className="inline-flex items-center gap-1 rounded-full bg-accent-soft text-accent px-2.5 py-0.5 text-xs font-medium">
                        {s.name}
                        <button type="button" onClick={() => toggleEditSupplier(sid)} className="hover:text-text" aria-label={`Remove ${s.name}`}>&times;</button>
                      </span>
                    ) : null
                  })}
                </div>
                  ...
                  <div className="absolute top-full left-0 right-0 z-50 mt-1 max-h-48 overflow-auto rounded-ctl border border-border bg-card shadow-pop">
                    {suppliers.filter(s => !editSupplierSearch.trim() || s.name.toLowerCase().includes(editSupplierSearch.toLowerCase())).slice(0, 15).map(s => (
                      <button key={s.id} type="button" onClick={() => { toggleEditSupplier(s.id); setEditSupplierSearch(''); setEditSupplierDropOpen(false) }}
                        className="w-full px-3 py-2 text-left text-sm hover:bg-canvas flex items-center justify-between">
                        <span className="text-text">{s.name}</span>
                        {editSupplierIds.includes(s.id) && <span className="text-accent text-xs">✓</span>}
                      </button>
                    ))}
      ...
      <Modal open={!!deleteId} onClose={() => setDeleteId(null)} title="Delete Product">
        <p className="text-sm text-muted mb-4">This will permanently delete this fluid/oil product. This cannot be undone.</p>
      ...
      <Modal open={showResult} onClose={() => setShowResult(false)} title="Product Saved">
        <p className="text-sm text-muted">Product has been saved to the database.</p>
