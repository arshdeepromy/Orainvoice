...
export default function PartsCatalogue() {
  ...
  return (
    <div>
      ...
      {error && <div className="mb-4 rounded-ctl border border-danger/30 bg-danger-soft px-4 py-3 text-sm text-danger">{error}</div>}
        ...
        <section className="overflow-hidden rounded-card border border-border bg-card shadow-card">
          <div className="overflow-x-auto">
            <table className="w-full border-collapse">
              ...
              <tbody>
                {parts.length === 0 ? (
                  <tr><td colSpan={8} className="px-4 py-12 text-center text-sm text-muted">No parts yet.</td></tr>
                ) : parts.map(part => (
                  <tr key={part.id} className="border-b border-border last:border-b-0 hover:bg-canvas">
                    <td className="px-4 py-3 text-sm font-medium text-text">{part.name}</td>
                    <td className="mono px-4 py-3 text-sm text-muted">{part.part_number || '—'}</td>
                    <td className="px-4 py-3 text-sm text-muted capitalize">{part.part_type}</td>
                    <td className="px-4 py-3 text-sm text-muted">{part.category_name || '—'}</td>
                    <td className="mono px-4 py-3 text-sm text-text text-right font-medium">${part.sell_price_per_unit || part.default_price}</td>
                    <td className="px-4 py-3 text-sm text-muted">{part.supplier_name || '—'}</td>
                    <td className="px-4 py-3 text-sm text-center">
                      <Badge variant={part.is_active ? 'success' : 'neutral'}>{part.is_active ? 'Active' : 'Inactive'}</Badge>
                    </td>
                    <td className="px-4 py-3 text-sm text-right">
                      <div className="flex justify-end gap-1">
                        <Button size="sm" variant="ghost" onClick={() => openEdit(part)}>Edit</Button>
                        <Button size="sm" variant="ghost" onClick={() => handleToggleActive(part)}>
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-4">
            <label className="flex items-center gap-2 text-sm cursor-pointer text-text">
              <input type="radio" checked={form.part_type === 'part'} onChange={() => updateField('part_type', 'part')} className="h-4 w-4 text-accent" />
              Part
            </label>
            <label className="flex items-center gap-2 text-sm cursor-pointer text-text">
              <input type="radio" checked={form.part_type === 'tyre'} onChange={() => updateField('part_type', 'tyre')} className="h-4 w-4 text-accent" />
              Tyre
            </label>
          </div>
          ...
          {form.part_type === 'tyre' && (
            <div className="grid grid-cols-5 gap-2">
              <div>
                <label className="block text-xs font-medium text-text mb-1">Width</label>
                <input value={form.tyre_width} onChange={e => updateField('tyre_width', e.target.value)} className="w-full rounded-ctl border border-border bg-card px-2 py-1.5 text-sm text-text focus:border-accent focus:outline-none focus:shadow-[0_0_0_3px_var(--accent-soft)]" placeholder="205" />
              </div>
              <div className="flex items-end pb-1 justify-center text-muted-2 font-bold">/</div>
              <div>
                <label className="block text-xs font-medium text-text mb-1">Profile</label>
                <input value={form.tyre_profile} onChange={e => updateField('tyre_profile', e.target.value)} className="w-full rounded-ctl border border-border bg-card px-2 py-1.5 text-sm text-text focus:border-accent focus:outline-none focus:shadow-[0_0_0_3px_var(--accent-soft)]" placeholder="55" />
              </div>
              <div>
                <label className="block text-xs font-medium text-text mb-1">Rim Dia</label>
                <div className="flex items-center gap-1">
                  <span className="text-muted-2 font-bold text-sm">R</span>
                  <input value={form.tyre_rim_dia} onChange={e => updateField('tyre_rim_dia', e.target.value)} className="w-full rounded-ctl border border-border bg-card px-2 py-1.5 text-sm text-text focus:border-accent focus:outline-none focus:shadow-[0_0_0_3px_var(--accent-soft)]" placeholder="16" />
                </div>
              </div>
              <div>
                <label className="block text-xs font-medium text-text mb-1">Load/Speed</label>
                <div className="flex gap-1">
                  <input value={form.tyre_load_index} onChange={e => updateField('tyre_load_index', e.target.value)} className="w-full rounded-ctl border border-border bg-card px-1 py-1.5 text-sm text-text focus:border-accent focus:outline-none focus:shadow-[0_0_0_3px_var(--accent-soft)]" placeholder="91" />
                  <input value={form.tyre_speed_index} onChange={e => updateField('tyre_speed_index', e.target.value)} className="w-full rounded-ctl border border-border bg-card px-1 py-1.5 text-sm text-text focus:border-accent focus:outline-none focus:shadow-[0_0_0_3px_var(--accent-soft)]" placeholder="V" />
                </div>
              </div>
            </div>
          )}
          ...
          <div ref={catRef} className="relative">
            <label className="block text-[12.5px] font-medium text-text mb-1">Category</label>
            <input
              value={catSearch}
            ...
            />
            {catDropOpen && (
              <div className="absolute z-10 mt-1 w-full rounded-ctl border border-border bg-card shadow-pop max-h-48 overflow-y-auto">
                {categories.map(cat => (
                  <button key={cat.id} onClick={() => selectCategory(cat)}
                    className="w-full text-left px-3 py-2 text-sm hover:bg-accent-soft text-text">
                    {cat.name}
                  </button>
                ))}
                {catSearch.trim() && !categories.some(c => c.name.toLowerCase() === catSearch.trim().toLowerCase()) && (
                  <button onClick={handleCreateCategory} disabled={catCreating}
                    className="w-full text-left px-3 py-2 text-sm text-accent hover:bg-accent-soft border-t border-border">
                    {catCreating ? 'Creating...' : `+ Create "${catSearch.trim()}"`}
                  </button>
                )}
                {categories.length === 0 && !catSearch.trim() && (
                  <div className="px-3 py-2 text-sm text-muted-2">No categories yet</div>
                )}
              </div>
            )}
          </div>
                  ...
                  {suppliers
                    ...
                    .map(s => (
                      <button key={s.id} type="button" onClick={() => { toggleSupplier(s.id); setSupplierSearch(''); setSupplierDropOpen(false) }}
                        ...
                        {form.supplier_ids.includes(s.id) && <span className="text-accent text-xs">✓ selected</span>}
                      </button>
                    ))}
          ...
          <div className="rounded-card border border-border bg-canvas p-3 space-y-3">
            <p className="text-sm font-medium text-text">Pricing &amp; Packaging</p>

            {/* Packaging Type */}
            <div>
              <label className="block text-[12.5px] font-medium text-text mb-1">Packaging Type</label>
              <select
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-[12.5px] font-medium text-text mb-1">Qty Per Pack</label>
                <input
                  type="number"
                  min="1"
                  step="1"
                  ...
                  className={`w-full rounded-ctl border border-border bg-card px-3 py-2 text-sm text-text focus:border-accent focus:outline-none focus:shadow-[0_0_0_3px_var(--accent-soft)] ${form.packaging_type === 'single' ? 'bg-canvas text-muted-2 cursor-not-allowed' : ''}`}
                  placeholder="e.g. 10"
                />
              </div>
              <div>
                <label className="block text-[12.5px] font-medium text-text mb-1">Total Packs</label>
                <input
                  type="number"
                  min="1"
                  step="1"
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-[12.5px] font-medium text-text mb-1">Purchase Price</label>
                <div className="flex">
                  <span className="rounded-l-ctl border border-r-0 border-border bg-canvas px-3 py-2 text-sm text-muted">$</span>
                  <input
                    type="number"
                    min="0"
                    step="0.01"
                    ...
                    className="flex-1 rounded-r-ctl border border-border bg-card px-3 py-2 text-sm text-text focus:border-accent focus:outline-none focus:shadow-[0_0_0_3px_var(--accent-soft)]"
                    placeholder="0.00"
                  />
                </div>
              </div>
              <div>
                <label className="block text-[12.5px] font-medium text-text mb-1">Sell Price Per Unit</label>
                <div className="flex">
                  <span className="rounded-l-ctl border border-r-0 border-border bg-canvas px-3 py-2 text-sm text-muted">$</span>
                  <input
                    type="number"
            ...
            {/* GST Mode */}
            <div>
              <label className="block text-[12.5px] font-medium text-text mb-1">GST Mode</label>
                ...
                {([
                  ...
                  <button key={opt.value} type="button" onClick={() => updateField('gst_mode', opt.value)}
                    className={`flex-1 py-2 text-sm font-medium transition-colors ${i > 0 ? 'border-l border-border' : ''} ${
                      form.gst_mode === opt.value ? 'bg-accent text-white' : 'bg-card text-text hover:bg-canvas'
                    }`}>
                    {opt.label}
                  </button>
                ))}
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="rounded-ctl bg-card border border-border px-3 py-2 text-center">
                <span className="block text-xs text-muted">Cost/Unit</span>
                <span className="block text-sm font-semibold text-text">{costPerUnit !== null ? fmtNZD(costPerUnit) : '—'}</span>
              </div>
              <div className="rounded-ctl bg-card border border-border px-3 py-2 text-center">
                <span className="block text-xs text-muted">Margin $</span>
                <span className="block text-sm font-semibold text-text">{margin !== null ? fmtNZD(margin) : '—'}</span>
              </div>
              <div className="rounded-ctl bg-card border border-border px-3 py-2 text-center">
                <span className="block text-xs text-muted">Margin %</span>
                <span className="block text-sm font-semibold text-text">{marginPct !== null ? marginPct.toFixed(2) + '%' : '—'}</span>
              </div>
            </div>
          </div>
          ...
          <label className="flex items-center gap-2 text-sm text-text">
            <input type="checkbox" checked={form.is_active} onChange={e => updateField('is_active', e.target.checked)} className="h-4 w-4 rounded border-border text-accent" />
            Active
          </label>
        </div>
      ...
      <Modal open={!!deleteId} onClose={() => setDeleteId(null)} title="Delete Part">
        <p className="text-sm text-muted mb-4">This will permanently delete the part. This cannot be undone.</p>
