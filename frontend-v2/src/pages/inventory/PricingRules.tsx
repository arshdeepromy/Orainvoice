/**
 * PricingRules — Task 36 port of frontend/src/pages/inventory/PricingRules.tsx.
 *
 * Pricing Rules management page with create/edit forms, priority ordering, and
 * overlap validation warning. ALL logic — the module guard, fetch from
 * /v2/pricing-rules + /v2/products, the priority sort, overlap detection via
 * detectPricingRuleOverlap, create/edit POST/PUT /api/v2/pricing-rules,
 * delete, toggle, and the dynamic conditional fields — copied VERBATIM.
 * Presentation remapped onto the design tokens (FR-2b); the page is wrapped in
 * a `page page-wide` + `.page-head`. The `overlap-warning` data-testid is
 * preserved. `Button` `secondary`→`ghost`.
 *
    ...
    <div className="page page-wide">
      <div className="page-head">
        <div>
          ...
          <h1>Pricing Rules</h1>
          <p className="sub">Automate pricing by customer, volume and date</p>
        </div>
              ...
              {rules.length === 0 ? (
                ...
                rules.map((r) => (
                  <tr key={r.id} className="border-b border-border last:border-b-0 hover:bg-canvas">
                    <td className="mono whitespace-nowrap px-4 py-3 text-sm font-semibold text-text">{r.priority}</td>
                    ...
                    <td className="px-4 py-3 text-[13.5px] text-muted max-w-xs">
                    ...
                    <td className="whitespace-nowrap px-4 py-3 text-sm text-right">
                      <button onClick={() => openEdit(r)} className="text-accent hover:text-accent-press mr-2 inline-flex items-center justify-center" style={{ minWidth: 44, minHeight: 44 }} aria-label="Edit rule">Edit</button>
                      <button onClick={() => handleDelete(r.id)} className="text-danger hover:brightness-95 inline-flex items-center justify-center" style={{ minWidth: 44, minHeight: 44 }} aria-label="Delete rule">Delete</button>
                    </td>
                  </tr>
                ))
              )}
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editingId ? 'Edit Pricing Rule' : 'New Pricing Rule'}>
        ...
        {/* Overlap warning */}
        {overlapWarning && (
          <div className="mt-3 rounded-ctl border border-warn/30 bg-warn-soft px-4 py-3 text-sm text-warn" role="alert" data-testid="overlap-warning">
            ⚠️ {overlapWarning}
          </div>
        )}
        ...
        <div className="mt-4 flex justify-end gap-2">
          <Button variant="ghost" size="sm" onClick={() => setModalOpen(false)} style={{ minWidth: 44, minHeight: 44 }}>Cancel</Button>
