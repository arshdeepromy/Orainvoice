/**
 * ProductList — Task 35 port of frontend/src/pages/inventory/ProductList.tsx.
 *
 * Product list with search, category filter, barcode scanning, low-stock
 * dashboard, and supplier catalogue integration view. ALL logic — fetches from
 * /v2/products, /v2/product-categories, /v2/suppliers; low-stock filtering;
 * supplier grouping; barcode scan + PO create — copied VERBATIM. Presentation
 * remapped onto the design tokens (FR-2b). `Badge` `warning`→`warn`,
 * `success` kept; `Button` `secondary`→`ghost`.
 *
      ...
      <div className="flex gap-1 mb-4 border-b border-border pb-0">
        {(['products', 'low-stock', 'supplier-catalogue'] as ViewMode[]).map((mode) => (
          <button
            ...
            className={`-mb-px border-b-2 px-3.5 py-[11px] text-[13.5px] font-medium transition-colors inline-flex items-center justify-center
        ...
        <>
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between mb-4">
            ...
            <div className="flex gap-2">
              <Button variant="ghost" onClick={handleScanBarcode} loading={scanning} style={{ minWidth: 44, minHeight: 44 }} aria-label="Scan barcode">
                📷 Scan Barcode
              </Button>
                ...
                <table className="w-full border-collapse" role="grid">
                  ...
                  <thead>
                    <tr>
                      ...
                      <th scope="col" className={TH_R}>Price</th>
                      <th scope="col" className={TH_R}>Stock Qty</th>
                  ...
                  <tbody>
                    {products.length === 0 ? (
                      ...
                      products.map((p) => (
                        <tr key={p.id} className="border-b border-border last:border-b-0 hover:bg-canvas cursor-pointer" onClick={() => window.location.assign(`/inventory/products/${p.id}`)}>
                          <td className="whitespace-nowrap px-4 py-3 text-[13.5px] font-medium text-text">{p.name}</td>
                          <td className="mono whitespace-nowrap px-4 py-3 text-[13.5px] text-muted">{p.sku || '—'}</td>
                          <td className="whitespace-nowrap px-4 py-3 text-[13.5px] text-muted">{categoryMap.get(p.category_id || '') || '—'}</td>
                          <td className="mono whitespace-nowrap px-4 py-3 text-[13.5px] text-right text-text">{formatPrice(p.sale_price)}</td>
                          <td className="mono whitespace-nowrap px-4 py-3 text-[13.5px] text-right font-medium text-text">{parseFloat(p.stock_quantity)}</td>
              ...
              <table className="w-full border-collapse" role="grid">
                ...
                <thead>
                  <tr>
                    ...
                    <th scope="col" className={TH_R}>Reorder Point</th>
                    <th scope="col" className={TH_R}>Reorder Qty</th>
                    <th scope="col" className={TH}>Supplier</th>
                    <th scope="col" className={TH_R}>Action</th>
                  </tr>
                </thead>
                <tbody>
                  {lowStockProducts.map((p) => (
                    <tr key={p.id} className="border-b border-border last:border-b-0 bg-warn-soft/50 hover:bg-warn-soft">
                      <td className="whitespace-nowrap px-4 py-3 text-[13.5px] font-medium text-text">{p.name}</td>
                      <td className="mono whitespace-nowrap px-4 py-3 text-[13.5px] text-muted">{p.sku || '—'}</td>
                      <td className="mono whitespace-nowrap px-4 py-3 text-[13.5px] text-right font-semibold text-danger">{p.stock_level}</td>
                      <td className="mono whitespace-nowrap px-4 py-3 text-[13.5px] text-right text-muted">{p.reorder_point}</td>
                      <td className="mono whitespace-nowrap px-4 py-3 text-[13.5px] text-right text-muted">{p.reorder_quantity || '—'}</td>
                      <td className="whitespace-nowrap px-4 py-3 text-[13.5px] text-muted">{supplierMap.get(p.supplier_id || '') || '—'}</td>
          ...
          {supplierCatalogue.size === 0 ? (
            ...
            <div className="space-y-6">
              {Array.from(supplierCatalogue.entries()).map(([supplierId, prods]) => (
                <section key={supplierId} className="overflow-hidden rounded-card border border-border bg-card shadow-card">
                  <div className="bg-canvas px-4 py-3 border-b border-border">
                    <h3 className="text-[13.5px] font-semibold text-text">
                      {supplierMap.get(supplierId) || 'Unknown Supplier'}
                    </h3>
                    <p className="text-xs text-muted">{prods.length} {productLabel.toLowerCase()}(s)</p>
                  </div>
                  <div className="overflow-x-auto">
                  <table className="w-full border-collapse" role="grid">
                    <thead>
                    ...
                    <tbody>
                      {prods.map((p) => {
                        ...
                        return (
                          <tr key={p.id} className="border-b border-border last:border-b-0 hover:bg-canvas cursor-pointer" onClick={() => window.location.assign(`/inventory/products/${p.id}`)}>
                            <td className="whitespace-nowrap px-4 py-2 text-[13.5px] font-medium text-text">{p.name}</td>
                            <td className="mono whitespace-nowrap px-4 py-2 text-[13.5px] text-muted">{p.sku || '—'}</td>
                            <td className="mono whitespace-nowrap px-4 py-2 text-[13.5px] text-right text-text">{formatPrice(p.sale_price)}</td>
                            <td className="mono whitespace-nowrap px-4 py-2 text-[13.5px] text-right font-medium text-text">{stock}</td>
                            <td className="whitespace-nowrap px-4 py-2 text-sm text-center">
                              <Badge variant={isLow ? 'warn' : 'success'}>{isLow ? 'Low Stock' : 'OK'}</Badge>
                            </td>
                          </tr>
                        )
                      })}
