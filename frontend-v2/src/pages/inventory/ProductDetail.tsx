/**
 * ProductDetail — Task 35 port of frontend/src/pages/inventory/ProductDetail.tsx.
 *
 * Product detail page with tabs: Details, Stock History, Pricing Rules.
 * Supports create (no id) and edit (with id) modes. Integrates barcode
 * scanning for product lookup. ALL logic — fetches, save (create/update),
 * image upload, movement/pricing-rule fetches — copied VERBATIM. Presentation
 * remapped onto the design tokens (FR-2b). `Badge` `error`→`danger`;
...
export default function ProductDetail({ productId }: { productId?: string }) {
  ...
  if (error) {
    return <div className="rounded-ctl border border-danger/30 bg-danger-soft px-4 py-3 text-[13px] text-danger" role="alert">{error}</div>
  }
  ...
  const detailsTab = (
    <form onSubmit={handleSave} className="max-w-2xl space-y-4">
      ...
      <div>
        <label htmlFor="description" className="text-[12.5px] font-medium text-text">Description</label>
        <textarea
          id="description"
      ...
      <div className="flex gap-6">
        <label className="flex items-center gap-2 text-[13.5px] text-text">
          <input type="checkbox" checked={form.tax_applicable} onChange={(e) => updateField('tax_applicable', e.target.checked)} className="rounded border-border" />
          Tax applicable
        </label>
        <label className="flex items-center gap-2 text-[13.5px] text-text">
          <input type="checkbox" checked={form.allow_backorder} onChange={(e) => updateField('allow_backorder', e.target.checked)} className="rounded border-border" />
          Allow backorder
        </label>
      </div>
      ...
      {/* Image upload */}
      <div>
        <label className="text-[12.5px] font-medium text-text">{productLabel} images (max 5)</label>
        <div className="mt-1 flex flex-wrap gap-2">
          {existingImages.map((url, i) => (
            <div key={i} className="relative w-20 h-20 rounded-ctl border border-border overflow-hidden">
              <img src={url} alt={`${productLabel} image ${i + 1}`} className="w-full h-full object-cover" />
              <button
                type="button"
                onClick={() => setExistingImages((prev) => prev.filter((_, idx) => idx !== i))}
                className="absolute top-0 right-0 bg-danger text-white text-xs px-1 rounded-bl"
                style={{ minWidth: 44, minHeight: 44 }}
          ...
          {imageFiles.map((f, i) => (
            <div key={`new-${i}`} className="w-20 h-20 rounded-ctl border border-accent/30 bg-accent-soft flex items-center justify-center text-xs text-accent overflow-hidden">
              {f.name.slice(0, 10)}
            </div>
          ))}
        </div>
      ...
      {success && <div className="rounded-ctl border border-ok/30 bg-ok-soft px-4 py-3 text-[13px] text-ok" role="status">{success}</div>}

      <div className="flex gap-2">
        <Button type="submit" loading={saving} style={{ minWidth: 44, minHeight: 44 }}>{productId ? 'Save Changes' : `Create ${productLabel}`}</Button>
        <Button type="button" variant="ghost" onClick={() => window.history.back()} style={{ minWidth: 44, minHeight: 44 }}>Cancel</Button>
      </div>
    </form>
  )
  ...
  const stockHistoryTab = (
    <div>
      {product && (
        <div className="mb-4 rounded-ctl border border-border bg-canvas p-3 text-sm text-text">
          Current stock: <span className="mono font-semibold">{parseFloat(product.stock_quantity)}</span> {product.unit_of_measure}
        </div>
      )}
      {movements.length === 0 ? (
        <p className="text-[13px] text-muted py-8 text-center">No stock movements recorded yet.</p>
      ) : (
              ...
              {movements.map((m) => {
                ...
                return (
                  <tr key={m.id} className="border-b border-border last:border-b-0 hover:bg-canvas">
                    <td className="mono whitespace-nowrap px-4 py-3 text-[13.5px] text-muted">{new Date(m.created_at).toLocaleDateString('en-NZ')}</td>
                    <td className="whitespace-nowrap px-4 py-3 text-sm">
                      <Badge variant={change > 0 ? 'success' : change < 0 ? 'danger' : 'neutral'}>{m.movement_type}</Badge>
                    </td>
                    <td className={`mono whitespace-nowrap px-4 py-3 text-[13.5px] text-right font-medium ${change > 0 ? 'text-ok' : 'text-danger'}`}>
                      {change > 0 ? '+' : ''}{change}
                    </td>
                    <td className="mono whitespace-nowrap px-4 py-3 text-[13.5px] text-right text-text">{parseFloat(m.resulting_quantity)}</td>
                    <td className="px-4 py-3 text-[13.5px] text-muted">{m.notes || m.reference_type || '—'}</td>
                  </tr>
                )
              })}
            </tbody>
  ...
  const pricingRulesTab = (
    <div>
      {pricingRules.length === 0 ? (
        <p className="text-[13px] text-muted py-8 text-center">No pricing rules configured for this {productLabel.toLowerCase()}.</p>
      ) : (
        <section className="overflow-hidden rounded-card border border-border bg-card shadow-card">
          <div className="overflow-x-auto">
          <table className="w-full border-collapse" role="grid">
            ...
            <thead>
              <tr>
                ...
                <th scope="col" className={TH}>Type</th>
                <th scope="col" className={TH_R}>Price / Discount</th>
            ...
            <tbody>
              {pricingRules.map((r) => (
                <tr key={r.id} className="border-b border-border last:border-b-0 hover:bg-canvas">
                  <td className="mono whitespace-nowrap px-4 py-3 text-[13.5px] font-medium text-text">{r.priority}</td>
                  <td className="whitespace-nowrap px-4 py-3 text-[13.5px] text-muted">{r.rule_type}</td>
    ...
    <div className="page page-wide">
      <div className="page-head">
        <div>
          ...
          <h1>{productId ? (product?.name || productLabel) : `New ${productLabel}`}</h1>
        </div>
      </div>
