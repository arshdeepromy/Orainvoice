 ...
 * camera/geolocation capture, handlers and every data-testid preserved
 * verbatim.
 */
...
export default function SelfServiceClockScreen() {
  ...
  if (loading) {
    return (
      <div className="mx-auto max-w-md p-6 text-center text-muted">
        Loading…
      </div>
    )
  }
  ...
  if (loadError) {
    return (
      <div
        role="alert"
        className="mx-auto mt-6 max-w-md rounded-card border border-danger/30 bg-danger-soft px-4 py-3 text-sm text-danger"
  ...
  return (
    ...
    >
      ...
      {!showClockButton && (
        <div
          role="status"
          className="rounded-card border border-warn/30 bg-warn-soft px-4 py-3 text-sm text-warn"
      ...
      {showClockButton && (
        <button
          ...
          disabled={busy}
          className={`flex min-h-[120px] w-full items-center justify-center rounded-card px-6 py-6 text-2xl font-semibold text-white shadow-card focus:outline-none focus:shadow-[0_0_0_4px_var(--accent-soft)] disabled:cursor-not-allowed disabled:opacity-60 ${
            action === 'in'
      ...
      {actionMessage && (
        <p
          role="status"
          className="rounded-ctl bg-ok-soft px-3 py-2 text-center text-sm font-medium text-ok"
      ...
      {showRunningLate && inWindowShift && (
        <button
          ...
          className="min-h-[44px] w-full rounded-card border border-warn/40 bg-card px-4 py-3 text-sm font-semibold text-warn hover:bg-warn-soft"
