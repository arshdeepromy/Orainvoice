/**
 * QuoteDetail — Task 21 port of frontend/src/pages/quotes/QuoteDetail.tsx.
 *
 * Quote detail view with a document-template preview matching the invoice
 * preview style. EVERY action (download PDF, print, copy link, send/email,
 * issue, convert-to-invoice, requote, delete, cancel) and ALL money display
 * (line tax = line_total × 0.15, amount = line_total × 1.15 unless gst-exempt,
 * subtotal/discount/gst/total) is copied VERBATIM — this is money, do not
 * "simplify". The print stylesheet, template-style resolution, status gating,
 * and AbortController-free fetch (matches original) are all preserved.
 *
 * The action toolbar is reframed onto the design-system tokens (FR-2 / FR-2b);
 * `Button variant="secondary"` maps to the v2 `ghost` variant (v2 has no
 * `secondary`). The document template preview keeps its template-driven inline
 * styling and neutral palette byte-for-byte so the rendered quote matches the
...
export default function QuoteDetail({ quoteId }: QuoteDetailProps) {
  ...
  if (loading) {
    return (
      <div className="px-4 py-16 text-center sm:px-6 lg:px-8">
        <div className="text-muted" role="status" aria-label="Loading quote">Loading quote…</div>
      </div>
    )
  }
  ...
  if (!quote) {
    return (
      <div className="px-4 py-16 text-center sm:px-6 lg:px-8">
        <div className="text-danger">Quote not found</div>
        <button onClick={() => navigate('/quotes')} className="mt-4 text-sm text-accent hover:text-accent-press">
          Back to Quotes
        </button>
      </div>
    )
    ...
    <div className="px-4 py-6 sm:px-6 lg:px-8">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6" data-print-hide>
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold text-text mono">{quote.quote_number}</h1>
        ...
        <div className="flex flex-wrap items-center justify-end gap-2">
          ...
          {canSend && (
            <>
              <Button variant="ghost" onClick={() => navigate(`/quotes/${quoteId}/edit`)}>Edit</Button>
              {isDraft && (
                <Button variant="ghost" onClick={handleIssueQuote} loading={actionLoading} disabled={actionLoading}>
                  Issue Quote
                </Button>
              )}
          ...
          {canRequote && (
            <Button variant="ghost" onClick={handleRequote} loading={actionLoading} disabled={actionLoading}>
              Requote
            </Button>
          )}
          {canConvert && (
            <Button variant="ghost" onClick={handleConvert} loading={actionLoading} disabled={actionLoading}>
              Convert to Invoice
            </Button>
          )}
          {canCancel && (
            <button
              ...
              className="rounded-ctl border border-danger/30 px-3 py-1.5 text-sm font-medium text-danger hover:bg-danger-soft"
          ...
          {deleteConfirm && (
            <div className="flex items-center gap-2">
              <span className="text-sm text-danger">Delete this quote?</span>
              ...
              <button onClick={() => setDeleteConfirm(false)}
                className="rounded-ctl border border-border px-3 py-1.5 text-sm font-medium text-muted hover:bg-canvas">
                Cancel
              </button>
            </div>
          )}
