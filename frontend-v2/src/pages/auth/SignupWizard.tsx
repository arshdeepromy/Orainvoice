...
/**
 * SignupWizard — multi-step "start free trial" wizard, routed at /signup (Task
 * 13 port of frontend/src/pages/auth/SignupWizard — the component the original
 * `frontend/src/App.tsx` lazy-loads for /signup).
 *
 * ALL logic is copied verbatim from the original: the four sliding form cards
 * (Welcome → Business → Plan → Security), per-card validation, the card
 * slide animation state machine, plans + trade-families fetch (refetch on
 * country change), coupon validate/apply/remove + effective-price maths, the
 * CAPTCHA load/verify/refresh flow, the deferred Stripe key load, the
 * /auth/signup submit → requires_payment branch (payment step / config-error /
 * done), and the PaymentStep / ConfirmationStep handoff with session-expiry
 * reset. The component renders into the AuthLayout `<Outlet/>` (Task 12).
 *
 * Styling is remapped to the design system per OraInvoice_Handoff/app/Signup.html
 * (the `.wiz-steps` progress indicator, token cards, `.plan` selection styling)
 * — every gray and blue utility from the original is mapped to a design token.
 */
...
/**
 * StepDots — wizard progress indicator. Logic (done/active per index) is
 * unchanged from the original; styling matches the prototype's `.wiz-steps`
 * (mono numerals, accent active circle, ok done circle, connecting bars).
 */
function StepDots({ steps, current }: { steps: readonly { key: string; label: string }[]; current: number; total: number }) {
  return (
    <div className="mb-[30px] flex items-center justify-center gap-0" role="navigation" aria-label="Signup progress">
      {steps.map((s, i) => {
        ...
        return (
          <div key={s.key} className="flex items-center gap-0">
            <div className="flex items-center gap-[11px]">
              <div
                className={`mono flex h-[30px] w-[30px] flex-shrink-0 items-center justify-center rounded-full border text-[13px] font-semibold transition-all duration-300 ${
                  active
              ...
              <span className={`whitespace-nowrap text-[13px] font-semibold transition-colors duration-200 ${active || done ? 'text-text' : 'text-muted-2'}`}>
                {s.label}
              </span>
            </div>
            {i < steps.length - 1 && (
              <div className={`mx-[14px] h-px min-w-[24px] flex-1 transition-colors duration-300 ${i < current ? 'bg-ok' : 'bg-border'}`} />
            )}
          </div>
        )
      })}
  ...
  return (
    <div className="w-full max-w-lg">
        ...
        <div ref={cardRef} className={`transition-all duration-300 ease-in-out ${animClass}`}>
          <div className="rounded-card border border-border bg-card p-8 shadow-card">
            ...
            {cardIndex === 0 && (
              <div>
                <div className="mb-6 text-center">
                  ...
                  <p className="mt-1 text-[13.5px] text-muted">Let's get you set up. Where is your business based?</p>
                </div>
                ...
                <div className="space-y-5">
                  ...
                  <div>
                    <label className="mb-2 block text-[12.5px] font-medium text-text">What type of business do you run?</label>
                    {tradeFamiliesLoading ? (
                      <div className="flex h-[42px] items-center"><Spinner size="sm" label="Loading..." /></div>
                        ...
                        {tradeFamilies.map(family => {
                          ...
                          return (
                            <button key={family.slug} type="button"
                              ...
                              className={`flex items-center gap-2 rounded-ctl border px-3 py-2.5 text-left text-[13px] transition-all duration-150
                                focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent
                                ${sel ? 'border-accent bg-accent-soft text-accent shadow-sm' : 'border-border text-text hover:border-border-strong hover:bg-canvas'}`}
                            >
                              <span className="flex-shrink-0 text-lg" aria-hidden="true">{icon}</span>
                              <span className="text-[12px] font-medium leading-tight">{family.display_name}</span>
                            </button>
                          )
                        })}
                      </div>
            ...
            {cardIndex === 1 && (
              <div>
                <div className="mb-6 text-center">
                  <h1 className="text-2xl font-bold text-text">Business details</h1>
                  <p className="mt-1 text-[13.5px] text-muted">Tell us about your organisation</p>
                </div>
            ...
            {cardIndex === 2 && (
              <div>
                <div className="mb-6 text-center">
                  ...
                  <p className="mt-1 text-[13.5px] text-muted">Select the plan that works for you</p>
                </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      {plans.map(plan => {
                        ...
                        return (
                          <label key={plan.id}
                            className={`flex items-center justify-between rounded-ctl border p-4 transition-all duration-150 ${
                              isUnavailable ? 'cursor-not-allowed border-border bg-canvas opacity-60'
                                : isSelected ? 'cursor-pointer border-accent bg-accent-soft shadow-sm'
                                : 'cursor-pointer border-border hover:border-border-strong hover:shadow-sm'
                            }`}>
                            <div className="flex items-center gap-3">
                              <input type="radio" name="plan_id" value={plan.id} checked={isSelected} disabled={isUnavailable}
                                onChange={() => handleFieldChange('plan_id', plan.id)} className="h-4 w-4 accent-accent" />
                              <div>
                            ...
                            {!isUnavailable && (
                              <div className="text-right">
                                {discountPercent > 0 && <span className="mb-0.5 inline-block rounded-full bg-ok-soft px-2 py-0.5 text-[10px] font-semibold text-ok">Save {discountPercent}%</span>}
                                {couponApplied && effectivePrice !== displayPrice ? (
                                  <>
                                    <span className="mono text-[13px] text-muted-2 line-through">${(displayPrice ?? 0).toFixed(2)}</span>
                                    <span className="mono ml-1 text-[13px] font-semibold text-text">${(effectivePrice ?? 0).toFixed(2)}{priceSuffix}</span>
                                    <span className="block text-[12px] text-muted">excl. GST</span>
                                  </>
                                ) : (
                                  <>
                                    <span className="mono text-[13px] font-semibold text-text">${(displayPrice ?? 0).toFixed(2)}{priceSuffix}</span>
                                    {displayPrice > 0 && <span className="block text-[12px] text-muted">excl. GST</span>}
                                  </>
                                )}
                                {savingsAmount > 0 && <span className="block text-[12px] text-ok">You save ${(savingsAmount ?? 0).toFixed(2)}{priceSuffix}</span>}
                                {selectedInterval !== 'monthly' && couponEquivMonthly > 0 && <span className="block text-[12px] text-muted-2">${(couponEquivMonthly ?? 0).toFixed(2)}/mo equivalent</span>}
                              </div>
                            )}
                          </label>
                        )
                    ...
                    <div className="pt-2">
                      {!showCouponInput ? (
                        <button type="button" onClick={() => setShowCouponInput(true)} className="text-[13px] text-accent hover:underline">Have a coupon code?</button>
                      ) : (
                        <div className="space-y-2">
                          <label className="block text-[12.5px] font-medium text-text">Coupon code</label>
                          {couponApplied ? (
                            <div className="flex items-center justify-between rounded-ctl border border-ok/40 bg-ok-soft p-3">
                              <span className="text-[13px] text-ok">{couponApplied.code} — {formatDiscountLabel(couponApplied)}</span>
                              <button type="button" onClick={handleRemoveCoupon} className="text-[13px] text-danger hover:underline">Remove</button>
                            </div>
                          ) : (
                            <div className="flex gap-2">
                              <input type="text" value={couponCode} onChange={e => setCouponCode(e.target.value.toUpperCase())}
                                placeholder="Enter code" className="flex-1 rounded-ctl border border-border px-3 py-2 text-[13px] focus:border-accent focus:outline-none focus:shadow-[0_0_0_3px_var(--accent-soft)]" />
                              <Button type="button" variant="ghost" size="sm" onClick={handleApplyCoupon} loading={couponValidating}>Apply</Button>
                            </div>
                          )}
            ...
            {cardIndex === 3 && (
              <form onSubmit={handleSubmit}>
                <div className="mb-6 text-center">
                  <h1 className="text-2xl font-bold text-text">Secure your account</h1>
                  <p className="mt-1 text-[13.5px] text-muted">Set a strong password and verify you're human</p>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  {/* CAPTCHA */}
                  <div>
                    <label className="mb-2 block text-[12.5px] font-medium text-text">Verification</label>
                    <div className="flex items-start gap-3">
                      <div className="flex-shrink-0">
                        {captchaLoading ? (
                          <div className="h-12 w-32 animate-pulse rounded bg-canvas" />
                        ) : (
                          <img src={captchaUrl} alt="CAPTCHA" className="h-12 cursor-pointer rounded-ctl border border-border"
                      ...
                      <div className="flex-1 space-y-1">
                        <div className="flex gap-2">
                          <input type="text" maxLength={6} value={formData.captcha_code}
                            ...
                            placeholder="Enter code" className="w-28 rounded-ctl border border-border px-3 py-1.5 text-[13px] focus:border-accent focus:outline-none focus:shadow-[0_0_0_3px_var(--accent-soft)]"
                            disabled={captchaVerified} />
                          {!captchaVerified ? (
                            <Button type="button" variant="ghost" size="sm" onClick={verifyCaptcha} loading={captchaVerifying}>Verify</Button>
                          ) : (
                            <span className="flex items-center text-[13px] font-medium text-ok">✓ Verified</span>
                          )}
                        </div>
                        ...
                        <button type="button" onClick={refreshCaptcha} className="text-[12px] text-muted hover:underline">Refresh image</button>
                      </div>
                    </div>
                  </div>
                </div>
      ...
      <p className="mt-6 text-center text-[13.5px] text-muted">
        ...
        <a href="/login" className="font-semibold text-accent hover:text-accent-press">Log in</a>
      </p>
    </div>
  )
}
