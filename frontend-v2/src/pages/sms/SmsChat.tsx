...
import { Button, Input, Spinner, AlertBanner } from '@/components/ui'
...
function StatusIndicator({ status }: { status: string }) {
  const map: Record<string, { label: string; color: string; icon: string }> = {
    pending: { label: 'Pending', color: 'text-muted-2', icon: '○' },
    accepted: { label: 'Accepted', color: 'text-accent', icon: '✓' },
    queued: { label: 'Queued', color: 'text-accent', icon: '◷' },
    delivered: { label: 'Delivered', color: 'text-ok', icon: '✓✓' },
    undelivered: { label: 'Undelivered', color: 'text-danger', icon: '✕' },
    failed: { label: 'Failed', color: 'text-danger', icon: '✕' },
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" role="dialog" aria-modal="true" aria-label="New conversation">
      <div className="w-full max-w-md rounded-card bg-card p-6 shadow-pop">
        <h3 className="text-lg font-semibold text-text mb-4">New Conversation</h3>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            />
            {validating && <p className="mt-1 text-xs text-muted-2">Validating…</p>}
          ...
          <div>
            <label htmlFor="new-conv-body" className="text-sm font-medium text-text">Message</label>
            <textarea
              id="new-conv-body"
              className="mt-1 w-full rounded-ctl border border-border bg-card px-3 py-2 text-sm text-text shadow-sm focus:border-accent focus:outline-none focus:shadow-[0_0_0_3px_var(--accent-soft)]"
              rows={3}
  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        className="rounded p-1.5 text-muted-2 hover:text-text hover:bg-canvas transition-colors"
      ...
      {open && (
        <div className="absolute bottom-full left-0 z-30 mb-1 w-64 max-h-48 overflow-y-auto rounded-card border border-border bg-card py-1 shadow-pop" role="menu" aria-label="SMS templates">
          {templates.map((t) => (
            <button
              key={t.id}
              role="menuitem"
              className="w-full px-3 py-2 text-left text-sm hover:bg-canvas focus-visible:bg-canvas focus-visible:outline-none"
  ...
  return (
    <div className="flex h-full flex-col border-r border-border bg-card">
      ...
      <div className="flex items-center justify-between border-b border-border px-4 py-3">
        <h2 className="text-lg font-semibold text-text">SMS</h2>
      ...
      <div className="flex-1 overflow-y-auto" role="listbox" aria-label="Conversations">
        ...
        {!loading && conversations.length === 0 && (
          <p className="px-4 py-8 text-center text-sm text-muted-2">No conversations yet</p>
        )}
        {conversations.map((c) => (
          <button
            key={c.id}
            ...
            onClick={() => onSelect(c)}
            className={`w-full px-4 py-3 text-left transition-colors hover:bg-canvas focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-accent ${
              c.id === selectedId ? 'bg-accent-soft' : ''
            }`}
          >
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  <span className="truncate font-medium text-text">
                    {c.contact_name || <span className="mono">{c.phone_number}</span>}
                  </span>
                  {c.unread_count > 0 && (
                    <span className="inline-flex h-5 min-w-[20px] items-center justify-center rounded-full bg-accent px-1.5 text-xs font-semibold text-white" aria-label={`${c.unread_count} unread`}>
                      {c.unread_count}
                    </span>
                  )}
                </div>
                {c.contact_name && (
                  <p className="mono text-xs text-muted-2">{c.phone_number}</p>
                )}
                <p className="mt-0.5 truncate text-sm text-muted">{c.last_message_preview}</p>
              </div>
              <span className="mono shrink-0 text-xs text-muted-2">
  ...
  if (!conversation) {
    return (
      <div className="flex h-full items-center justify-center bg-canvas">
        <p className="text-muted-2">Select a conversation to start messaging</p>
      </div>
    )
  }
  ...
  return (
    <div className="flex h-full flex-col bg-card">
      ...
      <div className="flex items-center gap-3 border-b border-border px-4 py-3">
        ...
        <button
          onClick={onBack}
          className="rounded p-1 text-muted-2 hover:text-text md:hidden"
        ...
        <div className="min-w-0 flex-1">
          <h3 className="truncate font-medium text-text">
            {conversation.contact_name || <span className="mono">{conversation.phone_number}</span>}
          </h3>
          {conversation.contact_name && (
            <p className="mono text-xs text-muted">{conversation.phone_number}</p>
          )}
        </div>
      </div>
          ...
          return (
            <div key={m.id} className={`flex ${isOutbound ? 'justify-end' : 'justify-start'}`}>
              ...
              >
                ...
                <div className={`mono mt-1 flex items-center gap-1.5 text-xs ${isOutbound ? 'text-white/70' : 'text-muted-2'}`}>
      ...
      <div className="border-t border-border px-4 py-3">
        <div className="flex items-end gap-2">
          ...
          <div className="flex-1">
            <textarea
              className="w-full resize-none rounded-ctl border border-border bg-card px-3 py-2 text-sm text-text shadow-sm placeholder:text-muted-2 focus:border-accent focus:outline-none focus:shadow-[0_0_0_3px_var(--accent-soft)]"
              rows={2}
...
export function SmsChat() {
  ...
  /* ---- render ---- */
  return (
    <div className="relative flex h-[calc(100vh-4rem)] overflow-hidden rounded-card border border-border bg-card">
      {error && (
