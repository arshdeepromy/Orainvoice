...
/**
 * QrPaymentAmountModal — Task 19 port of
 * frontend/src/pages/invoices/QrPaymentAmountModal.
 *
 * All logic copied VERBATIM: full/partial mode, amount sanitisation (truncate to
 * 2dp), Stripe $0.50 minimum + balance-cap validation, focus management, Escape /
 * backdrop close (suppressed while loading), and the `onContinue(null | amount)`
 * contract. Styling remapped onto the design-system tokens (accent / danger).
 * Shared with Task 22.
 */
...
export function QrPaymentAmountModal({
  ...
  return (
    ...
    >
      <div className="flex w-full max-w-md flex-col rounded-card bg-card shadow-pop">
        {/* Header */}
        <div className="flex items-start justify-between border-b border-border px-6 py-4">
          ...
          <button
            ...
            disabled={loading}
            aria-label="Close"
            className="ml-4 inline-flex h-11 w-11 items-center justify-center rounded-ctl text-muted-2 hover:bg-canvas hover:text-text focus:outline-none focus:ring-2 focus:ring-accent disabled:opacity-50"
          >
            <svg
              className="h-5 w-5"
              fill="none"
        ...
        <div className="px-6 py-5">
          <div role="radiogroup" aria-label="Payment amount" className="space-y-3">
            {/* Full payment radio */}
            <label
              className={`flex min-h-[44px] cursor-pointer items-center gap-3 rounded-ctl border px-4 py-3 transition-colors ${
                mode === 'full'
              ...
              <div className="pl-1 pt-1">
                ...
                <div className="relative mt-1">
                  ...
                  <input
                    ...
                    className={`block w-full min-h-[44px] rounded-ctl border pl-7 pr-3 py-2 text-base shadow-sm focus:outline-none focus:ring-2 disabled:bg-canvas disabled:opacity-60 ${
                      validationError
        ...
        {/* Footer */}
        <div className="flex items-center justify-between gap-3 border-t border-border bg-canvas px-6 py-4 rounded-b-card">
          <button
            type="button"
            onClick={onClose}
            disabled={loading}
            className="inline-flex min-h-[44px] items-center justify-center rounded-ctl border border-border bg-card px-5 py-2 text-sm font-medium text-text hover:bg-canvas focus:outline-none focus:ring-2 focus:ring-accent focus:ring-offset-2 disabled:opacity-50"
          >
            Cancel
          </button>
          <button
            ...
            className="inline-flex min-h-[44px] items-center justify-center gap-2 rounded-ctl bg-accent px-5 py-2 text-sm font-medium text-white shadow-sm hover:bg-accent-press focus:outline-none focus:ring-2 focus:ring-accent focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
          >
            {loading && (
              <svg
