...
/* ============================================================
   InvoiceDetail — Task 20 port of frontend/src/pages/invoices/InvoiceDetail.tsx.

   Read-only/detail view of a single invoice + every action (edit, credit note,
   refund, payment link, QR payment, duplicate, void, email, reminder, print,
   POS print, receipt preview, download PDF, report issue). ALL logic — fetch,
   payment-summary maths, GST line maths, profit-margin computation, status
   gating — is copied VERBATIM. v2 Button `secondary` → `ghost`. The neutral
   gray/blue palette is available in v2 and kept here so the page matches the
   InvoiceList split-panel detail; framed per FR-2 / FR-2b.

   ROUTING NOTE: the real frontend (and Task 19's v2 wiring) routes
   `/invoices/:id` to the InvoiceList split-panel, NOT to this page. This
   component is ported for parity (it is the standalone detail page used by the
   original tests + reachable design) and so the actions/markup exist in v2.
   ============================================================ */
...
/** Map the original BadgeVariant union onto the v2 Badge variants. */
function toBadgeVariant(v: BadgeVariant): 'success' | 'info' | 'warn' | 'danger' | 'neutral' {
  switch (v) {
...
export default function InvoiceDetail() {
  ...
  useEffect(() => {
    ...
    if (passedInvoice) {
      setInvoice(passedInvoice as unknown as InvoiceDetailData)
      setLoading(false)
    ...
    <div className="px-4 py-6 sm:px-6 lg:px-8" data-print-content>
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-6" data-print-hide>
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-semibold text-gray-900 mono">
            {invoice.invoice_number || 'Draft Invoice'}
          </h1>
          <Badge variant={toBadgeVariant(statusCfg.variant)} data-status-badge className="badge-print">
            {statusCfg.label}
          </Badge>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {isCreditNoteButtonVisible(invoice.status) && (
            <Button size="sm" variant="ghost" onClick={() => setCreditNoteModalOpen(true)}>
              Create Credit Note
            </Button>
          )}
          {isRefundButtonVisible(invoice.amount_paid) && (
            <Button size="sm" variant="ghost" onClick={() => setRefundModalOpen(true)}>
              Process Refund
            </Button>
          )}
          {canShowPaymentLink && (
          ...
          {canShowPaymentLink && (
            <Button size="sm" variant="ghost" onClick={handleRegeneratePaymentLink} loading={regeneratingLink}>
              Regenerate Payment Link
            </Button>
          )}
          {canShowQrPayment && (
          ...
          <Button size="sm" variant="ghost" onClick={handleEmail} loading={emailing}>
            Email
          </Button>
          ...
          {!isDraft && (
            <Button size="sm" variant="ghost" onClick={handlePosPrint} loading={posPrinting}>
          ...
          {!isDraft && (
            <Button
              size="sm"
              variant={showReceiptPreview ? 'primary' : 'ghost'}
      ...
      <div className="hidden print:block mb-8">
        <div className="flex items-start justify-between">
          ...
          <div className="text-right">
            ...
            <Badge variant={toBadgeVariant(statusCfg.variant)} data-status-badge className="badge-print mt-1">
              {statusCfg.label}
            </Badge>
          </div>
        </div>
        ...
        <div>
          ...
          <dl className="space-y-1 text-sm">
            <div className="flex gap-2">
              ...
              <dd className="text-gray-900 font-medium mono">{invoice.invoice_number || 'Not assigned'}</dd>
            </div>
            <div className="flex gap-2">
              <dt className="text-gray-500 w-24">Issued:</dt>
              <dd className="text-gray-900 mono">{formatDate(invoice.issue_date)}</dd>
            </div>
            <div className="flex gap-2">
              <dt className="text-gray-500 w-24">Due:</dt>
              <dd className="text-gray-900 mono">{formatDate(invoice.due_date)}</dd>
            </div>
            <div className="flex gap-2">
              <dt className="text-gray-500 w-24">Created:</dt>
              <dd className="text-gray-900 mono">{formatDate(invoice.created_at)}</dd>
            </div>
          </dl>
        </div>
      </div>
      ...
      <Modal open={voidModalOpen} onClose={() => { setVoidModalOpen(false); setVoidError('') }} title="Void Invoice">
        ...
        <div className="mt-4 flex justify-end gap-2">
          <Button variant="ghost" size="sm" onClick={() => { setVoidModalOpen(false); setVoidError('') }}>
            Cancel
          </Button>
      ...
      >
        ...
        <div className="flex flex-wrap gap-2">
          <Button
            size="sm"
            variant={linkCopied ? 'primary' : 'ghost'}
