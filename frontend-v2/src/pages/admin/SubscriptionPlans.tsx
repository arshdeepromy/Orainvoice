  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <label className="text-sm font-medium text-text">Enabled modules</label>
        <div className="flex gap-2">
          <button type="button" className="text-xs text-accent hover:underline" onClick={selectAll}>Select all</button>
          <button type="button" className="text-xs text-accent hover:underline" onClick={selectNone}>Core only</button>
        </div>
      </div>
      <div className="max-h-64 overflow-y-auto rounded-ctl border border-border p-3 space-y-4" role="group" aria-label="Module selection">
      ...
      <p className="text-xs text-muted">{selected.length} module{selected.length !== 1 ? 's' : ''} selected</p>
    </div>
  )
}
  ...
  return (
    <Modal open={open} onClose={onClose} title={editPlan ? 'Edit plan' : 'Create plan'} className="max-w-2xl">
      <form onSubmit={handleSubmit} className="space-y-4">
        {/* Tabs */}
        <div className="flex border-b border-border -mx-6 px-6" role="tablist" aria-label="Plan form tabs">
          {tabs.map(t => (
            <button
              key={t.key}
              type="button"
        ...
        {activeTab === 'general' && (
          <div className="space-y-4" role="tabpanel" aria-label="General settings">
            ...
            <fieldset className="rounded-ctl border border-border p-3 space-y-2">
              <legend className="text-sm font-medium text-text px-1">Carjam vehicle lookups</legend>
              <label className="flex items-center gap-2 text-sm cursor-pointer">
                ...
                <span>
                  ...
                  <br />
                  <span className="text-xs text-muted">Only needed for automotive trades (workshops, mechanics, fleet). Leave off for other industries.</span>
                </span>
              </label>
            ...
            <fieldset className="rounded-ctl border border-border p-3 space-y-2">
              <legend className="text-sm font-medium text-text px-1">PPSR vehicle lookups</legend>
              <label className="flex items-center gap-2 text-sm cursor-pointer">
                <input
                  type="checkbox"
            ...
            <fieldset className="rounded-ctl border border-border p-3 space-y-2">
              <legend className="text-sm font-medium text-text px-1">PPSR hidden-plate lookups</legend>
              <label className="flex items-center gap-2 text-sm cursor-pointer">
                <input
                  type="checkbox"
            ...
            {/* SMS service */}
            <fieldset className="rounded-ctl border border-border p-3 space-y-2">
              <legend className="text-sm font-medium text-text px-1">SMS service</legend>
              <label className="flex items-center gap-2 text-sm cursor-pointer">
                <input
                  type="checkbox"
                ...
                <span>
                  ...
                  <br />
                  <span className="text-xs text-muted">Enable SMS reminders, appointment confirmations and overdue notices for this plan. MFA verification codes are always available regardless of this setting.</span>
                </span>
              </label>
              {form.sms_included && (
                <div className="ml-6 space-y-3">
                  ...
                  <div className="space-y-2">
                    <h4 className="text-sm font-medium text-text">SMS package tiers</h4>
                    <p className="text-xs text-muted">Bulk SMS packages organisations can purchase as add-ons.</p>
                    {form.sms_package_pricing.length === 0 && (
                      <p className="text-sm text-muted">No SMS package tiers configured.</p>
                    )}
                    ...
                    <Button type="button" variant="ghost" size="sm" onClick={addSmsPackageTier}>
            ...
            {/* Visibility */}
            <fieldset className="rounded-ctl border border-border p-3 space-y-2">
              <legend className="text-sm font-medium text-text px-1">Plan visibility</legend>
              <label className="flex items-start gap-2 text-sm cursor-pointer">
                <input type="radio" name="visibility" checked={form.is_public} onChange={() => set('is_public', true)} className="mt-0.5" />
                <span>
                  <span className="font-medium">Public</span>
                  <br />
                  <span className="text-xs text-muted">Visible on the public signup page. Organisations can self-select this plan.</span>
                </span>
              </label>
              <label className="flex items-start gap-2 text-sm cursor-pointer">
                ...
                <span>
                  ...
                  <br />
                  <span className="text-xs text-muted">Hidden from signup. Can only be manually assigned to organisations or used for custom pricing.</span>
                </span>
              </label>
            </fieldset>
            ...
            {/* Trial period */}
            <fieldset className="rounded-ctl border border-border p-3 space-y-2">
              <legend className="text-sm font-medium text-text px-1">Trial period</legend>
              <p className="text-xs text-muted">Set to 0 for no trial. New signups on this plan will get a free trial for the specified duration.</p>
              <div className="flex items-end gap-3">
                ...
                <div className="flex-1">
                  ...
                  <select
                    ...
                    className="block w-full rounded-ctl border border-border bg-card px-3 py-2 text-sm focus:border-accent focus:outline-none focus:shadow-[0_0_0_3px_var(--accent-soft)]"
        ...
        {activeTab === 'storage' && (
          <div className="space-y-4" role="tabpanel" aria-label="Storage configuration">
            ...
            <div className="space-y-3">
              <h3 className="text-sm font-medium text-text">Add-on storage tiers</h3>
              {form.storage_tier_pricing.length === 0 && (
                <p className="text-sm text-muted">No add-on storage tiers configured. Organisations will only have the included storage.</p>
              )}
              ...
              <Button type="button" variant="ghost" size="sm" onClick={addStorageTier}>
                Add storage tier
              </Button>
            </div>
          </div>
        ...
        {activeTab === 'pricing' && (
          <div className="space-y-4" role="tabpanel" aria-label="Pricing configuration">
            <div>
              <h3 className="text-sm font-semibold text-text mb-1">Intervals &amp; Discounts</h3>
              <p className="text-xs text-muted">
            ...
            {errors.interval_config && (
              <div className="rounded-ctl bg-danger-soft border border-danger p-3">
                <p className="text-sm text-danger">{errors.interval_config}</p>
              </div>
            )}
                ...
                return (
                  <div
                    ...
                    className={`rounded-ctl border p-3 ${
                      ic.enabled ? 'border-accent bg-accent-soft/50' : 'border-border bg-canvas'
                    }`}
                  >
                    <div className="flex items-center justify-between">
                      <label className="flex items-center gap-2 text-sm cursor-pointer">
                        ...
                        {isMonthly && (
                          <span className="text-xs text-muted-2">(always enabled)</span>
                        )}
                      </label>
                    ...
                    {ic.enabled && (
                      <div className="mt-2 ml-6 flex items-center gap-3">
                        <div className="w-32">
                          <label className="block text-xs text-muted mb-0.5" htmlFor={`discount-${ic.interval}`}>
                            Discount %
                          </label>
                          <input
                            ...
                            className={`block w-full rounded-ctl border px-2 py-1.5 text-sm focus:outline-none focus:shadow-[0_0_0_3px_var(--accent-soft)] ${
                              discountError ? 'border-danger bg-danger-soft' : 'border-border focus:border-accent'
                            }`}
                            aria-label={`${INTERVAL_LABELS[ic.interval]} discount percentage`}
                          />
                          {discountError && (
                            <p className="text-xs text-danger mt-0.5">{discountError}</p>
                          )}
                        </div>
                        <div className="text-xs text-muted pt-4">
                          Effective: <span className="font-medium text-text mono">{formatCurrency(effectivePrice)}</span>
...
function GlobalStoragePricing({ onToast }: { onToast: (v: 'success' | 'error', msg: string) => void }) {
  ...
  return (
    <form onSubmit={handleSave} className="rounded-card border border-border bg-card p-4 space-y-3">
      <h2 className="text-base font-semibold text-text">Global Storage Pricing</h2>
      <p className="text-xs text-muted">Default add-on storage pricing applied when plans don't define their own tiers.</p>
...
function StoragePackagesContent({ onToast }: { onToast: (v: 'success' | 'error', msg: string) => void }) {
  ...
  const packageColumns: Column<StoragePackage>[] = [
    ...
    { key: 'storage_gb', header: 'Storage (GB)', sortable: true, render: (r) => <span className="mono">{r.storage_gb} GB</span> },
    {
      key: 'price_nzd_per_month', header: 'Price/mo (NZD)', sortable: true,
      render: (r) => <span className="mono">{formatCurrency(r.price_nzd_per_month)}</span>,
    },
    {
      key: 'description', header: 'Description',
      render: (r) => r.description
        ? <span className="text-sm text-muted truncate max-w-[200px] block">{r.description}</span>
        : <span className="text-xs text-muted-2">—</span>,
    },
    { key: 'sort_order', header: 'Sort Order', sortable: true },
    {
      key: 'is_active', header: 'Status',
      render: (r) => <Badge variant={r.is_active ? 'success' : 'warn'}>{r.is_active ? 'Active' : 'Inactive'}</Badge>,
    },
    {
      key: 'id', header: 'Actions',
      render: (r) => (
        <div className="flex gap-2">
          <Button size="sm" variant="ghost" onClick={() => { setEditPackage(r); setFormOpen(true) }}>Edit</Button>
          {r.is_active ? (
            <Button size="sm" variant="danger" onClick={() => handleDeactivate(r)}>Deactivate</Button>
          ) : (
            <Button size="sm" variant="ghost" onClick={() => handleReactivate(r)}>Reactivate</Button>
          )}
        </div>
      ),
    },
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold text-text">Storage Packages</h2>
    ...
    <Modal open={open} onClose={onClose} title={editCoupon ? 'Edit coupon' : 'Create coupon'} className="max-w-lg">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          {errors.discount_type && <p className="mt-1 text-xs text-danger">{errors.discount_type}</p>}
        </div>
...
export function SubscriptionPlans() {
  ...
  const columns: Column<Plan>[] = [
    { key: 'name', header: 'Plan', sortable: true },
    {
      ...
      render: (r) => <span className="mono">{formatCurrency(r.monthly_price_nzd)}</span>,
    },
    { key: 'user_seats', header: 'Seats', sortable: true },
    { key: 'storage_quota_gb', header: 'Storage', render: (r) => <span className="mono">{r.storage_quota_gb} GB</span> },
    { key: 'carjam_lookups_included', header: 'Carjam', render: (r) => r.carjam_lookups_included > 0 ? <span className="mono">{r.carjam_lookups_included}/mo</span> : <span className="text-xs text-muted-2">—</span> },
    { key: 'ppsr_lookups_included', header: 'PPSR', render: (r) => r.ppsr_lookups_included > 0 ? <span className="mono">{r.ppsr_lookups_included}/mo</span> : <span className="text-xs text-muted-2">—</span> },
    { key: 'ppsr_hidden_plate_lookups_included', header: 'PPSR Hidden', render: (r) => r.ppsr_hidden_plate_lookups_included > 0 ? <span className="mono">{r.ppsr_hidden_plate_lookups_included}/mo</span> : <span className="text-xs text-muted-2">—</span> },
    { key: 'sms_included', header: 'SMS', render: (r) => (r as Plan).sms_included ? <span className="text-ok">✓</span> : <span className="text-xs text-muted-2">—</span> },
    {
    ...
    },
    {
      ...
      render: (r) => (
        <Badge variant={r.is_public ? 'success' : 'warn'}>
    ...
    },
    {
      ...
      render: (r) => {
        ...
        return d > 0 ? <span className="text-sm">{d} {u}</span> : <span className="text-xs text-muted-2">None</span>
      },
    },
    {
      key: 'is_archived', header: 'Status',
      render: (r) => (
        <Badge variant={r.is_archived ? 'warn' : 'success'}>
          {r.is_archived ? 'Archived' : 'Active'}
        </Badge>
      ),
    },
    { key: 'created_at', header: 'Created', render: (r) => <span className="mono">{formatDate(r.created_at)}</span> },
    {
      key: 'id', header: 'Actions',
      render: (r) => (
        <div className="flex gap-2">
          {!r.is_archived && (
            <Button size="sm" variant="ghost" onClick={() => { setEditPlan(r); setFormOpen(true) }}>
              Edit
            </Button>
          )}
          {!r.is_archived && (
  ...
  const couponColumns: Column<Coupon>[] = [
    {
      ...
      render: (r) => <span className="mono text-sm uppercase">{r.code}</span>,
    },
    {
      key: 'description', header: 'Description',
      render: (r) => r.description ? <span className="text-sm text-muted truncate max-w-[200px] block">{r.description}</span> : <span className="text-xs text-muted-2">—</span>,
    },
    {
    ...
    },
    {
      ...
      render: (r) => {
        if (r.discount_type === 'percentage') return <span className="mono">{r.discount_value}%</span>
        if (r.discount_type === 'fixed_amount') return <span className="mono">{formatCurrency(r.discount_value)}</span>
        return <span className="mono">+{r.discount_value} days</span>
      },
    },
    {
      key: 'duration_months', header: 'Duration',
      render: (r) => r.duration_months ? <span>{r.duration_months} months</span> : r.discount_type === 'trial_extension' ? <span className="text-xs text-muted-2">—</span> : <span className="text-sm text-muted">Perpetual</span>,
    },
    {
      key: 'times_redeemed', header: 'Usage',
      render: (r) => <span className="mono">{r.times_redeemed} / {r.usage_limit ?? '∞'}</span>,
    },
    {
      key: 'is_active', header: 'Status',
      render: (r) => <Badge variant={r.is_active ? 'success' : 'warn'}>{r.is_active ? 'Active' : 'Inactive'}</Badge>,
    ...
    { key: 'created_at', header: 'Created', render: (r) => <span className="mono">{formatDate(r.created_at)}</span> },
    {
      ...
      render: (r) => (
        <div className="flex gap-2">
          <Button size="sm" variant="ghost" onClick={() => { setEditCoupon(r); setCouponFormOpen(true) }}>Edit</Button>
          {r.is_active ? (
            <Button size="sm" variant="danger" onClick={() => handleDeactivateCoupon(r)}>Deactivate</Button>
          ) : (
            <Button size="sm" variant="ghost" onClick={() => handleReactivateCoupon(r)}>Reactivate</Button>
          )}
        </div>
      ),
    },
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-semibold text-text">Subscription Management</h1>

      {/* Tab bar */}
      <div className="flex border-b border-border" role="tablist" aria-label="Subscription management tabs">
        <button
          type="button"
          role="tab"
      ...
      {activeMainTab === 'coupons' && (
        <>
          {couponsLoading ? (
            ...
            <>
              <div className="flex items-center justify-between">
                <h2 className="text-lg font-semibold text-text">Coupons</h2>
