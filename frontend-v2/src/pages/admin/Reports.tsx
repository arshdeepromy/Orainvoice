...
const fmt = (v: number) => `${(v ?? 0).toLocaleString('en-NZ', { minimumFractionDigits: 2 })}`
...
function MrrTab() {
  ...
  return (
    <div>
      <p className="text-sm text-muted mb-4">Platform Monthly Recurring Revenue with plan breakdown and trend.</p>
      ...
      {error && (
        <div className="mb-4 rounded-ctl border border-danger/30 bg-danger-soft px-4 py-3 text-sm text-danger" role="alert">
          {error}
        </div>
      )}
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-1 gap-4 mb-6">
            <div className="rounded-card border border-border bg-card p-4 shadow-card">
              ...
              <p className="text-2xl font-semibold text-text mono">{fmt(data.total_mrr)}</p>
            </div>
          </div>

          {/* Plan breakdown table */}
          <div className="rounded-card border border-border bg-card p-4 mb-6 shadow-card">
            <h3 className="text-sm font-medium text-text mb-3">MRR by Plan</h3>
            <DataTable
          ...
          <div className="rounded-card border border-border bg-card p-4 shadow-card">
            <h3 className="text-sm font-medium text-text mb-3">Month-over-Month Trend</h3>
            <SimpleBarChart
...
function OrganisationsTab() {
  ...
  const columns: Column<Record<string, unknown>>[] = [
    ...
    },
    {
      ...
      render: (r) => {
        ...
        const variant = v === 'active' ? 'success' : v === 'suspended' ? 'danger' : 'warn'
  ...
  return (
    <div>
      <p className="text-sm text-muted mb-4">Overview of all organisations on the platform.</p>
...
function CarjamCostTab() {
  ...
  return (
    <div>
      <p className="text-sm text-muted mb-4">Carjam API cost vs revenue recovered from organisations.</p>
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
            <div className="rounded-card border border-border bg-card p-4 shadow-card">
              ...
              <p className="text-2xl font-semibold text-danger mono">{fmt(data.total_cost)}</p>
            </div>
            <div className="rounded-card border border-border bg-card p-4 shadow-card">
              <p className="text-sm text-muted">Total Revenue</p>
              <p className="text-2xl font-semibold text-ok mono">{fmt(data.total_revenue)}</p>
            </div>
            <div className="rounded-card border border-border bg-card p-4 shadow-card">
              <p className="text-sm text-muted">Net</p>
              <p className={`text-2xl font-semibold mono ${data.net >= 0 ? 'text-ok' : 'text-danger'}`}>
                {fmt(data.net)}
              </p>
            </div>
          </div>
          ...
          <div className="rounded-card border border-border bg-card p-4 shadow-card">
            <h3 className="text-sm font-medium text-text mb-3">Monthly Cost vs Revenue</h3>
            <SimpleBarChart
              title="Monthly Carjam cost vs revenue"
              items={data.monthly_breakdown.flatMap((m) => [
                { label: `${m.month} Cost`, value: m.cost, colour: 'bg-danger' },
                { label: `${m.month} Rev`, value: m.revenue, colour: 'bg-ok' },
              ])}
              formatValue={fmt}
            />
          </div>
...
function VehicleDbTab() {
  ...
  return (
    <div>
      <p className="text-sm text-muted mb-4">Global Vehicle Database statistics.</p>
      ...
      {!loading && data && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="rounded-card border border-border bg-card p-4 shadow-card">
            ...
            <p className="text-2xl font-semibold text-text mono">{(data.total_records ?? 0).toLocaleString()}</p>
          </div>
          <div className="rounded-card border border-border bg-card p-4 shadow-card">
            <p className="text-sm text-muted">Cache Hit Rate</p>
            <p className="text-2xl font-semibold text-text mono">{data.cache_hit_rate}%</p>
          </div>
          <div className="rounded-card border border-border bg-card p-4 shadow-card">
            <p className="text-sm text-muted">Total Lookups</p>
            <p className="text-2xl font-semibold text-text mono">{(data.total_lookups ?? 0).toLocaleString()}</p>
          </div>
        </div>
      )}
    </div>
...
function ChurnTab() {
  ...
  const columns: Column<Record<string, unknown>>[] = [
    ...
    { key: 'plan', header: 'Plan', sortable: true },
    {
      ...
      render: (r) => {
        ...
        return <Badge variant={v === 'cancelled' ? 'danger' : 'warn'}>{v}</Badge>
      },
    },
  ...
  return (
    <div>
      <p className="text-sm text-muted mb-4">Organisations that cancelled or were suspended.</p>
...
export function Reports() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-semibold text-text mb-6">Admin Reports</h1>
