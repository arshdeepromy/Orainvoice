...
function ActionsMenu({
  ...
  return (
    <div className="relative" ref={ref}>
      <Button size="sm" variant="ghost" onClick={() => setOpen(!open)} aria-label="User actions">
        {loading ? <Spinner /> : '⋯'}
      </Button>
      {open && (
        <div className="absolute right-0 z-20 mt-1 w-52 bg-card border border-border rounded-lg shadow-pop py-1">
...
export function UserManagement() {
  ...
  const columns: Column<UserRow>[] = [
    ...
    ...(tab === 'org' ? [{
      ...
      header: 'Organisation',
      render: (row: UserRow) => row.org_name ?? <span className="text-muted-2">Platform</span>,
    }] : []),
    {
      key: 'is_active',
      header: 'Status',
    ...
    },
    {
      ...
      header: 'Verified',
      render: (row) => (
        <Badge variant={row.is_email_verified ? 'success' : 'warn'}>
          {row.is_email_verified ? 'Yes' : 'No'}
        </Badge>
      ),
    },
    { key: 'last_login_at', header: 'Last login', render: (row) => <span className="mono">{formatDate(row.last_login_at)}</span> },
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm text-muted">{total} users total</span>
        </div>
      </div>
      ...
      {total > 25 && (
        <div className="flex justify-center gap-2 mt-4">
          ...
          <span className="text-sm text-muted self-center">Page {page} of {Math.ceil(total / 25)}</span>
      ...
      {editModalOpen && (
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/50 overflow-y-auto py-8">
          <div className="bg-canvas rounded-card shadow-pop w-full max-w-2xl mx-4 relative">
            <div className="sticky top-0 bg-card rounded-t-card border-b border-border px-6 py-4 flex items-center justify-between z-10">
              <h2 className="text-lg font-semibold text-text">Edit Profile</h2>
              <button
                type="button"
                onClick={() => setEditModalOpen(false)}
                className="rounded-md p-1.5 text-muted-2 hover:text-text hover:bg-canvas"
                aria-label="Close"
              >
      ...
      {createModalOpen && (
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/50 overflow-y-auto py-8">
          <div className="bg-card rounded-card shadow-pop w-full max-w-md mx-4 relative">
            <div className="border-b border-border px-6 py-4 flex items-center justify-between">
              <h2 className="text-lg font-semibold text-text">Create Global Admin</h2>
              <button
                type="button"
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-card rounded-card shadow-pop w-full max-w-sm mx-4 p-6">
            <h2 className="text-lg font-semibold text-text mb-2">Delete User Permanently</h2>
