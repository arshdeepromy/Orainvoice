...
function jobStatusBadge(status: string): { label: string; className: string } {
  const map: Record<string, { label: string; className: string }> = {
    ...
    schema_migrating: { label: 'Schema Migrating', className: 'bg-accent-soft text-accent' },
    copying_data: { label: 'Copying Data', className: 'bg-accent-soft text-accent' },
    draining_queue: { label: 'Draining Queue', className: 'bg-accent-soft text-accent' },
    integrity_check: { label: 'Integrity Check', className: 'bg-warn-soft text-warn' },
    ready_for_cutover: { label: 'Ready for Cutover', className: 'bg-ok-soft text-ok' },
    cutting_over: { label: 'Cutting Over', className: 'bg-purple-soft text-purple' },
    completed: { label: 'Completed', className: 'bg-ok-soft text-ok' },
    failed: { label: 'Failed', className: 'bg-danger-soft text-danger' },
...
export function ConnectionForm({ onMigrationStarted, disabled }: ConnectionFormProps) {
  ...
  return (
    <div className="rounded-card border border-border bg-card p-6 shadow-card" role="region" aria-label="Connection configuration">
      <h3 className="text-lg font-semibold text-text mb-4">Target Database Connection</h3>

      <div className="space-y-4">
        <div>
          <label htmlFor="conn-string" className="block text-sm font-medium text-text mb-1">
            Connection String
          </label>
          <input
            id="conn-string"
            ...
            disabled={disabled}
            className="w-full rounded-ctl border border-border bg-card px-3 py-2 text-sm text-text mono focus:border-accent focus:outline-none focus:shadow-[0_0_0_3px_var(--accent-soft)] disabled:bg-canvas disabled:text-muted"
        ...
        <div className="flex gap-4">
          <div className="flex-1">
            <label htmlFor="ssl-mode" className="block text-sm font-medium text-text mb-1">
              SSL Mode
            </label>
            <select
              id="ssl-mode"
              ...
              disabled={disabled}
              className="w-full rounded-ctl border border-border bg-card px-3 py-2 text-sm text-text focus:border-accent focus:outline-none focus:shadow-[0_0_0_3px_var(--accent-soft)] min-h-[44px] disabled:bg-canvas"
              aria-label="SSL mode"
            >
          ...
          <div className="flex-1">
            <label htmlFor="batch-size" className="block text-sm font-medium text-text mb-1">
              Batch Size
            </label>
            <input
              id="batch-size"
              ...
              disabled={disabled}
              className="w-full rounded-ctl border border-border bg-card px-3 py-2 text-sm text-text focus:border-accent focus:outline-none focus:shadow-[0_0_0_3px_var(--accent-soft)] disabled:bg-canvas"
        ...
        <button
          ...
          className="min-h-[44px] rounded-ctl bg-accent px-4 py-2 text-sm font-medium text-white hover:bg-accent-press disabled:bg-border-strong disabled:cursor-not-allowed"
        ...
        {validation && validation.valid && (
          <div className="rounded-ctl bg-ok-soft p-4 space-y-3">
            <p className="text-sm text-ok font-medium">Connection validated successfully</p>
            ...
            {validation.has_existing_tables && (
              <div className="rounded-ctl bg-warn-soft border border-warn/30 p-3">
                <label className="flex items-center gap-2 text-sm text-warn cursor-pointer min-h-[44px]">
                  <input
                    type="checkbox"
            ...
            <button
              ...
              className="min-h-[44px] rounded-ctl bg-ok px-4 py-2 text-sm font-medium text-white hover:brightness-95 disabled:bg-border-strong disabled:cursor-not-allowed"
...
export function MigrationProgress({ jobId, onStatusUpdate, onCancel }: MigrationProgressProps) {
  ...
  if (!status) {
    return (
      <div className="rounded-card border border-border bg-card p-6 shadow-card" role="region" aria-label="Migration progress">
        <p className="text-sm text-muted">Loading migration status…</p>
      </div>
    )
  }
  ...
  return (
    <div className="rounded-card border border-border bg-card p-6 space-y-4 shadow-card" role="region" aria-label="Migration progress">
      ...
      <div>
        <div className="flex justify-between text-sm text-muted mb-1">
          <span className="mono">{(status.rows_processed ?? 0).toLocaleString()} / {(status.rows_total ?? 0).toLocaleString()} rows</span>
          <span className="mono">{(pct ?? 0).toFixed(1)}%</span>
        </div>
        <div
      ...
      <div className="flex gap-6 text-sm text-muted">
        ...
        <span>Dual-write queue: <span className="mono">{status.dual_write_queue_depth}</span></span>
        {status.current_table && <span>Current table: <span className="mono">{status.current_table}</span></span>}
      </div>
          ...
          <table className="min-w-full text-sm" aria-label="Table migration breakdown">
            <thead>
              <tr className="border-b border-border">
                <th className="text-left py-2 pr-4 font-medium text-muted-2 mono text-[10.5px] uppercase tracking-[0.08em]">Table</th>
                <th className="text-right py-2 px-4 font-medium text-muted-2 mono text-[10.5px] uppercase tracking-[0.08em]">Source</th>
                <th className="text-right py-2 px-4 font-medium text-muted-2 mono text-[10.5px] uppercase tracking-[0.08em]">Migrated</th>
                <th className="text-left py-2 pl-4 font-medium text-muted-2 mono text-[10.5px] uppercase tracking-[0.08em]">Status</th>
              </tr>
            </thead>
            <tbody>
              {status.tables.map((t) => {
                ...
                return (
                  <tr key={t.table_name} className="border-b border-border last:border-b-0">
                    <td className="py-2 pr-4 text-text mono">{t.table_name}</td>
                    <td className="py-2 px-4 text-right text-muted mono">{(t.source_count ?? 0).toLocaleString()}</td>
                    <td className="py-2 px-4 text-right text-muted mono">{(t.migrated_count ?? 0).toLocaleString()}</td>
      ...
      {CANCELLABLE_STATUSES.includes(status.status) && (
        <button
          ...
          disabled={cancelling}
          className="min-h-[44px] rounded-ctl bg-danger px-4 py-2 text-sm font-medium text-white hover:brightness-95 disabled:bg-border-strong disabled:cursor-not-allowed"
...
export function LiveIntegrityReport({ result }: IntegrityReportProps) {
  ...
  return (
    <div className="rounded-card border border-border bg-card p-6 space-y-6 shadow-card" role="region" aria-label="Integrity check report">
      <div className="flex items-center gap-3">
        <h3 className="text-lg font-semibold text-text">Integrity Check Results</h3>
        <span
          className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${
            result.passed ? 'bg-ok-soft text-ok' : 'bg-danger-soft text-danger'
          }`}
        >
      ...
      {rowCountEntries.length > 0 && (
        <div>
          <h4 className="text-sm font-medium text-text mb-2">Row Count Comparison</h4>
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm" aria-label="Row count comparison">
              <thead>
                <tr className="border-b border-border">
                  <th className="text-left py-2 pr-4 font-medium text-muted-2 mono text-[10.5px] uppercase tracking-[0.08em]">Table</th>
                  <th className="text-right py-2 px-4 font-medium text-muted-2 mono text-[10.5px] uppercase tracking-[0.08em]">Source</th>
                  <th className="text-right py-2 px-4 font-medium text-muted-2 mono text-[10.5px] uppercase tracking-[0.08em]">Target</th>
                  <th className="text-center py-2 pl-4 font-medium text-muted-2 mono text-[10.5px] uppercase tracking-[0.08em]">Match</th>
                </tr>
              </thead>
              <tbody>
                {rowCountEntries.map(([table, cmp]) => (
                  <tr key={table} className="border-b border-border last:border-b-0">
                    <td className="py-2 pr-4 text-text mono">{table}</td>
                    <td className="py-2 px-4 text-right text-muted mono">{(cmp.source ?? 0).toLocaleString()}</td>
                    <td className="py-2 px-4 text-right text-muted mono">{(cmp.target ?? 0).toLocaleString()}</td>
                    <td className="py-2 pl-4 text-center">
                      <span className={cmp.match ? 'text-ok' : 'text-danger'}>
                        {cmp.match ? '✓' : '✗'}
                      </span>
                    </td>
                  </tr>
      ...
      {financialEntries.length > 0 && (
        <div>
          <h4 className="text-sm font-medium text-text mb-2">Financial Total Comparison</h4>
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm" aria-label="Financial total comparison">
              <thead>
                <tr className="border-b border-border">
                  <th className="text-left py-2 pr-4 font-medium text-muted-2 mono text-[10.5px] uppercase tracking-[0.08em]">Metric</th>
                  <th className="text-right py-2 px-4 font-medium text-muted-2 mono text-[10.5px] uppercase tracking-[0.08em]">Source</th>
                  <th className="text-right py-2 px-4 font-medium text-muted-2 mono text-[10.5px] uppercase tracking-[0.08em]">Target</th>
                  <th className="text-center py-2 pl-4 font-medium text-muted-2 mono text-[10.5px] uppercase tracking-[0.08em]">Match</th>
                </tr>
              ...
              <tbody>
                {financialEntries.map(([metric, cmp]) => (
                  <tr key={metric} className="border-b border-border last:border-b-0">
                    <td className="py-2 pr-4 text-text">{metric.replace(/_/g, ' ')}</td>
                    <td className="py-2 px-4 text-right text-muted mono">${(cmp.source_total ?? 0).toFixed(2)}</td>
                    <td className="py-2 px-4 text-right text-muted mono">${(cmp.target_total ?? 0).toFixed(2)}</td>
                    <td className="py-2 pl-4 text-center">
                      <span className={cmp.match ? 'text-ok' : 'text-danger'}>
                        {cmp.match ? '✓' : '✗'}
                      </span>
      ...
      {result.fk_errors.length > 0 && (
        <div>
          <h4 className="text-sm font-medium text-text mb-2">
            Foreign Key Errors
            <span className="ml-2 inline-flex items-center rounded-full bg-danger-soft px-2 py-0.5 text-xs font-medium text-danger">
              {result.fk_errors.length}
            </span>
          </h4>
          <ul className="space-y-1 text-sm text-danger" role="list" aria-label="Foreign key errors">
            {result.fk_errors.map((err, i) => (
              <li key={i} className="rounded bg-danger-soft px-3 py-1">{err}</li>
            ))}
          </ul>
        </div>
      )}
      ...
      {sequenceEntries.length > 0 && (
        <div>
          <h4 className="text-sm font-medium text-text mb-2">Sequence Checks</h4>
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm" aria-label="Sequence checks">
              <thead>
                <tr className="border-b border-border">
                  <th className="text-left py-2 pr-4 font-medium text-muted-2 mono text-[10.5px] uppercase tracking-[0.08em]">Sequence</th>
                  <th className="text-right py-2 px-4 font-medium text-muted-2 mono text-[10.5px] uppercase tracking-[0.08em]">Source</th>
                  <th className="text-right py-2 px-4 font-medium text-muted-2 mono text-[10.5px] uppercase tracking-[0.08em]">Target</th>
                  <th className="text-center py-2 pl-4 font-medium text-muted-2 mono text-[10.5px] uppercase tracking-[0.08em]">Valid</th>
                </tr>
              </thead>
              <tbody>
                {sequenceEntries.map(([seq, cmp]) => (
                  <tr key={seq} className="border-b border-border last:border-b-0">
                    <td className="py-2 pr-4 text-text mono">{seq}</td>
                    <td className="py-2 px-4 text-right text-muted mono">{(cmp.source_value ?? 0).toLocaleString()}</td>
                    <td className="py-2 px-4 text-right text-muted mono">{(cmp.target_value ?? 0).toLocaleString()}</td>
                    <td className="py-2 pl-4 text-center">
                      <span className={cmp.valid ? 'text-ok' : 'text-danger'}>
                        {cmp.valid ? '✓' : '✗'}
                      </span>
                    </td>
                  </tr>
...
export function CutoverPanel({ jobId, integrityPassed, onCutoverComplete, onError }: CutoverPanelProps) {
  ...
  return (
    <div className="rounded-card border border-border bg-card p-6 space-y-4 shadow-card" role="region" aria-label="Cutover panel">
      <h3 className="text-lg font-semibold text-text">Database Cutover</h3>

      {result === 'success' && (
        <div className="rounded-ctl bg-ok-soft p-3 text-sm text-ok" role="alert">
      ...
      {result === null && (
        <>
          ...
          <button
            ...
            className="min-h-[44px] rounded-ctl bg-purple px-4 py-2 text-sm font-medium text-white hover:brightness-95 disabled:bg-border-strong disabled:cursor-not-allowed"
      ...
      {/* Confirmation modal */}
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/50" role="dialog" aria-modal="true" aria-label="Confirm cutover">
          <div className="w-full max-w-md rounded-card bg-card p-6 shadow-pop space-y-4">
            <h4 className="text-lg font-semibold text-text">Confirm Cutover</h4>
            <p className="text-sm text-muted">
...
export function RollbackPanel({ jobId, cutoverAt, onRollbackComplete, onError }: RollbackPanelProps) {
  ...
  return (
    <div className="rounded-card border border-border bg-card p-6 space-y-4 shadow-card" role="region" aria-label="Rollback panel">
      <h3 className="text-lg font-semibold text-text">Rollback</h3>

      <div className="text-sm text-muted">
        {expired ? (
          <div className="rounded-ctl bg-warn-soft border border-warn/30 p-3 text-warn">
      ...
      {!expired && (
        <>
          <div>
            <label htmlFor="rollback-reason" className="block text-sm font-medium text-text mb-1">
              Reason for rollback
            </label>
            <input
              id="rollback-reason"
          ...
          <button
            ...
            className="min-h-[44px] rounded-ctl bg-warn px-4 py-2 text-sm font-medium text-white hover:brightness-95 disabled:bg-border-strong disabled:cursor-not-allowed"
...
export function MigrationHistory({ onSelectJob }: MigrationHistoryProps) {
  ...
  if (loading) {
    return (
      <div className="rounded-card border border-border bg-card p-6 shadow-card" role="region" aria-label="Migration history">
        <p className="text-sm text-muted">Loading migration history…</p>
      </div>
    )
  }

  return (
    <div className="rounded-card border border-border bg-card p-6 space-y-4 shadow-card" role="region" aria-label="Migration history">
      <h3 className="text-lg font-semibold text-text">Migration History</h3>

      {jobs.length === 0 ? (
        <p className="text-sm text-muted">No past migrations found.</p>
      ) : (
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm" aria-label="Past migration jobs">
            <thead>
              <tr className="border-b border-border">
                <th className="text-left py-2 pr-4 font-medium text-muted-2 mono text-[10.5px] uppercase tracking-[0.08em]">Status</th>
                <th className="text-left py-2 px-4 font-medium text-muted-2 mono text-[10.5px] uppercase tracking-[0.08em]">Started</th>
                <th className="text-left py-2 px-4 font-medium text-muted-2 mono text-[10.5px] uppercase tracking-[0.08em]">Completed</th>
                <th className="text-right py-2 px-4 font-medium text-muted-2 mono text-[10.5px] uppercase tracking-[0.08em]">Records</th>
                <th className="text-left py-2 px-4 font-medium text-muted-2 mono text-[10.5px] uppercase tracking-[0.08em]">Source</th>
                <th className="text-left py-2 pl-4 font-medium text-muted-2 mono text-[10.5px] uppercase tracking-[0.08em]">Target</th>
              </tr>
            </thead>
            <tbody>
              {jobs.map((job) => {
                ...
                return (
                  <>
                    <tr
                      ...
                      className="border-b border-border last:border-b-0 cursor-pointer hover:bg-canvas min-h-[44px]"
                      role="button"
                      tabIndex={0}
                    ...
                    >
                      ...
                      <td className="py-3 px-4 text-muted mono">{formatDateTime(job.started_at)}</td>
                      <td className="py-3 px-4 text-muted mono">{formatDateTime(job.completed_at)}</td>
                      <td className="py-3 px-4 text-right text-muted mono">{(job.rows_total ?? 0).toLocaleString()}</td>
                      <td className="py-3 px-4 text-muted mono">{job.source_host}</td>
                      <td className="py-3 pl-4 text-muted mono">{job.target_host}</td>
                    </tr>
                    {isExpanded && (
                      <tr key={`${job.job_id}-detail`}>
                        <td colSpan={6} className="p-4 bg-canvas">
                          {detailLoading ? (
                            <p className="text-sm text-muted">Loading details…</p>
                          ) : detail ? (
                            <div className="space-y-4">
                              {detail.error_message && (
                                <div className="rounded-ctl bg-danger-soft p-3 text-sm text-danger">
                                  Error: {detail.error_message}
                                </div>
                              )}
                              {detail.tables.length > 0 && (
                                <div>
                                  <h4 className="text-sm font-medium text-text mb-2">Tables</h4>
                                  <div className="grid grid-cols-2 gap-2 text-xs">
                                    {detail.tables.map((t) => (
                                      <div key={t.table_name} className="flex justify-between bg-card rounded px-2 py-1 border border-border">
                                        <span className="mono">{t.table_name}</span>
                                        <span className="text-muted mono">{t.migrated_count}/{t.source_count}</span>
                                      </div>
                                    ))}
                                  </div>
                                </div>
...
export function LiveMigrationTool() {
  ...
  return (
    <div className="space-y-6 max-w-5xl" role="main" aria-label="Live Database Migration Tool">
      <div>
        <h2 className="text-2xl font-bold text-text">Live Database Migration</h2>
      ...
      {/* Error banner */}
      {error && (
        <div className="rounded-ctl bg-danger-soft border border-danger/30 p-4 flex items-start gap-3" role="alert">
          <span className="text-danger text-sm flex-1">{error}</span>
          <button
            onClick={() => setError(null)}
            className="min-h-[44px] min-w-[44px] flex items-center justify-center text-danger/70 hover:text-danger"
