      ...
      <div key={f.key}>
        {f.type === 'select' && f.options ? (
          <div className="flex flex-col gap-1">
            ...
            <select
              className="rounded-ctl border border-border bg-card px-3 py-2 text-text focus:border-accent focus:outline-none focus:shadow-[0_0_0_3px_var(--accent-soft)]"
              value={values[f.key] ?? ''}
          ...
          <div className="flex flex-col gap-1">
            ...
            {f.helperText && (
              <p className="text-sm text-muted ml-6">{f.helperText}</p>
            )}
          </div>
  ...
  return (
    <div className="max-w-lg space-y-5">
      <div className="flex items-center gap-3 mb-2">
        <h2 className="text-lg font-semibold text-text">{INTEGRATION_LABELS[name]}</h2>
      ...
      {updatedAt && (
        <p className="text-sm text-muted">
          Last updated: <span className="mono">{new Date(updatedAt).toLocaleDateString('en-NZ', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</span>
        </p>
      )}
      ...
      {hasGroups ? (
        <>
          ...
          {/* API Keys section */}
          <div className="rounded-card border border-border p-5 space-y-4 shadow-card">
            <h3 className="text-md font-semibold text-text">API Keys</h3>
            <p className="text-xs text-muted">Your Stripe publishable and secret keys for processing payments.</p>
            ...
            <div className="flex gap-3 pt-1">
              ...
              <Button type="button" variant="ghost" onClick={handleTestKeys} loading={testingKeys}>
                Test API keys
              </Button>
            </div>
          </div>
          ...
          <form onSubmit={handleSave} className="rounded-card border border-border p-5 space-y-4 shadow-card">
            <h3 className="text-md font-semibold text-text">Platform &amp; Webhooks</h3>
            <p className="text-xs text-muted">Stripe Connect platform account and webhook configuration.</p>
            {fields.filter((f) => f.group === 'connect').map(renderField)}

            {/* Connect Setup Guide */}
            {name === 'stripe' && (
              <div className="mt-4 rounded-card bg-accent-soft border border-accent/30 p-4 space-y-3">
                <h4 className="text-sm font-medium text-accent">Stripe Connect Setup Guide</h4>
                <div className="text-sm text-accent space-y-2">
                  ...
                  <ol className="list-decimal list-inside space-y-1 ml-2">
                    ...
                    <li>Under <strong>OAuth settings</strong>, copy the <strong>client_id</strong> (starts with <code className="bg-accent-soft px-1 rounded mono">ca_</code>) and paste it above</li>
                    <li>Add this <strong>Redirect URI</strong> to your Stripe Connect OAuth settings:</li>
                  </ol>
                  <div className="bg-card rounded border border-accent/40 px-3 py-2 mono text-xs break-all select-all">
                    {window.location.origin}/settings/online-payments
                  </div>
                  <p className="text-xs text-accent">Click the URI above to select it, then paste it into Stripe's redirect URI field.</p>
                </div>
              </div>
            )}
      ...
      {name === 'carjam' && (
        <div className="mt-8 pt-6 border-t border-border">
          <h3 className="text-md font-semibold text-text mb-4">Test Vehicle Lookup</h3>
                  ...
                  {lookupResult.success && lookupResult.data && (
                    <div className="mt-3 p-3 bg-card rounded border border-border">
                      <div className="grid grid-cols-2 gap-2 text-sm">
                        ...
                        <div><span className="font-medium">Source:</span> <span className={lookupResult.source === 'cache' ? 'text-ok' : 'text-accent'}>{lookupResult.source === 'cache' ? 'Cached' : 'Carjam API'}</span></div>
      ...
      {name === 'carjam' && (
        <div className="mt-8 pt-6 border-t border-border">
          <h3 className="text-md font-semibold text-text mb-4">Test ABCD Lookup (Lower Cost)</h3>
          <p className="text-sm text-muted mb-4">
                  ...
                  {abcdResult.success && abcdResult.data && (
                    <div className="mt-3 p-3 bg-card rounded border border-border">
                      <div className="mb-2 text-xs text-muted">
                        Source: <span className="font-medium text-accent">Carjam ABCD API</span>
                        {abcdResult.mvr_used !== undefined && (
                          <span className="ml-2">
                            | MVR: <span className={abcdResult.mvr_used ? 'text-warn' : 'text-ok'}>
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          <Button variant="ghost" size="sm" onClick={handleBackup} loading={backing}>
            Backup Settings
          </Button>
          <Button variant="ghost" size="sm" onClick={handleRestore} loading={restoring}>
            Restore Settings
          </Button>
        </div>
      </div>
