  ...
  const indicators = [
    {
      ...
      icon: billingOk ? '✓' : '✗',
      colour: billingOk ? 'text-ok' : 'text-danger',
    },
    {
      label: storageLabel,
      icon: storageIcon,
    ...
    },
    {
      ...
      icon: seatsOk ? '✓' : '⚠',
      colour: seatsOk ? 'text-ok' : 'text-warn',
    },
    {
      label: mfaOk ? 'MFA OK' : 'MFA Low',
      icon: mfaOk ? '✓' : '⚠',
      colour: mfaOk ? 'text-ok' : 'text-warn',
    },
    {
      label: statusOk ? 'Status OK' : 'Status Issue',
      icon: statusOk ? '✓' : '✗',
      colour: statusOk ? 'text-ok' : 'text-danger',
    },
  ]

  return (
    <section className="bg-card rounded-card border border-border p-4">
      <h2 className="text-sm font-semibold text-text mb-2">Health Indicators</h2>
...
const STATUS_VARIANT_MAP: Record<string, 'success' | 'info' | 'warn' | 'danger' | 'neutral'> = {
  active: 'success',
  trial: 'info',
...
function OverviewCard({ overview }: { overview: OrgDetailOverview }) {
  ...
  return (
    <section className="bg-card rounded-card border border-border p-6">
      <h2 className="text-lg font-semibold text-text mb-4">Overview</h2>

      {/* Name + Status + Plan */}
      <div className="flex flex-wrap items-center gap-3 mb-4">
        <span
          className="text-base font-semibold text-text truncate max-w-xs"
          title={name}
        >
          {name}
        </span>
        ...
        <span
          className="text-sm text-muted truncate max-w-xs"
          title={planName}
        >
          {planName}
        </span>
      ...
      <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3 text-sm">
        <div>
          <dt className="text-muted">Signup Date</dt>
          <dd className="text-text truncate" title={signupDate}>{signupDate}</dd>
        </div>
        <div>
          <dt className="text-muted">Business Type</dt>
          <dd className="text-text truncate" title={businessType}>{businessType}</dd>
        </div>
        <div>
          <dt className="text-muted">Trade Category</dt>
          <dd className="text-text truncate" title={tradeCategoryName}>{tradeCategoryName}</dd>
        </div>
        <div>
          <dt className="text-muted">Billing Interval</dt>
          <dd className="text-text truncate" title={billingInterval}>{billingInterval}</dd>
        </div>
        {trialEndsAt && (
          <div>
            <dt className="text-muted">Trial Ends</dt>
            <dd className="text-text truncate" title={formatTrialEnd(trialEndsAt, overview?.locale)}>
        ...
        <div>
          ...
          <dd className="text-text truncate" title={timezone}>{timezone}</dd>
        </div>
        <div>
          <dt className="text-muted">Locale</dt>
          <dd className="text-text truncate" title={locale}>{locale}</dd>
        </div>
      </dl>
    </section>
  )
    ...
    <section className="bg-card rounded-card border border-border p-6">
      ...
      <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3 text-sm mb-4">
        <div>
          ...
          <dd className="text-text truncate" title={planName}>{planName}</dd>
        </div>
        <div>
          <dt className="text-muted">Monthly Price</dt>
          <dd className="text-text text-right mono" title={formatCurrency(monthlyPrice)}>
        ...
        <div>
          ...
          <dd className="text-text truncate mono" title={nextBillingDate}>{nextBillingDate}</dd>
        </div>
      </dl>

      {/* Payment Method */}
      <div className="mb-4">
        <h3 className="text-sm font-medium text-text mb-1">Payment Method</h3>
        {pm ? (
          <p className="text-sm text-text truncate" title={`${pm?.brand ?? ''} •••• ${pm?.last4 ?? ''} — Exp ${String(pm?.exp_month ?? '').padStart(2, '0')}/${pm?.exp_year ?? ''}`}>
      ...
      {/* Coupons */}
      <div className="mb-4">
        <h3 className="text-sm font-medium text-text mb-1">Active Coupons</h3>
        {coupons.length > 0 ? (
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {coupons.map((coupon, idx) => {
                  ...
                  return (
                    <tr key={`${code}-${idx}`} className="border-b border-border">
                      <td className="py-1 pr-4 truncate max-w-[120px] mono" title={code}>{code}</td>
                      <td className="py-1 pr-4 truncate" title={discountType}>{discountType}</td>
                      <td className="py-1 pr-4 text-right mono" title={valueDisplay}>{valueDisplay}</td>
                      <td className="py-1 text-right mono" title={`${remaining} months`}>
          ...
          <p className="text-sm text-muted">No active coupons</p>
        )}
      </div>

      {/* Storage Add-on */}
      <div className="mb-4">
        <h3 className="text-sm font-medium text-text mb-1">Storage Add-on</h3>
        {storageAddon ? (
          <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-1 text-sm">
            <div>
              <dt className="text-muted">Package</dt>
              <dd className="text-text truncate" title={storageAddon?.package_name ?? 'Custom'}>
            ...
            <div>
              ...
              <dd className="text-text text-right mono">{storageAddon?.quantity_gb ?? 0} GB</dd>
            </div>
            <div>
              <dt className="text-muted">Price</dt>
              <dd className="text-text text-right mono">
                {formatCurrency(storageAddon?.price_nzd_per_month)}/mo
              </dd>
            </div>
          </dl>
        ) : (
          <p className="text-sm text-muted">No storage add-on</p>
        )}
      </div>

      {/* Billing Receipt Summary */}
      <div>
        <h3 className="text-sm font-medium text-text mb-1">Receipts (Last 90 Days)</h3>
        <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-1 text-sm">
          <div>
            <dt className="text-muted">Successful</dt>
            <dd className="text-text text-right mono">{(receiptsSuccess ?? 0).toLocaleString()}</dd>
          </div>
          <div>
            <dt className="text-muted">Failed</dt>
            <dd className={`text-right mono ${receiptsFailed > 0 ? 'text-danger font-medium' : 'text-text'}`}>
              {(receiptsFailed ?? 0).toLocaleString()}
            </dd>
          </div>
          {receiptsFailed > 0 && lastFailureDate && (
            <div className="sm:col-span-2">
              <dt className="text-muted">Most Recent Failure</dt>
              <dd className="text-danger truncate mono" title={formatDate(lastFailureDate)}>
...
function ProgressBar({
  ...
  return (
    <div className="flex items-center gap-3">
      <div className="flex-1 h-2.5 bg-border rounded-full overflow-hidden" aria-hidden="true">
        <div
          className={`h-full rounded-full ${barColour}`}
          style={{ width: `${pct}%` }}
        />
      </div>
      <span className="text-sm text-text whitespace-nowrap text-right min-w-[80px] mono">
        {label}
      </span>
    </div>
  )
...
function UsageMetricsCard({ usage }: { usage: OrgDetailUsage }) {
  ...
  return (
    <section className="bg-card rounded-card border border-border p-6">
      <h2 className="text-lg font-semibold text-text mb-4">Usage Metrics</h2>

      {/* Aggregate counts */}
      <dl className="grid grid-cols-2 sm:grid-cols-4 gap-x-6 gap-y-3 text-sm mb-5">
        <div>
          <dt className="text-muted">Invoices</dt>
          <dd className="text-text text-right font-medium mono">{(invoiceCount ?? 0).toLocaleString()}</dd>
        </div>
        <div>
          <dt className="text-muted">Quotes</dt>
          <dd className="text-text text-right font-medium mono">{(quoteCount ?? 0).toLocaleString()}</dd>
        </div>
        <div>
          <dt className="text-muted">Customers</dt>
          <dd className="text-text text-right font-medium mono">{(customerCount ?? 0).toLocaleString()}</dd>
        </div>
        <div>
          <dt className="text-muted">Vehicles</dt>
          <dd className="text-text text-right font-medium mono">{(vehicleCount ?? 0).toLocaleString()}</dd>
        </div>
      </dl>
      ...
      <div className="space-y-4">
        <div>
          <h3 className="text-sm font-medium text-text mb-1">Storage</h3>
          <ProgressBar
        ...
        <div>
          <h3 className="text-sm font-medium text-text mb-1">Carjam Lookups (This Month)</h3>
          <ProgressBar
            used={carjamUsed}
        ...
        <div>
          <h3 className="text-sm font-medium text-text mb-1">SMS Sent (This Month)</h3>
          <ProgressBar
            used={smsSent}
            total={smsQuota}
            label={smsLabel}
  ...
  return (
    <section className="bg-card rounded-card border border-border p-6">
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        {atLimit && (
          <span className="text-warn text-sm font-medium flex items-center gap-1">
            ...
            <tbody>
              {userList.map((user) => {
                ...
                return (
                  <tr
                    key={user?.id ?? email}
                    className={`border-b border-border ${stale ? 'bg-warn-soft' : ''}`}
                  >
                    <td
                      className={`py-2 pr-4 truncate max-w-[160px] ${stale ? 'text-warn' : 'text-text'}`}
                      title={name}
                    >
                      {name}
                    </td>
                    <td
                      className={`py-2 pr-4 truncate max-w-[200px] ${stale ? 'text-warn' : 'text-text'}`}
                      title={email}
                    >
                      {email}
                    </td>
                    <td
                      className={`py-2 pr-4 truncate ${stale ? 'text-warn' : 'text-text'}`}
                      title={roleDisplay}
                    >
                      {roleDisplay}
                    </td>
                    <td
                      className={`py-2 pr-4 whitespace-nowrap mono ${stale ? 'text-warn' : 'text-text'}`}
                    ...
                    <td className="py-2 whitespace-nowrap">
                      {mfaEnabled ? (
                        <span className="text-ok font-medium">Enabled</span>
                      ) : (
...
function SecurityAuditCard({ security }: { security: OrgDetailSecurity }) {
  ...
  return (
    <section className="bg-card rounded-card border border-border p-6 lg:col-span-2">
      <h2 className="text-lg font-semibold text-text mb-4">Security &amp; Audit</h2>

      {/* MFA Enrollment Summary */}
      <div className="mb-4">
        <h3 className="text-sm font-medium text-text mb-1">MFA Enrollment</h3>
      ...
      <div className="mb-5">
        <h3 className="text-sm font-medium text-text mb-1">Failed Payments (Last 90 Days)</h3>
        <p className={`text-sm mono ${failedPayments > 0 ? 'text-danger font-medium' : 'text-text'}`}>
      ...
      <div className="mb-5">
        <h3 className="text-sm font-medium text-text mb-2">Login Attempts (Last 30 Days)</h3>
        {loginAttempts.length > 0 ? (
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {loginAttempts.map((attempt, idx) => {
                  ...
                  return (
                    <tr key={`login-${idx}`} className="border-b border-border">
                      ...
                      <td className="py-2 pr-4 truncate max-w-[140px] mono" title={ip}>{ip}</td>
                      <td className="py-2 pr-4 truncate max-w-[200px]" title={device}>{device}</td>
                      <td className="py-2 whitespace-nowrap mono" title={ts}>{ts}</td>
                    </tr>
                  )
                })}
              </tbody>
          ...
          <p className="text-sm text-muted">No login attempts in the last 30 days</p>
        )}
      </div>

      {/* Admin Actions Table */}
      <div>
        <h3 className="text-sm font-medium text-text mb-2">Admin Actions (Last 90 Days)</h3>
        {adminActions.length > 0 ? (
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {adminActions.map((action, idx) => {
                  ...
                  return (
                    <tr key={`action-${idx}`} className="border-b border-border">
          ...
          <p className="text-sm text-muted">No admin actions in the last 90 days</p>
        )}
      </div>
    </section>
  )
...
function QuickActionsBar({
  ...
  return (
    <section className="bg-card rounded-card border border-border p-4">
      <h2 className="text-sm font-semibold text-text mb-2">Quick Actions</h2>
      ...
      <Modal open={open} onClose={onClose} title="Send Notification">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex flex-col gap-1">
            <label className="text-sm font-medium text-text">Target Organisation</label>
            <p
              className="h-[42px] w-full rounded-ctl border border-border bg-canvas px-3 py-2 text-text truncate"
              title={orgName ?? ''}
            >
              {orgName ?? 'Unknown'}
            </p>
          ...
          {/* Message textarea */}
          <div className="flex flex-col gap-1">
            <label htmlFor="notification-message" className="text-sm font-medium text-text">
              Message
            </label>
            <textarea
              ...
              rows={4}
              required
              className="w-full rounded-ctl border border-border bg-card px-3 py-2 text-text transition-colors
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 sm:px-6 lg:px-8">
        ...
        <div className="mt-4">
          <Link
            ...
            className="text-accent hover:brightness-110 hover:underline text-sm"
          >
      ...
      <nav className="text-sm text-muted mb-4" aria-label="Breadcrumb">
        <ol className="flex items-center gap-1">
          <li>
            <Link
              to="/admin/organisations"
              className="text-accent hover:brightness-110 hover:underline"
            >
              Organisations
            </Link>
          </li>
          ...
          <li
            className="truncate max-w-xs text-text font-medium"
            title={org?.name ?? ''}
          >
            {org?.name ?? 'Unknown'}
          </li>
