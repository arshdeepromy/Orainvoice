...
export function GlobalAdminProfile() {
  ...
  const fetchProfile = useCallback(async () => {
    ...
    try {
      const res = await apiClient.get('/auth/me', { signal: controller.signal })
      setProfile(res.data)
  ...
  return (
    <div className="max-w-2xl space-y-8">
      ...
      {/* Name section */}
      <form onSubmit={handleNameSave} className="bg-card rounded-card border border-border p-6 space-y-4 shadow-card">
        <h3 className="text-sm font-medium text-text">Personal Information</h3>

        <div>
          <label className="block text-sm text-muted mb-1">Email</label>
          <p className="rounded-ctl border border-border bg-canvas px-3 py-2 text-sm text-muted">
            {profile.email}
          </p>
        </div>
      ...
      {profile.has_password && (
        <form onSubmit={handlePasswordChange} className="bg-card rounded-card border border-border p-6 space-y-4 shadow-card">
