  ...
  const fetchBalance = useCallback(async () => {
    ...
    try {
      ...
      if (res.data?.detail) {
        ...
        setBalance({ balance: res.data?.balance ?? 0, currency: res.data?.currency ?? '' })
      }
    } catch (err: unknown) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="border-t border-border pt-4">
        <h4 className="text-sm font-medium text-text mb-3">Account Balance</h4>
        ...
        {provider.credentials_set && balance && (
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <span className="text-2xl font-semibold text-text mono">
                ${(balance.balance ?? 0).toFixed(2)}
              </span>
              <span className="text-sm text-muted">{balance.currency}</span>
              <button
                onClick={fetchBalance}
                className="ml-2 rounded p-1 text-muted-2 hover:text-muted hover:bg-canvas transition-colors"
      ...
      {/* Webhook URLs */}
      <div className="border-t border-border pt-4">
        <h4 className="text-sm font-medium text-text mb-3">Webhook URLs</h4>
        <p className="text-xs text-muted mb-3">
          Configure these URLs in your Connexus dashboard, or use the button below to set them automatically.
        </p>
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            <label className="text-xs font-medium text-muted w-28 shrink-0">Incoming SMS:</label>
            <code className="flex-1 rounded bg-canvas px-3 py-1.5 text-xs text-text mono truncate">
              {incomingWebhookUrl}
            </code>
            <button
              onClick={() => copyToClipboard(incomingWebhookUrl, 'incoming')}
              className="shrink-0 rounded px-2 py-1 text-xs text-accent hover:bg-accent-soft transition-colors"
          ...
          <div className="flex items-center gap-2">
            <label className="text-xs font-medium text-muted w-28 shrink-0">Delivery Status:</label>
            <code className="flex-1 rounded bg-canvas px-3 py-1.5 text-xs text-text mono truncate">
              {statusWebhookUrl}
            </code>
            <button
...
function SmsCostConfig({ provider, addToast, onConfigSaved }: { provider: SmsProvider; addToast: (type: 'success' | 'error', msg: string) => void; onConfigSaved: () => void }) {
  ...
  return (
    <div className="border-t border-border pt-4">
      <h4 className="text-sm font-medium text-text mb-3">Pricing</h4>
...
function TokenRefreshConfig({ provider, addToast, onConfigSaved }: { provider: SmsProvider; addToast: (type: 'success' | 'error', msg: string) => void; onConfigSaved: () => void }) {
  ...
  return (
    <div className="border-t border-border pt-4">
      <h4 className="text-sm font-medium text-text mb-3">Token Refresh Settings</h4>
      <div className="flex items-end gap-3">
        <div className="max-w-xs">
          <Input
    ...
    >
      <div className="flex items-center gap-4 px-5 py-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-muted truncate">{provider.description}</p>
        </div>
          ...
          {provider.is_active && (
            <>
              <button
                ...
                className="min-h-[36px] min-w-[36px] flex items-center justify-center rounded-ctl text-muted-2 hover:text-danger hover:bg-danger-soft transition-colors"
                title="Deactivate"
              ...
              <button
                ...
                className={`min-h-[36px] min-w-[36px] flex items-center justify-center rounded-ctl transition-colors ${
                  provider.is_default
                    ? 'text-warn'
                    : 'text-muted-2 hover:text-warn hover:bg-warn-soft'
                }`}
      ...
      {/* Metadata line */}
      {provider.is_active && (
        <div className="flex items-center gap-4 px-5 pb-3 text-xs text-muted">
          <span>Credentials: {provider.credentials_set ? '✓ Set' : '✗ Not set'}</span>
          <span>Priority: {provider.priority}</span>
          <span>Updated: <span className="mono">{formatDate(provider.updated_at)}</span></span>
        </div>
      )}
      ...
      {expanded && (
        <div className="border-t border-border px-5 py-4 space-y-4">
          ...
          {provider.is_active && (
            <>
              <div>
                <h4 className="text-sm font-medium text-text mb-3">Credentials</h4>
              ...
              {provider.credentials_set && (
                <div className="border-t border-border pt-4">
                  <h4 className="text-sm font-medium text-text mb-3">Test Connection</h4>
              ...
              {provider.provider_key === 'firebase_phone_auth' && provider.is_active && provider.credentials_set && (
                <>
                  <div className="border-t border-border pt-4">
                    <h4 className="text-sm font-medium text-text mb-2">MFA / Phone Verification Default</h4>
                    <p className="text-xs text-muted mb-3">
                    ...
                    {isMfaDefault ? (
                      <div className="flex items-center gap-2">
                        <span className="inline-flex items-center rounded-full bg-ok-soft px-3 py-1 text-sm font-medium text-ok">
                  ...
                  <div className="border-t border-border pt-4">
                    <h4 className="text-sm font-medium text-text mb-2">Send Test Verification Code</h4>
                    <p className="text-xs text-muted mb-3">
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-card border border-border bg-card px-5 py-4 shadow-card">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-semibold text-text">SMS Verification Providers</h2>
      ...
      {chain.length > 0 && (
        <div className="rounded-card border border-border bg-card px-5 py-4 shadow-card">
          <h3 className="text-sm font-semibold text-text">Fallback Chain Order</h3>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {chain.map((item) => (
              <span
                ...
                className="inline-flex items-center gap-1.5 rounded-ctl border border-border bg-canvas px-3 py-1.5 text-sm font-medium text-text"
              >
                {item.display_name}
                <span className="text-xs text-muted-2 mono">P{item.priority}</span>
              </span>
            ))}
          </div>
        </div>
