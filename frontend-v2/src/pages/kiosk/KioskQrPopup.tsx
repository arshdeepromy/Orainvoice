...
export function KioskQrPopup({ session, onPaymentComplete, onExpired, onClose }: KioskQrPopupProps) {
  ...
  const timerColorClass = isWarning ? 'text-danger' : 'text-muted'
  ...
  if (popupState === 'success') {
    return (
      ...
      >
        <div className="flex flex-col items-center gap-6 rounded-card bg-card p-12 text-center shadow-pop">
          {/* Green tick */}
          <div className="flex h-24 w-24 items-center justify-center rounded-full bg-ok-soft">
            <svg
              className="h-14 w-14 text-ok"
              fill="none"
              viewBox="0 0 24 24"
              strokeWidth={2.5}
              stroke="currentColor"
          ...
          <p className="text-lg text-muted">Payment received</p>
          <p className="text-sm text-muted-2 mt-2">Tap anywhere to dismiss</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80">
      <div className="flex flex-col items-center gap-6 rounded-card bg-card p-12 text-center shadow-pop">
        ...
        {/* QR Code */}
        <div className="rounded-card border-4 border-border p-4">
          <QRCodeSVG
        ...
        {/* Countdown timer */}
        <div className={`mono text-2xl font-semibold ${timerColorClass}`}>
        ...
        {onClose && (
          <button
            type="button"
            onClick={onClose}
            className="mt-2 rounded-ctl border border-border-strong px-6 py-2.5 text-sm font-medium text-text hover:bg-canvas focus:outline-none focus:ring-2 focus:ring-accent focus:ring-offset-2 transition-colors"
          >
            Close
          </button>
        )}
