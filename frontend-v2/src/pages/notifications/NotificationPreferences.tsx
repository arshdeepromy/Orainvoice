      ...
      <div className="space-y-8">
        {categories
          ...
          <section key={group.category} aria-labelledby={`cat-${group.category}`}>
            <div className="mb-3">
              <h3 id={`cat-${group.category}`} className="text-lg font-medium text-text">{group.category}</h3>
            </div>

            {group.preferences.length === 0 ? (
              <p className="text-sm text-muted-2 italic">No notification types in this category.</p>
            ) : (
                    ...
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <button
                        ...
                        className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors
                          ...
                          ${pref.is_enabled ? 'bg-accent' : 'bg-border-strong'}
                      ...
                      <div className="min-w-0">
                        <span className="text-sm font-medium text-text">{formatLabel(pref.notification_type)}</span>
                        {pref.is_enabled && (
                    ...
                    {pref.is_enabled && (
                      <div className="flex items-center gap-4 ml-14 sm:ml-0">
                        <select
                          ...
                          disabled={saving === pref.notification_type}
                          className="rounded-ctl border-border text-sm focus:border-accent focus:ring-accent"
