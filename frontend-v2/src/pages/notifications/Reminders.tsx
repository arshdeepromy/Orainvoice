  ...
  return (
    <div>
      ...
      <section className="mb-8">
        <h3 className="text-lg font-medium text-text mb-3">Manual Reminders</h3>
        <div className="rounded-card border border-border bg-card overflow-hidden">
          <table className="min-w-full">
            <thead>
              <tr>
                <th className="mono border-b border-border px-4 py-3 text-left text-[10.5px] font-medium uppercase tracking-[0.08em] text-muted-2">Name</th>
            ...
            <tbody>
              {manualReminders.map(mr => (
                <tr key={mr.id} className="border-b border-border last:border-b-0">
                  <td className="px-4 py-3 text-sm text-accent font-medium">{mr.name}</td>
                  <td className="px-4 py-3 text-sm text-muted">{mr.description}</td>
                  <td className="px-4 py-3 text-right">
                    <span className="text-muted-2 text-sm" title="Edit template in Templates tab">
      ...
      {/* Automated Reminders */}
      <section>
        <h3 className="text-lg font-medium text-text mb-3">Automated Reminders</h3>
        <div className="rounded-card border border-border bg-card overflow-hidden">
          <table className="min-w-full">
            <thead>
              <tr>
                <th className="mono border-b border-border px-4 py-3 text-left text-[10.5px] font-medium uppercase tracking-[0.08em] text-muted-2">Name</th>
                <th className="mono border-b border-border px-4 py-3 text-left text-[10.5px] font-medium uppercase tracking-[0.08em] text-muted-2">Schedule</th>
                <th className="mono border-b border-border px-4 py-3 text-center text-[10.5px] font-medium uppercase tracking-[0.08em] text-muted-2">Status</th>
                <th className="mono border-b border-border px-4 py-3 text-right text-[10.5px] font-medium uppercase tracking-[0.08em] text-muted-2">Actions</th>
              </tr>
            </thead>
            {Object.entries(groupedByRef).map(([refDate, groupRules]) => (
              <tbody key={`group-${refDate}`}>
                <tr className="bg-canvas">
                  <td colSpan={4} className="px-4 py-2 text-xs font-semibold text-text uppercase">
                    ...
                    <td className="px-4 py-3 text-center">
                      <button
                        ...
                        className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors
                          ...
                          ${rule.is_enabled ? 'bg-accent' : 'bg-border-strong'}
                          ${saving ? 'opacity-50' : ''}`}
                      >
                        <span
                          aria-hidden="true"
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="relative inline-block">
                        <button
                          ...
                          className="text-muted-2 hover:text-text p-1"
                        ...
                        {actionsOpenId === rule.id && (
                          <div className="absolute right-0 z-10 mt-1 w-36 rounded-card border border-border bg-card shadow-pop">
                            <button
                              onClick={() => openEditReminder(rule)}
                              className="block w-full px-4 py-2 text-left text-sm text-text hover:bg-canvas"
                            >
                              Edit
                            </button>
                            <button
            ...
            {rules.length === 0 && (
              <tbody>
                <tr>
                  <td colSpan={4} className="px-4 py-6 text-center text-sm text-muted-2 italic">
        ...
        <button
          ...
          className="mt-4 inline-flex items-center gap-1 text-sm font-medium text-accent hover:text-accent-press"
        >
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors
                focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-2
                ${editingRule.is_enabled ? 'bg-accent' : 'bg-border-strong'}`}
            >
              <span
                aria-hidden="true"
                className={`pointer-events-none inline-block h-5 w-5 rounded-full bg-card shadow ring-0 transition-transform
