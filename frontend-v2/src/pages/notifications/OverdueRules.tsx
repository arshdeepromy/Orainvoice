...
export default function OverdueRules() {
  ...
  return (
    <div>
      ...
      {/* Master toggle */}
      <div className="mb-6 flex items-center gap-3 rounded-card border border-border bg-card px-4 py-4">
        <button
          role="switch"
        ...
        <div>
          <span className="text-sm font-medium text-text">Overdue payment reminders</span>
      ...
      {enabled && (
        <div className="space-y-4">
          {rules.map((rule, idx) => (
            <div key={rule.id} className="flex flex-col gap-3 sm:flex-row sm:items-end rounded-card border border-border bg-card px-4 py-4">
