 ...
 *
 * Ported from frontend/src/pages/dashboard/widgets/ReminderConfigWidget.tsx
 * (Task 18). ALL logic preserved verbatim (FR-1): the exported
 * `isValidThreshold` (integer 1–365), the prop→state sync effect, the
 * `canSave` gate, and the `PUT /dashboard/reminder-config` save mutation (with
 * the `{ wof_days, service_days }` body, the 3s success flash and the error
 * message). Presentation remapped onto the redesign tokens (FR-2): token inputs
 * with accent focus ring (danger ring when invalid), an accent primary Save
 * button, and ok/danger feedback text.
 *
  ...
  return (
    ...
    >
      <div className="space-y-4">
        {/* WOF threshold */}
        <div>
          <label htmlFor="wof-days" className="mb-1 block text-[12.5px] font-medium text-text">
          ...
          <input
            ...
            className={`mono block w-full rounded-ctl border bg-card px-3 py-2 text-[13.5px] text-text outline-none transition-[border-color,box-shadow] focus:shadow-[0_0_0_3px_var(--accent-soft)] ${
              wofValid
          ...
          />
          {!wofValid && (
            <p className="mt-1 text-[12px] text-danger">Must be between 1 and 365</p>
          )}
        </div>

        {/* Service threshold */}
        <div>
          <label htmlFor="service-days" className="mb-1 block text-[12.5px] font-medium text-text">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            disabled={!canSave}
            onClick={handleSave}
            className="rounded-ctl bg-accent px-4 py-2 text-[13.5px] font-medium text-white transition-colors hover:bg-accent-press disabled:cursor-not-allowed disabled:opacity-50"
          >
            {saving ? 'Saving…' : 'Save'}
          </button>
          {saveSuccess && (
            <span className="text-[12px] font-medium text-ok">Saved successfully</span>
          )}
          {saveError && (
