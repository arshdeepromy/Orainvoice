/**
 * OrgAdminDashboard tests (Task 17).
 *
 * Ported from frontend/src/pages/dashboard/__tests__/OrgAdminDashboard.test.tsx.
 * Verifies the automotive trade-family gate that decides whether the
 * <WidgetGrid> renders:
 *   (tradeFamily ?? 'automotive-transport') === 'automotive-transport'
 * i.e. shown for automotive (and the null default), hidden for other trades.
 *
 * The contexts + api client are mocked so the page renders deterministically;
 * the WidgetGrid stub (Task 18 placeholder) is further mocked to a marker so
 * the gate can be asserted without depending on the placeholder's markup.
 */
...
// Mock the WidgetGrid (Task 18 placeholder) to a marker so the gate is
// asserted independently of the placeholder's markup.
  ...
  beforeEach(() => {
    ...
    ;(apiClient.get as ReturnType<typeof vi.fn>).mockImplementation((url: string) => {
      ...
      if (url === '/reports/storage') {
        return Promise.resolve({ data: { used_bytes: 0, used_gb: 0, quota_gb: 10, usage_percent: 0 } })
      }
