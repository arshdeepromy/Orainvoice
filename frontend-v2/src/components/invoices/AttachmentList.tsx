...
/**
 * AttachmentList — Task 19 port of frontend/src/components/invoices/AttachmentList.
 *
 * All logic copied VERBATIM: fetch on mount with AbortController cleanup, safe
 * `?? []` consumption, open-in-new-tab via the raw `/api/v1/...` file path,
 * delete (draft only), and the "render nothing while loading / when empty"
 * behaviour. Styling remapped onto the design-system tokens.
 */
...
function FileTypeIcon({ mimeType }: { mimeType: string | undefined | null }) {
  if (isImageMime(mimeType)) {
    return (
      <div
        className="flex h-9 w-9 items-center justify-center rounded bg-accent-soft text-accent"
        aria-hidden="true"
      >
  ...
  if (isPdfMime(mimeType)) {
    return (
      <div
        className="flex h-9 w-9 items-center justify-center rounded bg-danger-soft text-danger"
        aria-hidden="true"
      >
        <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
          <path
  ...
  return (
    <div
      className="flex h-9 w-9 items-center justify-center rounded bg-canvas text-muted-2"
      aria-hidden="true"
    >
      <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
        <path
...
export default function AttachmentList({ invoiceId, isDraft }: AttachmentListProps) {
  ...
  return (
    <div className="mt-4">
      ...
      <ul className="divide-y divide-border rounded-ctl border border-border bg-card" role="list">
        {(attachments ?? []).map((attachment) => (
          <li key={attachment.id} className="flex items-center gap-3 px-3 py-2.5">
            ...
            <button
              type="button"
              className="flex min-w-0 flex-1 flex-col text-left rounded hover:bg-canvas focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-1 p-1 -m-1 transition-colors"
            ...
            >
              <span className="truncate text-sm font-medium text-accent hover:text-accent-press">
            ...
            {isDraft && (
              <button
                type="button"
                className="flex-shrink-0 rounded p-1.5 text-muted-2 hover:bg-danger-soft hover:text-danger focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-danger transition-colors"
