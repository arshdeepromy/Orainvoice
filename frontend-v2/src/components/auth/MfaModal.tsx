...
/**
 * MfaModal — two-factor verification dialog shown after a login that requires
 * MFA (Task 13 port of frontend/src/components/auth/MfaModal — a direct
 * dependency of the Login page's MFA-required branch).
 *
 * ALL logic is copied verbatim from the original: the method selection
 * (totp/sms/email/backup/passkey), the OTP auto-advance / backspace / paste
 * handling, the Firebase phone-auth challenge path (dynamic firebase import +
 * invisible reCAPTCHA), the server-side OTP challenge send + resend cooldown,
 * the WebAuthn passkey assertion flow (base64url buffer helpers), and the
 * submit → completeMfa / completeFirebaseMfa branch with full 429/401/403 error
 * handling. Only the presentational markup is remapped to the design tokens and
 * the prototype's MFA modal (OraInvoice_Handoff/app/Login.html `.modal` +
 * `.otp` inputs): modal-scrim backdrop, shadow-pop card, accent OTP inputs.
 */
...
export function MfaModal({ open, onClose, onSuccess }: MfaModalProps) {
  const { completeMfa, completeFirebaseMfa, mfaSessionToken, mfaMethods, mfaDefaultMethod } = useAuth()
      ...
      try {
        ...
        if (selectedMethod === 'sms') {
          ...
          if (provider === 'firebase_phone_auth' && firebase_config && phone_number) {
            // Use Firebase JS SDK to send the verification code
        ...
        // Default: use server-side OTP (Connexus or email)
  ...
  return (
    ...
    >
      <div className="w-full max-w-md space-y-6 rounded-card bg-card p-8 shadow-pop">
        <div className="text-center">
          <div className="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-accent-soft">
            <svg className="h-6 w-6 text-accent" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" aria-hidden="true">
        ...
        {availableMethods.length > 1 && (
          <div>
            <p className="mb-2 text-[13px] font-medium text-text">Verification method</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {method === 'backup' ? (
              <div className="flex flex-col gap-[7px]">
                <label htmlFor="backup-code-modal" className="text-[12.5px] font-medium text-text">
                  Backup code
                </label>
                <input
                  id="backup-code-modal"
                  type="text"
                  value={backupCode}
                  onChange={(e) => setBackupCode(e.target.value)}
                  className="mono rounded-ctl border border-border-strong px-3 py-2.5 text-center text-lg tracking-widest focus:border-accent focus:outline-none focus:shadow-[0_0_0_3px_var(--accent-soft)]"
              ...
              <div>
                <label className="mb-2 block text-[12.5px] font-medium text-text">6-digit code</label>
