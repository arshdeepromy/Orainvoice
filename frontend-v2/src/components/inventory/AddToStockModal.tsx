...
import { Modal, Button, FormField, Badge, Spinner, useToast, ToastContainer } from '@/components/ui'
import { useTenant } from '@/contexts/TenantContext'
import { InlineCreateForm } from './InlineCreateForm'

/**
 * AddToStockModal — Task 20 port of frontend/src/components/inventory/AddToStockModal.
 *
 * Three-step "Quick Add Stock" wizard (category → catalogue picker → stock
 * details) used by the invoice form's inventory picker. ALL logic — fetches,
 * pricing maths, supplier/location create, trade-family category gating, submit
 * to /inventory/stock-items — is copied VERBATIM. Styling stays on the neutral
 * palette (available in v2); the full token restyle of the Inventory module is
 * owned by Tasks 35-38, which will adopt this component. v2 Button has no
 * `secondary` variant, so `secondary` is mapped to `ghost`.
 */
  ...
  return (
    <div>
      <div className="mb-4 flex items-center gap-2">
        <Button variant="ghost" size="sm" onClick={onBack} type="button" aria-label="Back to category selection">
          ← Back
        </Button>
        ...
        <div className="rounded-md border border-gray-200 bg-gray-50 px-4 py-8 text-center text-sm text-gray-500">
          {onCreateNew ? (
            <>
              ...
              <Button variant="ghost" size="sm" onClick={onCreateNew} type="button">
...
function StockDetailsForm({
  ...
  return (
    <div>
      <div className="mb-4 flex items-center gap-2">
        <Button variant="ghost" size="sm" onClick={onBack} type="button" aria-label="Back to catalogue picker">
          ← Back
        </Button>
      ...
      <div className="space-y-4">
        ...
        <div className="flex justify-end gap-2 pt-2">
          <Button variant="ghost" size="sm" onClick={onBack} disabled={submitting} type="button">
            Back
          </Button>
          <Button
            variant="primary"
      ...
      <Modal open={showNewSupplierModal} onClose={() => setShowNewSupplierModal(false)} title="New Supplier">
        <div className="space-y-3">
          ...
          <div className="flex justify-end gap-2 pt-2">
            <Button variant="ghost" size="sm" onClick={() => setShowNewSupplierModal(false)}>Cancel</Button>
