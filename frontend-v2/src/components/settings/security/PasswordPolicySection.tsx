  ...
  return (
    <div className="space-y-4 max-w-2xl">
      ...
      <fieldset className="space-y-2">
        <legend className="text-sm font-medium text-text mb-1">Complexity Requirements</legend>
        {TOGGLES.map(({ key, label }) => (
          <div key={key} className="flex items-center gap-3">
            <button
              ...
              className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${form[key] ? 'bg-accent' : 'bg-border-strong'}`}
            >
