...
/**
 * AttachmentList — Task 27 port of
 * frontend/src/components/attachments/AttachmentList.tsx.
 *
 * ALL logic copied VERBATIM: blob-based authenticated image thumbnails, PDF /
 * generic file icons, image lightbox via onImageClick (PDFs/others open in a new
 * tab from a fetched blob), optional delete, readOnly mode. Styling remapped
 * onto the design tokens (PDF icon keeps a semantic danger tint).
 */
...
function ImageThumbnail({ attachment }: { attachment: Attachment }) {
  ...
  if (!blobUrl) {
    ...
    return (
      <div className="flex h-10 w-10 items-center justify-center rounded bg-canvas text-muted-2">
...
function AttachmentThumbnail({ attachment }: { attachment: Attachment }) {
  ...
  if (isPdfMime(attachment.mime_type)) {
    return (
      <div
        className="flex h-10 w-10 items-center justify-center rounded bg-danger-soft text-danger"
        aria-hidden="true"
      >
  ...
  return (
    <div
      className="flex h-10 w-10 items-center justify-center rounded bg-canvas text-muted"
      aria-hidden="true"
    >
      <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m2.25 0H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />
...
export default function AttachmentList({
  ...
  return (
    <ul className="divide-y divide-border rounded-card border border-border" role="list">
      {safeAttachments.map((attachment) => (
        <li key={attachment.id} className="flex items-center gap-3 px-3 py-2">
          {/* Clickable area: thumbnail + file info */}
          <button
            type="button"
            className="-m-1 flex min-w-0 flex-1 items-center gap-3 rounded p-1 text-left transition-colors hover:bg-canvas focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-1"
          ...
          {!readOnly && onDelete && (
            <button
              type="button"
              className="flex-shrink-0 rounded p-1.5 text-muted-2 transition-colors hover:bg-danger-soft hover:text-danger focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-danger"
