...
/**
 * AttachmentUploader — Task 27 port of
 * frontend/src/components/attachments/AttachmentUploader.tsx.
 *
 * Drag-and-drop / click file uploader for job-card attachments. ALL logic copied
 * VERBATIM: file validation (50MB, JPEG/PNG/WebP/GIF/PDF), multipart upload with
 * progress + AbortController, 413/507 error mapping, input reset. Styling
 * remapped onto the design tokens.
 */
  ...
  return (
    <div>
      ...
      <div
        ...
        className={`relative flex min-h-[120px] flex-col items-center justify-center rounded-card border-2 border-dashed p-6 transition-colors ${
          isDisabled
            ? 'cursor-not-allowed border-border bg-canvas opacity-60'
            : dragActive
              ? 'cursor-pointer border-accent bg-accent-soft'
              : fileError
                ? 'cursor-pointer border-danger/40 bg-danger-soft'
                : 'cursor-pointer border-border bg-canvas hover:border-border-strong'
        }`}
      ...
      >
        ...
        {uploading ? (
          ...
          <div className="w-full max-w-xs text-center">
            <svg
              className="mx-auto mb-2 h-8 w-8 animate-spin text-accent"
