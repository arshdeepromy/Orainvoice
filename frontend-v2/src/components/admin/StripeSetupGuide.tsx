...
const STEPS: Step[] = [
  {
    ...
    content: (
      <a
        ...
        className="inline-flex items-center gap-1 text-sm text-accent hover:text-accent-press underline"
      >
    ...
    content: (
      <ul className="mt-1 space-y-0.5">
        {WEBHOOK_EVENTS.map((evt) => (
          <li key={evt} className="text-sm text-muted mono">
            {evt}
          </li>
        ))}
      </ul>
...
export function StripeSetupGuide({ progress }: StripeSetupGuideProps) {
  ...
  return (
    <div className="rounded-card border border-accent/30 bg-accent-soft mb-5" role="region" aria-label="Stripe setup guide">
      {/* Header */}
      <div className="flex items-center justify-between px-5 py-3">
        <button
          type="button"
          onClick={() => setCollapsed((c) => !c)}
          className="flex items-center gap-2 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-2 rounded"
          aria-expanded={!collapsed}
        >
          <svg
            className={`h-5 w-5 text-accent transition-transform ${collapsed ? '' : 'rotate-180'}`}
          ...
          <h3 className="text-md font-semibold text-accent">Setup Guide</h3>
        ...
        <button
          ...
          className="text-accent/60 hover:text-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent rounded p-1"
            ...
            return (
              <li key={step.number} className="flex gap-3">
                ...
                <div className="flex-shrink-0 mt-0.5">
                  {done ? (
                    <span
                      className="flex h-6 w-6 items-center justify-center rounded-full bg-ok text-white"
                    ...
                    <span
                      className="flex h-6 w-6 items-center justify-center rounded-full border-2 border-accent/40 text-xs font-semibold text-accent"
                ...
                <div className="min-w-0">
                  <p className={`text-sm font-medium ${done ? 'text-ok line-through' : 'text-text'}`}>
                    {step.title}
                  </p>
                  <p className="text-sm text-muted mt-0.5">{step.explanation}</p>
