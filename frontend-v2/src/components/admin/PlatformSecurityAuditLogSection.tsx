...
export function PlatformSecurityAuditLogSection() {
  ...
  return (
    <div className="space-y-4">
      {/* Error banner */}
      {error && (
        <div className="rounded-ctl bg-danger-soft border border-danger/30 px-4 py-2 text-sm text-danger">
          {error}
        </div>
      )}
      ...
      <div className="flex flex-wrap gap-3 items-end">
        <div className="flex flex-col gap-1">
          ...
          <input
            ...
            className="rounded-ctl border border-border bg-card px-2 py-1.5 text-sm text-text focus:outline-none focus:border-accent focus:shadow-[0_0_0_3px_var(--accent-soft)]"
          />
        </div>
        <div className="flex flex-col gap-1">
        ...
        <div className="flex flex-col gap-1">
          ...
          <select
            ...
            className="rounded-ctl border border-border px-2 py-1.5 text-sm bg-card text-text focus:outline-none focus:border-accent focus:shadow-[0_0_0_3px_var(--accent-soft)]"
          >
            {ACTION_TYPES.map((a) => (
        ...
        <div className="flex flex-col gap-1">
          ...
          <input
            ...
            className="rounded-ctl border border-border bg-card px-2 py-1.5 text-sm text-text placeholder:text-muted-2 focus:outline-none focus:border-accent focus:shadow-[0_0_0_3px_var(--accent-soft)]"
          />
        </div>
        <div className="flex flex-col gap-1">
      ...
      {/* Truncation warning */}
      {truncated && (
        <div className="rounded-ctl bg-warn-soft border border-warn/30 px-4 py-2 text-sm text-warn">
      ...
      <div className="overflow-x-auto rounded-card border border-border bg-card shadow-card">
        <table className="min-w-full text-sm">
          <thead>
            <tr>
              <th className="mono border-b border-border px-4 py-3 text-left text-[10.5px] font-medium uppercase tracking-[0.08em] text-muted-2">Timestamp</th>
              <th className="mono border-b border-border px-4 py-3 text-left text-[10.5px] font-medium uppercase tracking-[0.08em] text-muted-2">User</th>
              <th className="mono border-b border-border px-4 py-3 text-left text-[10.5px] font-medium uppercase tracking-[0.08em] text-muted-2">Action</th>
              <th className="mono border-b border-border px-4 py-3 text-left text-[10.5px] font-medium uppercase tracking-[0.08em] text-muted-2">Organisation</th>
              ...
              <th className="mono border-b border-border px-4 py-3 text-left text-[10.5px] font-medium uppercase tracking-[0.08em] text-muted-2">OS</th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={7} className="px-4 py-8 text-center text-muted">No audit log entries found.</td></tr>
            ) : (
              (entries ?? []).map((entry) => (
                <tr key={entry.id} className="border-b border-border last:border-b-0 hover:bg-canvas">
                  ...
                  <td className="px-4 py-2 text-text">{entry.user_email ?? '—'}</td>
                  <td className="px-4 py-2 text-text">{entry.action_description ?? entry.action ?? '—'}</td>
                  <td className="px-4 py-2 text-text">{entry.org_name ?? 'Platform'}</td>
                  <td className="mono px-4 py-2 text-muted text-xs">{entry.ip_address ?? '—'}</td>
                  <td className="px-4 py-2 text-muted text-xs">{entry.browser ?? '—'}</td>
                  <td className="px-4 py-2 text-muted text-xs">{entry.os ?? '—'}</td>
                </tr>
              ))
            )}
          </tbody>
