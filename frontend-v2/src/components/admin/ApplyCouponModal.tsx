  ...
  return (
    <Modal open={open} onClose={onClose} title={`Apply coupon to ${orgName}`}>
      <div className="space-y-4">
        ...
        {/* Error banner */}
        {error && <div className="text-sm text-danger bg-danger-soft p-3 rounded-ctl">{error}</div>}
        ...
        {/* Coupon list */}
        {!loading && (
          <div className="max-h-64 overflow-y-auto border border-border rounded-ctl divide-y divide-border">
            {(filteredCoupons ?? []).map((coupon) => (
              <div
                key={coupon.id}
                className={`p-3 cursor-pointer hover:bg-canvas ${selectedCouponId === coupon.id ? 'bg-accent-soft border-l-4 border-accent' : ''}`}
              ...
              >
                ...
                <p className="text-xs text-muted mt-1">{coupon.description ?? 'No description'}</p>
                <div className="flex gap-4 text-xs text-muted-2 mt-1">
            ...
            {filteredCoupons.length === 0 && !loading && (
              <p className="text-sm text-muted p-4 text-center">No coupons found</p>
            )}
          </div>
        )}
        ...
        {/* Benefit preview */}
        {selectedCoupon && (
          <div className="bg-canvas p-3 rounded-ctl text-sm">
