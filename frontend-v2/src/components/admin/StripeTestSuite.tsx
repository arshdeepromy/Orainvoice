...
function statusBadgeVariant(status: TestResult['status']): 'success' | 'danger' | 'warn' {
  switch (status) {
    case 'passed':
      return 'success'
    case 'failed':
...
function ResultGroup({ category, results }: { category: string; results: TestResult[] }) {
  return (
    <div className="rounded-card border border-border">
      <div className="px-4 py-2.5 bg-canvas border-b border-border">
        ...
        {results.map((r) => (
          <li key={r.test_name} className="px-4 py-3">
            <div className="flex items-center justify-between gap-3">
              <span className="text-sm text-text">{r.test_name}</span>
              <Badge variant={statusBadgeVariant(r.status)}>
                {statusLabel(r.status)}
              </Badge>
            </div>
            {r.status === 'failed' && r.error_message && (
              <p className="mt-1.5 text-xs text-danger bg-danger-soft rounded px-2 py-1">
                {r.error_message}
              </p>
            )}
            {r.status === 'skipped' && r.skip_reason && (
              <p className="mt-1.5 text-xs text-warn bg-warn-soft rounded px-2 py-1">
                {r.skip_reason}
              </p>
            )}
          </li>
...
function SummaryLine({ summary }: { summary: TestSummary }) {
  ...
  return (
    <div
      className={`flex items-center gap-2 rounded-ctl px-4 py-2.5 text-sm font-medium ${
        allPassed
    ...
    >
      ...
      {summary.skipped > 0 && (
        <span className="text-warn ml-1">({summary.skipped} skipped)</span>
      )}
    </div>
  )
}
  ...
  return (
    <div className="rounded-card border border-border p-5 space-y-4 shadow-card">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-md font-semibold text-text">Stripe Test Suite</h3>
          <p className="text-xs text-muted mt-0.5">
            Run automated tests for all Stripe API functions and webhook handlers.
          </p>
        </div>
        <Button onClick={runTests} loading={running} variant="ghost" size="sm">
          Run All Tests
        </Button>
      </div>
