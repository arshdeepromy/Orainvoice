...
import { Button, Badge, Spinner, AlertBanner, ConfirmDialog, ToastContainer, useToast } from '@/components/ui'
...
function PaymentMethodCard({
  ...
  return (
    <div
      className={`flex flex-col sm:flex-row sm:items-center justify-between gap-3 rounded-card border p-4 ${
        method.is_default ? 'border-accent bg-accent-soft' : 'border-border bg-card'
      }`}
    >
      {/* Card info */}
      <div className="flex items-center gap-3 min-w-0">
        {/* Brand icon placeholder */}
        <div className="flex h-10 w-14 items-center justify-center rounded bg-canvas text-xs font-bold text-muted shrink-0">
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {method.is_expiring_soon && (
              <span
                className="inline-flex items-center gap-1 text-xs font-medium text-warn"
          ...
          {isSoleCard && (
            <div
              className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block
                w-64 rounded-ctl bg-ink px-3 py-2 text-xs text-white shadow-pop z-10"
              role="tooltip"
            >
  ...
  if (error) {
    return (
      <div className="space-y-3">
        ...
        <Button variant="ghost" size="sm" onClick={fetchMethods}>
          Retry
        </Button>
      </div>
    )
    ...
    <div className="space-y-4">
      ...
      {methods.length === 0 ? (
        <div className="rounded-card border border-dashed border-border p-8 text-center">
          <p className="text-muted-2">No payment methods on file.</p>
          <p className="text-sm text-muted-2 mt-1">Add a card to get started.</p>
          {onAddCard && (
