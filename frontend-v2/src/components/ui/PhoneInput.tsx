...
/**
 * PhoneInput — Task 20 port of frontend/src/components/ui/PhoneInput.tsx.
 *
 * All logic (country dial-code picker with search, leading-0 stripping, country
 * name→code lookup helpers) is copied VERBATIM so CustomerCreateModal behaves
 * identically. Styling is remapped onto the design-system tokens (FR-2b): the
 * `.field`/`.input` language (h-42, ctl radii, accent focus ring, muted-2
 * placeholder) and token-coloured dropdown surfaces.
 */
      ...
      <div ref={containerRef} className="flex gap-2">
        ...
        <div className="relative">
          <button
            ...
            className="h-[42px] flex items-center gap-1 rounded-ctl border border-border bg-card px-3 py-2 text-sm text-text shadow-sm hover:bg-canvas focus:outline-none focus-visible:ring-2 focus-visible:ring-accent min-w-[90px]"
          >
            <span className="text-base">{selectedCountry.flag}</span>
            <span className="mono">{selectedCountry.dialCode}</span>
            <svg className="h-4 w-4 text-muted-2 ml-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
            </svg>
          </button>

          {showDropdown && (
            <div className="absolute top-full left-0 z-50 mt-1 w-64 rounded-ctl border border-border bg-card shadow-pop">
              {/* Search input */}
              <div className="p-2 border-b border-border">
                <input
                  ...
                  className="w-full rounded-ctl border border-border px-2 py-1.5 text-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-accent"
                />
              </div>
              {/* Country list */}
              <div className="max-h-48 overflow-auto">
                {filteredCountries.map((country) => (
                  <button
                    ...
                    className={`w-full flex items-center gap-2 px-3 py-2 text-sm text-left hover:bg-accent-soft ${
                      selectedCountry.code === country.code ? 'bg-accent-soft text-accent' : 'text-text'
                    }`}
                  >
                    <span className="text-base">{country.flag}</span>
                    <span className="flex-1">{country.country}</span>
                    <span className="text-muted mono">{country.dialCode}</span>
                  </button>
                ))}
        ...
        <input
          ...
          className={`h-[42px] flex-1 rounded-ctl border px-3 py-2 text-text shadow-sm transition-colors
