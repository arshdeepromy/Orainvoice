...
/**
 * POSReceiptPreview — Task 19 port of frontend/src/components/pos/POSReceiptPreview.
 *
 * Pure presentational receipt render (monospace, dashed separators) copied
 * VERBATIM from the original (same rows, same conditional fields, same $X.XX
 * formatting). Restyled only via the design-system border tokens; the receipt
 * body intentionally keeps the Courier New mono paper look.
 */
...
export default function POSReceiptPreview({ receiptData, paperWidth }: POSReceiptPreviewProps) {
  ...
  return (
    <div
      className="mx-auto border border-dashed border-border-strong bg-white p-4 text-xs leading-relaxed text-text"
...
function Separator() {
  return <div className="my-1 border-t border-dashed border-border-strong" />;
}

function SeparatorDouble() {
  return <div className="my-1 border-t-2 border-double border-border-strong" />;
}
