...
/**
 * TerminologyContext — Task 26 port of frontend/src/contexts/TerminologyContext.tsx.
 *
 * Logic copied VERBATIM: fetches the org's terminology map from
 * GET /v2/terminology (per-trade-family label overrides, e.g. "job" → "Ticket"),
 * exposes `useTerm(key, fallback)` for label lookups and `useTerminology()` for
 * the full map + loading/error/refetch. Fetches only when authenticated with an
 * org; clears on logout. Used by the jobs pages (Task 26).
 */
