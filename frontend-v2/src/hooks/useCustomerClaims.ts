 ...
 *
 * Task 23 port of frontend/src/hooks/useCustomerClaims — copied VERBATIM
 * (fetch with AbortController + safe `?? 0` / `?? []` fallbacks, refetch).
 * The only change is the `@/` import path for the v2 api client + types.
 *
 * Requirements: 9.1, 9.2, 9.3 (original) / FR-1 (redesign).
 */

import { useState, useEffect, useCallback, useRef } from 'react'
import apiClient from '@/api/client'
import type { ClaimListItem } from '@/hooks/useClaims'
