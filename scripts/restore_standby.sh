...
echo W4h3guru1# | sudo -S docker cp /tmp/workshoppro_data.sql invoicing-postgres-1:/tmp/workshoppro_data.sql

# Drop subscription
echo W4h3guru1# | sudo -S docker exec invoicing-postgres-1 bash -c 'cat > /tmp/setup_repl.sql << SQLEOF
...
echo W4h3guru1# | sudo -S docker exec invoicing-postgres-1 bash -c 'psql -U postgres -d workshoppro -c "SET session_replication_role = replica;" -f /tmp/workshoppro_data.sql 2>&1 | tail -5'

echo "=== Data restored, creating subscription ==="

# Create subscription with copy_data=false
echo W4h3guru1# | sudo -S docker exec invoicing-postgres-1 bash -c 'cat > /tmp/create_sub_nocopy.sql << SQLEOF
SET statement_timeout = 0;
SET idle_in_transaction_session_timeout = 0;
