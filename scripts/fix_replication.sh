...
echo W4h3guru1# | sudo -S docker exec invoicing-postgres-1 psql -U postgres -d workshoppro -c "ALTER SUBSCRIPTION orainvoice_ha_sub DISABLE;" 2>&1 || true
echo W4h3guru1# | sudo -S docker exec invoicing-postgres-1 psql -U postgres -d workshoppro -c "ALTER SUBSCRIPTION orainvoice_ha_sub SET (slot_name = NONE);" 2>&1 || true
echo W4h3guru1# | sudo -S docker exec invoicing-postgres-1 psql -U postgres -d workshoppro -c "DROP SUBSCRIPTION IF EXISTS orainvoice_ha_sub;" 2>&1
EOF
echo "Done step 1."
...
echo W4h3guru1# | sudo -S docker exec invoicing-postgres-1 psql -U postgres -d workshoppro -c "SELECT 'orgs' as t, count(*) FROM organisations UNION ALL SELECT 'users', count(*) FROM users UNION ALL SELECT 'invoices', count(*) FROM invoices UNION ALL SELECT 'customers', count(*) FROM customers ORDER BY t;"
EOF2

echo ""
echo "=== STEP 5: Confirm clean state ==="
ssh -o ConnectTimeout=15 nerdy@192.168.10.87 << 'EOF3'
echo W4h3guru1# | sudo -S docker exec invoicing-postgres-1 psql -U postgres -d workshoppro -c "SELECT subname FROM pg_subscription;"
EOF3
...
CREATE SUBSCRIPTION orainvoice_ha_sub
  CONNECTION '"'"'host=192.168.1.90 port=5432 dbname=workshoppro user=replicator password=NoorHarleen1 sslmode=disable connect_timeout=60'"'"'
