"""Add job_card_attachments table for file attachments on job cards.

Creates the job_card_attachments table for storing metadata about images
(JPEG, PNG, WebP, GIF) and PDFs attached to job cards. The actual file
...
def upgrade() -> None:
    ...
    result = conn.execute(sa.text(
        ...
        "WHERE table_schema = 'public' AND table_name = 'job_card_attachments'"
    ))
    if result.scalar():
        return

    # ── 1. Create job_card_attachments table ──────────────────────────────
    op.create_table(
        _TABLE,
        ...
        sa.ForeignKeyConstraint(
            ["job_card_id"],
            ["job_cards.id"],
            name="fk_job_card_attachments_job_card_id",
            ondelete="CASCADE",
        ),
        sa.ForeignKeyConstraint(
            ["org_id"],
            ["organisations.id"],
            name="fk_job_card_attachments_org_id",
        ),
        sa.ForeignKeyConstraint(
            ["uploaded_by"],
            ["users.id"],
            name="fk_job_card_attachments_uploaded_by",
        ),
    )

    # ── 2. Create indexes for common query patterns ───────────────────────
    op.create_index("ix_job_card_attachments_job_card_id", _TABLE, ["job_card_id"])
    op.create_index("ix_job_card_attachments_org_id", _TABLE, ["org_id"])
...
def downgrade() -> None:
    ...
    # Drop indexes
    op.drop_index("ix_job_card_attachments_org_id", table_name=_TABLE)
    op.drop_index("ix_job_card_attachments_job_card_id", table_name=_TABLE)
