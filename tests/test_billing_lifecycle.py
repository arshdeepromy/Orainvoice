...
Covers:
- Stripe metered billing functions (report_metered_usage, get_subscription_invoices, handle_subscription_webhook)
...
# ---------------------------------------------------------------------------
# handle_subscription_webhook tests
# ---------------------------------------------------------------------------


class TestHandleSubscriptionWebhook:
    """Test the webhook event handler logic."""

    @pytest.mark.asyncio
    async def test_payment_succeeded_event(self):
        from app.integrations.stripe_billing import handle_subscription_webhook
        ...
        assert result["invoice_pdf"] == "https://stripe.com/inv.pdf"
    ...
    async def test_payment_failed_event(self):
        ...
        assert result["next_payment_attempt"] == 1700100000
    ...
    @pytest.mark.asyncio
    async def test_payment_failed_missing_fields(self):
        from app.integrations.stripe_billing import handle_subscription_webhook
