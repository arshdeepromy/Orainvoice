"""Service layer for quote attachments.

Handles file upload, compression, encryption, storage quota enforcement,
and CRUD operations for quote attachments.

Direct port of app/modules/invoices/attachment_service.py with:
- invoice_id → quote_id
- Invoice → Quote
- InvoiceAttachment → QuoteAttachment
- Storage namespace: "quote-attachments"
- File keys: quote-attachments/{org_id}/{quote_id}/{uuid}-{filename}
- Draft-only delete rule (ValueError when quote.status != 'draft')

Validates: Requirements 3.1–3.10, 4.1–4.3, 5.3–5.5, 6.4, 12.1–12.4
"""
...
ATTACHMENT_CATEGORY = "quote-attachments"
...
def _store_file(
    ...
    File keys use the format: quote-attachments/{org_id}/{quote_id}/{uuid}-{filename}
    ...
    # Generate unique file key: quote-attachments/{org_id}/{quote_id}/{uuid}-{filename}
    file_key = (
        f"{ATTACHMENT_CATEGORY}/{org_id}/{quote_id}/"
...
async def upload_attachment(
    ...
    *,
    ...
    Validates file type and size, checks quote exists and belongs to org,
    enforces max 5 attachments per quote, compresses and encrypts the file,
    ...
    Raises:
        ValueError: Invalid file type (router maps to 400),
                    file too large (router maps to 413),
                    max attachments exceeded (router maps to 400),
                    quote not found
        HTTPException: Storage quota exceeded (from StorageManager, router maps to 507)
    """
    ...
    # Validate quote exists and belongs to org
    ...
    # Enforce max attachments per quote
    count_result = await db.execute(
        select(func.count()).select_from(QuoteAttachment).where(
    ...
    if current_count >= MAX_COUNT:
        raise ValueError(
            f"Maximum {MAX_COUNT} attachments per quote. "
            f"This quote already has {current_count}."
        )

    # Store file (compress + encrypt)
    file_key, file_size = _store_file(
        content, filename, org_id_str, quote_id_str, mime_type
    )
...
async def list_attachments(
    ...
    *,
    ...
    """List all attachments for a quote with uploader name.
    ...
    result = await db.execute(
        select(
            QuoteAttachment.id,
            QuoteAttachment.quote_id,
            QuoteAttachment.file_key,
            QuoteAttachment.file_name,
            QuoteAttachment.file_size,
            QuoteAttachment.mime_type,
            QuoteAttachment.uploaded_by,
            QuoteAttachment.sort_order,
            QuoteAttachment.created_at,
            User.first_name,
            User.last_name,
        )
        .join(User, User.id == QuoteAttachment.uploaded_by, isouter=True)
        .where(
            QuoteAttachment.quote_id == quote_id,
            QuoteAttachment.org_id == org_id,
        )
        .order_by(QuoteAttachment.sort_order, QuoteAttachment.created_at)
    )
...
async def delete_attachment(
    ...
    *,
    ...
    Only allowed when the quote is in 'draft' status. Deletes the file
    from disk, removes the database record, and decrements the
    organisation's storage usage.

    Returns:
        dict with deletion confirmation and storage freed

    Raises:
        ValueError: Attachment not found, or quote is not in draft status
                    (router maps non-draft ValueError to 403)
    """
    org_id_str = str(org_id)

    # Validate quote exists, belongs to org, and is in draft status
...
async def get_attachment_count(
    ...
    *,
    ...
    """Return the number of attachments for a quote."""
    result = await db.execute(
        select(func.count()).select_from(QuoteAttachment).where(
            QuoteAttachment.quote_id == quote_id,
            QuoteAttachment.org_id == org_id,
