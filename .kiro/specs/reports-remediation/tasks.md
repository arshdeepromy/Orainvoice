...
- [x] 1. Backend A1 — Revenue `monthly_breakdown` + `total_invoices` alias
  - [x] 1.1 Add nested schema + fields to `RevenueSummaryResponse`
  ...
  - [x] 1.2 Compute `monthly_breakdown` + `total_invoices` in `get_revenue_summary`
  ...
  - [x]* 1.3 Write property test for revenue monthly breakdown (Hypothesis)
  ...
  - [x]* 1.4 Write unit/integration test for revenue endpoint
...
- [x] 2. Backend A5 — Fleet `vehicles[]` breakdown
  - [x] 2.1 Add `FleetVehicleRow` + `vehicles` to `FleetReportResponse`
  ...
  - [x] 2.2 Add per-vehicle aggregate to `get_fleet_report`
  ...
  - [x]* 2.3 Write unit test for fleet vehicles aggregate
    - Assert `vehicles[]` present; `Σ vehicles[i].total_spend <= total_spend`; empty list when no qualifying vehicles.
    - _Requirements: 6.1, 6.2_

- [x] 3. Backend A6 — Storage breakdown via `calculate_org_storage`
  - [x] 3.1 Populate real `breakdown` in `get_storage_usage`
  ...
  - [x]* 3.2 Write unit test for storage breakdown
    - Assert `breakdown` is non-empty when the org has storage data; `{category, bytes}` shape.
    - _Requirements: 7.1_

- [x] 4. Backend C4 — SMS `daily_breakdown`
  - [x] 4.1 Add `SmsDailyPoint` + `daily_breakdown` to `SmsUsageResponse`
  ...
  - [x] 4.2 Compute daily series in `get_sms_usage`
  ...
  - [x]* 4.3 Write property test for SMS daily breakdown (Hypothesis)
...
- [x] 5. Backend C3 — `GET /org/plan-sms-pricing` endpoint
  - [x] 5.1 Add `PlanSmsPricingResponse` schema
  ...
  - [x] 5.2 Add the `org_admin`-gated endpoint
  ...
  - [x]* 5.3 Write unit test for plan-sms-pricing endpoint
    - Assert tiers returned for a plan that has them; `[]` when the plan has none; `org_admin` guard enforced.
    - _Requirements: 8.1, 8.2_

- [x] 6. Backend C1 — Server-side export layer (`reports/export.py` + router wiring)
  - [x] 6.1 Create the CSV registry + renderers in `app/modules/reports/export.py`
  ...
  - [x] 6.2 Add the WeasyPrint PDF renderer (off-loop)
  ...
  - [x] 6.3 Add the `_maybe_export` router helper and wire it into every `/reports/*` endpoint
  ...
  - [x]* 6.4 Write property test for CSV round-trip (Hypothesis)
  ...
  - [x]* 6.5 Write integration tests for export endpoints
...
- [x] 7. Backend backward-compatibility checkpoint
  - [x]* 7.1 Write backward-compat assertion tests
  ...
  - [x] 7.2 Checkpoint — run the full backend pytest suite
    - Ensure all backend tests pass, ask the user if questions arise.

- [x] 8. Frontend A1 — Revenue tab field mapping + safety (`RevenueSummary.tsx`)
  - [x] 8.1 Map corrected fields and add D1/D3 safety
  ...
  - [x]* 8.2 Write Vitest+RTL test for Revenue tab
...
- [x] 9. Frontend A2 — Invoice Status tab (`InvoiceStatus.tsx`)
  - [x] 9.1 Map `breakdown`/`total`, compute total, add safety
...
- [x] 10. Frontend A3 — Top Services tab (`TopServices.tsx`)
  - [x] 10.1 Map `description`/`total_revenue`, add safety
...
- [x] 11. Frontend A4 + C2 + B1 — Outstanding tab (`OutstandingInvoices.tsx`)
  - [x] 11.1 Map fields, derive status, remove date filter
  ...
  - [x] 11.2 Fix Send Reminder + add D1/D2 safety
...
- [x] 12. Frontend A5 — Fleet tab + account picker (`FleetReport.tsx`)
  - [x] 12.1 Add fleet-account picker and render `vehicles[]`
...
- [x] 13. Frontend A6 — Storage tab (`StorageUsage.tsx`)
  - [x] 13.1 Render real breakdown + add AbortController
...
- [x] 14. Frontend C3 + C4 — SMS tab (`SmsUsage.tsx`)
  - [x] 14.1 Source tiers from plan endpoint + render daily chart
...
- [x] 15. Frontend B2 — Controlled `DateRangeFilter.tsx`
  - [x] 15.1 Derive preset from `value` + add `presetFromValue`
...
- [x] 16. Frontend B3 + D1/D2/D3 — Branch sourcing + remaining-tab safety pass
  - [x] 16.1 Fix branch sourcing in Customer Statement and verify all tabs
...
- [x] 17. Frontend C1 — Fixed `ExportButtons.tsx`
  - [x] 17.1 Send `export` param, parse filename, surface errors
...
- [x] 18. Frontend checkpoint
  - [x] 18.1 Checkpoint — typecheck/build and run frontend tests so far
    - Run the frontend build and Vitest (cwd `frontend-v2`). Ensure all tests pass, ask the user if questions arise.

- [x] 19. Frontend E1 — Rebuilt `ReportsPage` landing + overview hook
  - [x] 19.1 Create `useReportsOverview.ts` hook
  ...
  - [x] 19.2 Rebuild `ReportsPage.tsx` to the prototype
...
- [x] 20. Frontend E2 + E3 — `ReportLibrary` grouped, module-gated cards
  - [x] 20.1 Create `ReportLibrary.tsx` wiring all routes + module gating
