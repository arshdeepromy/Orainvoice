...
- [ ] 1. Database migrations: tables, RLS, seeds, and performance indexes
  - [ ] 1.1 Migration A (rev `0226`): tables, CHECK constraints, RLS, inline indexes, and seeds
    - Create a new Alembic revision under `alembic/versions/` parented on current head `0225`, revision id `0226` (short id per ISSUE-029 width limit), an ordinary transactional migration, idempotent throughout.
  ...
  - [ ] 1.2 Migration B (rev `0227`): performance indexes via `CREATE INDEX CONCURRENTLY`
    - Create a **separate** Alembic revision file `0227` parented on `0226`, following the canonical `0202_add_perf_indexes.py` template — all DDL inside `op.get_context().autocommit_block()` (mixing `CONCURRENTLY` with other DDL in one `upgrade()` is a banned pattern).
  ...
  - [ ] 1.3 Post-migration verification (run inside the app container)
  ...
  - [ ]* 1.4 Migration and RLS isolation tests
    - Assert both migrations apply, revert, and are idempotent on re-run against a copy of head `0225`; assert the `esignatures` `module_registry` row is seeded (R2.1 smoke). If the optional `feature_flags` catalogue row is seeded, assert it is keyed `key='esignatures'` and is removed on downgrade.
...
- [ ] 2. ORM models and Pydantic schemas
  - [ ] 2.1 Create `app/modules/esignatures/models.py`
  ...
  - [ ] 2.2 Create `app/modules/esignatures/schemas.py`
...
- [ ] 3. Pure status reducer (`status.py`)
  - [ ] 3.1 Implement `next_status(current, event, recipients_state) -> EnvelopeStatus | None`
  ...
  - [ ]* 3.2 Property test — terminal immutability
    - **Property 9: Terminal statuses are immutable under non-void events**
    - **Validates: Requirements 6.1, 6.6**

  - [ ]* 3.3 Property test — lifecycle transitions from non-terminal states
...
- [ ] 4. Pure validators (`validation.py`)
  - [ ] 4.1 Implement `is_pdf(...)`, `validate_recipients(...)`, and secret-compare helper
  ...
  - [ ]* 4.2 Property test — atomic, side-effect-free send validation
...
- [ ] 5. DocumensoClient and per-organisation connection loader (`app/integrations/documenso.py`)
  - [ ] 5.1 Implement the per-org connection loader and exception types
  ...
  - [ ] 5.2 Implement `DocumensoClient` over async httpx (Documenso REST API **v2**), instantiated per organisation
  ...
  - [ ]* 5.3 Property test — unconfigured integration fails every operation
    - **Property 4: Unconfigured integration fails every operation with a message** (including connection test)
    - **Validates: Requirements 1.9, 1.10**

  - [ ]* 5.4 Property test — HTTPS for all Documenso traffic
  ...
  - [ ]* 5.5 Property test — Documenso calls always use the calling org's own team-scoped token
  ...
  - [ ] 5.6 Implement the optional provisioning adapter (`app/integrations/documenso_provisioning.py`)
...
- [ ] 6. Checkpoint - Ensure all foundation tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 7. Error humanization helper and module gating dependencies
  - [ ] 7.1 Implement `humanize_esign_error(exc) -> EsignError` and shared FastAPI dependencies
  ...
  - [ ]* 7.2 Property test — error responses are human-readable and leak nothing
    - **Property 24: Error responses are human-readable and leak nothing**
    - **Validates: Requirements 15.5, 16.1, 16.2, 16.3**

  - [ ] 7.3 Wire the concrete module gate (middleware entry + router dependency)
  ...
  - [ ]* 7.4 Property/example test — module-disabled endpoints return 403 via the module gate
...
- [ ] 8. Service: create and send envelope (`service.py`)
  - [ ] 8.1 Implement `create_and_send_envelope(...)`
  ...
  - [ ] 8.2 Wire audit log + in-app notification on send (best-effort)
  ...
  - [ ]* 8.3 Property test — successful send persists a faithful envelope
    - **Property 7: Successful send persists a faithful envelope**
    - **Validates: Requirements 3.2, 3.6, 4.4, 10.3, 10.4, 13.1**

  - [ ]* 8.4 Property test — send validation atomic and side-effect-free (service layer)
  ...
  - [ ]* 8.5 Property test — Documenso failure records an error envelope
    - **Property 18: Documenso failure records an error envelope**
    - **Validates: Requirements 3.5**

  - [ ]* 8.6 Example test — send orchestration order
  ...
  - [ ] 8.7 Ensure a signature field per signer before send (R17)
  ...
  - [ ]* 8.8 Property test — every signer has a signature field before send
  ...
  - [ ]* 8.9 Property test — sends are blocked while the org's connection is missing or unverified
...
- [ ] 9. Service: void, list, and detail
  - [ ] 9.1 Implement `void_envelope(...)`
  ...
  - [ ] 9.2 Implement `list_envelopes(...)` and `get_envelope_detail(...)`
  ...
  - [ ]* 9.3 Property test — void allowed exactly when non-terminal
    - **Property 12: Void is allowed exactly when non-terminal**
    - **Validates: Requirements 5.4, 7.1, 7.2, 7.3**

  - [ ]* 9.4 Property test — dashboard filter, ordering, and detail correctness (fail-closed)
    - **Property 21: Dashboard filter, ordering, and detail are correct and fail-closed**
    - **Validates: Requirements 11.2, 11.3, 11.4, 11.5, 11.6**

  - [ ]* 9.5 Property test — multi-tenant isolation on read and list
    - **Property 20: Multi-tenant isolation on read and list**
    - **Validates: Requirements 11.1, 13.3, 13.4, 13.5, 13.6**

- [ ] 10. Routes and module-gate/RBAC wiring (`router.py`)
  - [ ] 10.1 Implement `/api/v2/esign` endpoints and register the router
  ...
  - [ ]* 10.2 Property test — module-disabled endpoints rejected
    - **Property 5: Module-disabled endpoints are rejected**
    - **Validates: Requirements 2.2**

  - [ ]* 10.3 Property test — RBAC for send and void
  ...
  - [ ]* 10.4 Example test — no trade-family gating
    - **Property 6: No trade-family gating** (enabling `esignatures` permitted for any trade family)
    - **Validates: Requirements 2.5**

- [ ] 11. Checkpoint - Ensure all service/route tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 12. Webhook handler (`webhook_router.py` + `service.apply_webhook`)
  - [ ] 12.1 Implement per-org-routed, shared-secret-gated webhook ingestion
  ...
  - [ ] 12.2 Implement idempotent apply + per-recipient update (scoped to the resolved org)
  ...
  - [ ]* 12.3 Property test — per-org webhook routing and shared-secret verification gates all processing
  ...
  - [ ]* 12.4 Property test — webhook processing is idempotent
  ...
  - [ ]* 12.5 Property test — webhooks for unmapped documents are no-ops
    - **Property 15: Webhooks for unmapped documents are no-ops**
    - **Validates: Requirements 8.5**

  - [ ]* 12.6 Property test — per-recipient status reflects latest event
    - **Property 11: Per-recipient status reflects the latest recipient event**
    - **Validates: Requirements 4.5**

  - [ ]* 12.7 Property test — transition records audit and notification
  ...
  - [ ]* 12.8 Property test — audit/notification side-effects are best-effort
    - **Property 17: Audit and notification side-effects are best-effort**
    - **Validates: Requirements 14.3**

  - [ ] 12.9 Wire the webhook into the public middleware surface and system DB context
  ...
  - [ ]* 12.10 Property/integration test — webhook reaches the handler through the middleware stack
...
- [ ] 13. Signed-document retrieval, storage, and scheduled sweep (`signed_document.py`)
  - [ ] 13.1 Implement retrieve + encrypted-pipeline store + attach
  ...
  - [ ] 13.2 Extend the staff documents listing to merge esign signed docs (Staff → Documents tab)
  ...
  - [ ] 13.3 Implement scheduled retry sweep
  ...
  - [ ]* 13.4 Property test — signed documents stored only via encrypted pipeline
  ...
  - [ ]* 13.5 Example test — retrieval invoked on completed transition
    - Assert reaching `completed` triggers signed-document retrieval (`download_signed`) with a mocked client.
    - _Requirements: 9.1_

- [ ] 14. Per-organisation Documenso connection service and endpoints (`connection_service.py`)
  - [ ] 14.1 Implement the per-org connection service (`app/modules/esignatures/connection_service.py`)
  ...
  - [ ]* 14.3 Property test — credential storage round-trip
    - **Property 1: Credential storage round-trip** (per-org `esign_org_connections`)
    - **Validates: Requirements 1.2, 15.1**

  - [ ]* 14.4 Property test — masked credentials never returned in plaintext
  ...
  - [ ]* 14.5 Property test — saving a masked value retains the stored secret
    - **Property 3: Saving a masked value retains the stored secret** (per-org connection)
    - **Validates: Requirements 1.5**

  - [ ]* 14.6 Example/integration tests — connection UI, team_id round-trip, connection test, credential-source guard
  ...
  - [ ] 14.7 Surface the org's webhook URL + subscription status and document manual per-org registration (R18/R19)
  ...
  - [ ]* 14.8 Example test — connection lifecycle
  ...
  - [ ] 14.9 Implement `auto_provision_connection(db, org_id)` (optional best-effort orchestration)
  ...
  - [ ]* 14.11 Property test — failed auto-provisioning never leaves broken state and preserves the manual path
...
- [ ] 16. Frontend: API client and sidebar entry (`frontend-v2/`)
  - [ ] 16.1 Implement typed esign API client functions
  ...
  - [ ] 16.2 Add "Agreements" sidebar entry gated on the `esignatures` module
    - Show/hide via module context; no trade-family gate.
    - _Requirements: 2.3, 2.4, 2.5_

  - [ ]* 16.3 Vitest — sidebar gating
    - Assert "Agreements" entry renders when module enabled and is hidden when disabled.
    - _Requirements: 2.3, 2.4_

- [ ] 17. Frontend: SendForSignatureModal and Agreements dashboard
  - [ ] 17.1 Implement reusable `SendForSignatureModal`
  ...
  - [ ] 17.2 Implement `AgreementsDashboardPage`
  ...
  - [ ]* 17.3 Vitest — send modal rendering
  ...
  - [ ] 17.4 Implement Void confirmation modal (destructive action)
  ...
  - [ ]* 17.5 Vitest — void confirmation + dashboard states
...
- [ ] 18. Frontend: contextual send actions and Global-Admin connection settings
  - [ ] 18.1 Add "Send for signature" actions on invoice pages, quote pages, and Staff → Documents tab
  ...
  - [ ]* 18.2 Vitest — contextual action gating
    - Assert the action shows/hides on invoice, quote, and Staff → Documents surfaces based on the module flag.
    - _Requirements: 10.1, 10.2, 10.5_

  - [ ] 18.3 Implement the Global-Admin per-org Documenso connection settings view
  ...
  - [ ]* 18.4 Vitest — connection settings page
  ...
  - [ ] 18.5 Add the Global-Admin Organisations-list "Provision e-signature" per-row action
  ...
  - [ ]* 18.6 Vitest — Organisations-list provision action states
...
- [ ] 19. External-signing smoke test and end-to-end script
  - [ ]* 19.1 Smoke test — one-time links, no account, no Documenso UI exposure
  ...
  - [ ]* 19.2 Mandatory end-to-end script (`scripts/test_esignature_e2e.py`)
...
- [ ] 20. Version bump and changelog
  - [ ] 20.1 Bump MINOR version and add changelog entry
...
- [ ] 21. Operational prerequisites (go-live gates, non-code)
  - [ ] 21.1 Confirm per-org/per-environment provisioning, prod signing certificate, webhook delivery, and secret rotation
...
- The database work is split across two migrations: Migration A (`0226`, transactional — **four** org-scoped tables including the per-org `esign_org_connections` connection record, CHECK constraints, RLS, inline indexes incl. `esign_org_connections` `UNIQUE(org_id)` + `UNIQUE(webhook_routing_id)`, the mandatory `module_registry` seed + an optional `feature_flags` catalogue/visibility row keyed `key='esignatures'`) and Migration B (`0227`, `CREATE INDEX CONCURRENTLY` in an `autocommit_block`). They are separate files because mixing `CONCURRENTLY` with other DDL is a banned pattern; Task 1.3 verifies them in the app container.
